    ...
    <div style={{ background: DARK, minHeight: '100vh', display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '14px 16px', borderBottom: `1px solid ${BORDER}`, background: RAISED }}>
        ...
        <button
          onClick={handleSave}
          disabled={!imgSrc}
          style={{ background: imgSrc ? GOLD : 'rgba(212,160,23,.3)', border: 'none', borderRadius: 8, padding: '8px 16px', color: '#F2F2F7', fontWeight: 700, fontSize: 13, cursor: imgSrc ? 'pointer' : 'not-allowed' }}
        >
          Attach
        </button>
      </div>
      ...
      {!imgSrc ? (
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 16, padding: 24 }}>
          ...
          <button
            ...
            style={{ padding: '12px 28px', background: GOLD, border: 'none', borderRadius: 10, color: '#F2F2F7', fontWeight: 700, fontSize: 15, cursor: 'pointer' }}
          >
            Choose Photo
          </button>
          ...
          {pendingPos && (
            <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
              ...
              <button
                ...
                style={{ padding: '9px 18px', background: newText.trim() ? GOLD : 'rgba(212,160,23,.3)', border: 'none', borderRadius: 8, color: '#F2F2F7', fontWeight: 700, fontSize: 13, cursor: newText.trim() ? 'pointer' : 'not-allowed' }}
              >
                Add
              </button>
            </div>
