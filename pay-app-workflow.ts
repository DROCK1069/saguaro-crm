...
export async function createPayApplicationHandler(req: NextRequest) {
  ...
  const tenantId   = g.user.tenantId; // from auth — never trust a client-supplied tenantId
...
export async function recordPaymentHandler(req: NextRequest, payAppId: string) {
  ...
  if (!amountPaid) return NextResponse.json({ error: 'amountPaid required' }, { status: 400 });
