...
export async function GET(
  ...
  if (seg0 === 'marketing' && !seg1) {
    try {
      ...
      return new Response(
        `<!DOCTYPE html><html><head><title>Saguaro Control Systems</title></head><body style="background:#0a0a0a;color:#e8edf8;font-family:system-ui;display:flex;align-items:center;justify-content:center;min-height:100vh;flex-direction:column;gap:16px"><div style="font-size:48px">🌵</div><h1 style="color:#D4A017;margin:0">Saguaro Control Systems</h1><a href="/sandbox" style="background:#D4A017;color:#0a0a0a;padding:12px 24px;border-radius:8px;text-decoration:none;font-weight:800">Start Free Sandbox</a><a href="/login" style="color:#8fa3c0;text-decoration:none">Sign In</a></body></html>`,
...
export async function POST(
  ...
  if (seg0 === 'autopilot' && seg1 === 'dismiss') {
    // Auth + tenant scope — service-role bypasses RLS, so an id-only update would
    // let any caller dismiss another tenant's alert (cross-tenant IDOR).
    const user = await getUserFromRequest(req);
    if (!user) return NextResponse.json({ error: 'Unauthorized' }, { status: 401 });
    const body = await req.json().catch(() => ({})) as Record<string, unknown>;
  ...
  if (seg0 === 'change-orders' && seg1 && seg2 === 'approve') {
    // Auth + tenant scope — service-role bypasses RLS, so an id-only update would
    // let any caller approve another tenant's change order (cross-tenant IDOR).
    const user = await getUserFromRequest(req);
    if (!user) return NextResponse.json({ error: 'Unauthorized' }, { status: 401 });
    try {
      const { data, error } = await supabaseAdmin.from('change_orders').update({ status: 'approved', approved_at: new Date().toISOString() }).eq('id', seg1).eq('tenant_id', user.tenantId).select('id').maybeSingle();
      if (error) throw error;
      if (!data) return NextResponse.json({ error: 'Change order not found' }, { status: 404 });
