...
import { takeoffShareToken, newShareNonce, SHARE_TTL_MS } from '@/lib/takeoff/share';

export const runtime = 'nodejs';

/**
 * GET /api/takeoff/share?id=  → mint (or reuse) a read-only estimate share URL (owner-only).
...
export async function GET(req: NextRequest) {
  ...
  const { data: row, error } = await supabase
    ...
    .single();
  if (error || !row) return Response.json({ error: 'Takeoff not found' }, { status: 404 });
  ...
  const token = takeoffShareToken(id, nonce as string, expMs);
  const path = `/share/takeoff/${id}?t=${token}&e=${expMs}`;
  return Response.json({ token, path, url: `${new URL(req.url).origin}${path}`, expiresAt: new Date(expMs).toISOString() });
}

/** DELETE /api/takeoff/share?id=  → revoke every share link for a takeoff (owner-only). */
export async function DELETE(req: NextRequest) {
  ...
  const { data, error } = await supabase
    ...
    .select('id')
    .single();
  if (error || !data) return Response.json({ error: 'Takeoff not found' }, { status: 404 });
