...
export async function GET(req: NextRequest, { params }: { params: { projectId: string } }) {
  ...
  try {
    ...
    const [incidents, inspections] = await Promise.all([
      supabase.from('safety_incidents').select('*').eq('project_id', params.projectId).eq('tenant_id', user.tenantId).order('incident_date', { ascending: false }),
      supabase.from('safety_inspections').select('*').eq('project_id', params.projectId).eq('tenant_id', user.tenantId).order('inspection_date', { ascending: false }),
    ]);
