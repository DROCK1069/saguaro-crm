...
export async function PATCH(req: NextRequest, { params }: { params: { projectId: string; id: string } }) {
  ...
  try {
    ...
    const { data: existingForm } = await supabase
      .from('prequalification_forms').select('form_data').eq('id', params.id).maybeSingle();
    if (existingForm) {
      ...
      const res = await supabase.from('prequalification_forms').update(formUpdates).eq('id', params.id).select().single();
      ...
      const res = await supabase.from('prequalification_submissions').update(subUpdates).eq('id', params.id).select().single();
      data = res.data; error = res.error;
    }
