...
import { toWinAnsi } from '@/lib/winansi';
...
async function buildPdf(
  ...
  function drawPageHeader(p: ReturnType<typeof pdfDoc.addPage>): void {
    ...
    p.drawText(toWinAnsi(brandName), { x: textX, y: PAGE_H - HEADER_H + 28, size: 13, font: boldFont, color: GOLD });
    p.drawText(toWinAnsi(title),     { x: textX, y: PAGE_H - HEADER_H + 13, size: 11, font: boldFont, color: WHITE });

    // Sub-header bar
    p.drawRectangle({ x: 0, y: PAGE_H - HEADER_H - SUBHDR_H, width: PAGE_W, height: SUBHDR_H, color: MGRAY });
    p.drawText(toWinAnsi(`Generated: ${dateStr}`), { x: MARGIN, y: PAGE_H - HEADER_H - SUBHDR_H + 6, size: 8, font, color: DARK });
    p.drawText(toWinAnsi(`${rows.length} records`), { x: PAGE_W - MARGIN - 70, y: PAGE_H - HEADER_H - SUBHDR_H + 6, size: 8, font, color: DARK });
  }
  ...
  function drawColumnHeaders(p: ReturnType<typeof pdfDoc.addPage>, yPos: number): void {
    ...
    for (let i = 0; i < columns.length; i++) {
      ...
      p.drawText(toWinAnsi(label), { x, y: yPos - COL_HDR_H + 5, size: 7, font: boldFont, color: WHITE });
      x += colWidths[i];
    }
  }
  ...
  function drawFooter(p: ReturnType<typeof pdfDoc.addPage>, pNum: number): void {
    ...
    p.drawText(toWinAnsi(`Page ${pNum}`), { x: MARGIN, y: 6, size: 8, font, color: DARK });
    p.drawText(toWinAnsi(`Generated by ${brandName} — ${dateStr}`), { x: PAGE_W / 2 - 100, y: 6, size: 8, font, color: DARK });
  }
  ...
  for (const row of rows) {
    ...
    for (let i = 0; i < columns.length; i++) {
      ...
      page.drawText(toWinAnsi(text), { x, y: y - ROW_H + 4, size: 8, font, color: DARK });
      x += colWidths[i];
    }
    y -= ROW_H;
  }
  ...
  if (totals && Object.keys(totals).length > 0) {
    ...
    for (let i = 0; i < columns.length; i++) {
      ...
      page.drawText(toWinAnsi(text), { x, y: y - ROW_H + 2, size: 8, font: boldFont, color: GOLD });
      x += colWidths[i];
    }
    y -= ROW_H + 4;
  }
