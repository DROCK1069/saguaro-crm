 ...
 * (`Authorization: Bearer sk_live_…`). Returns contracts scoped to the key's
 * tenant, optionally filtered to a single project.
 *
 * Query params:
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    let q = db
      .from('contracts')
      .select(
        'id, project_id, contract_number, title, contract_type, party_name, trade, amount, status, executed_at, created_at',
      )
