 ...
 * (`Authorization: Bearer sk_live_…`). Returns pay applications scoped to the
 * key's tenant, optionally filtered to a single project.
 *
 * Query params:
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    let q = db
      ...
      .select(
        'id, project_id, app_number, status, current_payment_due, period_from, period_to, submitted_at, certified_at, created_at',
      )
