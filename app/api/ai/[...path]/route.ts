...
async function robustChatHandler(req: NextRequest): Promise<Response> {
  ...
  // Resolve the authenticated user (cookie/bearer) up front — does not touch the
  // request body, so it's safe before we read JSON inside the stream. Gives us the
  // tenant_id we need to load + tenant-scope live project context.
  ...
  const stream = new ReadableStream({
    async start(controller) {
      try {
        ...
        // Inject LIVE, tenant-scoped project context when we know the project + tenant.
        // This is what makes "I have full context on this project" true — without it the
        // model only has general construction expertise. Fail-open: if it can't load, we
        // still answer with the base expert prompt.
        let systemPrompt = CONSTRUCTION_SYSTEM_PROMPT;
        const projectId = typeof body.projectId === 'string' ? body.projectId : undefined;
        if (projectId && user?.tenantId) {
          const contextBlock = await buildProjectContext(createServerClient(), user.tenantId, projectId);
          if (contextBlock) {
            systemPrompt = `${CONSTRUCTION_SYSTEM_PROMPT}\n\n## CURRENT PROJECT\nYou are answering inside a specific project workspace. The block below is live, authoritative data pulled from this project's records. Ground every project-specific answer (budget, RFIs, change orders, subs, schedule, daily logs) in it, cite the real numbers, and say so plainly if the data doesn't cover what was asked.\n\n${contextBlock}`;
          }
        }
