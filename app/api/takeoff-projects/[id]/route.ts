  ...
  try {
    ...
    const [projectRes, sheetsRes, lineItemsRes] = await Promise.all([
      ...
      db
        ...
        // No FK from takeoff_line_items → takeoff_assemblies exists (assembly_id lives in
        // metadata), so the old `takeoff_assemblies(*)` embed threw PGRST200 and 500'd this
        // whole load. Select the row flat; the client fetches assemblies separately.
        .select('*')
        .eq('takeoff_project_id', id)
        .eq('tenant_id', user.tenantId)
        .order('sort_order', { ascending: true }),
