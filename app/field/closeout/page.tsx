'use client';
/**
 * Saguaro Control Systems — Project Closeout Tracking
...
async function api(url: string, opts?: RequestInit) {
  if (navigator.onLine) {
    ...
    if (!r.ok) { console.error('closeout api error', r.status); throw new Error('request failed'); }
    return r.json();
  }
...
function CloseoutInner() {
  ...
  useEffect(() => {
    ...
    api(base)
      ...
      .catch((e) => { console.error(e); setError(humanError(e, 'Failed to load closeout items. Please try again.')); })
  ...
  async function patchItem(id: string, patch: Partial<CloseoutItem>) {
    ...
    } catch (e: unknown) { console.error(e); setError(humanError(e, 'Update failed. Please try again.')); }
  }
  ...
  /* ── styles ────────────────────────────────────────────────────── */
  const page: React.CSSProperties = { minHeight: '100vh', background: '#0a0a0a', color: TEXT, fontFamily: 'system-ui, sans-serif', paddingBottom: 80 };
  ...
  function renderProgressBar(value: number, max: number, color: string) {
    ...
    return (
      <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
        <div style={{ flex: 1, height: 10, background: '#1c1c1e', borderRadius: 5, overflow: 'hidden' }}>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                  <span style={{ width: 22, height: 22, borderRadius: 4, display: 'flex', alignItems: 'center', justifyContent: 'center', background: complete ? GREEN : 'transparent', border: `2px solid ${complete ? GREEN : BORDER}`, fontSize: 14, color: '#fff' }}>
                    {complete ? <Check size={14} weight="bold" color="#fff" /> : ''}
                  </span>
  ...
  function renderList() {
    return (
      <div style={{ padding: '0 20px 20px' }}>
        ...
        {/* bulk bar */}
        {bulkIds.size > 0 && (
          <div style={{ ...card, display: 'flex', alignItems: 'center', gap: 12, background: '#141416' }}>
  ...
  return (
    <div style={page}>
      ...
      <FieldPageHeader
        ...
        subtitle={`${stats.accepted}/${stats.total} accepted — ${stats.pct}% complete`}
        icon={scopedFieldIcon('inspections', 'ph')}
        actions={<button style={{ ...btnOutline, fontSize: 11, padding: '6px 10px' }} onClick={exportPDF} title="Export PDF">Print</button>}
      />
...
export default function CloseoutPage() {
  return (
    <Suspense fallback={<div style={{ minHeight: '100vh', background: '#0a0a0a', color: '#CBD5E1', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>Loading...</div>}>
      <CloseoutInner />
    </Suspense>
  );
}