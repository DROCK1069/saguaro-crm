'use client';
/**
 * Saguaro Control Systems -- AR Blueprint Overlay
            ...
            <div style={{ ...glass, padding: 24, textAlign: 'center', marginBottom: 16 }}>
              ...
              {xrSupported && (
                <button onClick={enterXR} style={{
                  ...
                  color: BLUE, background: 'rgba(245,158,11,0.1)', border: `1px solid rgba(245,158,11,0.3)`,
        ...
        <div style={{ position: 'relative' }}>
            ...
            <div style={{
              ...
              {calibStep === 'point_a' && (
                <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                  <div style={{ width: 28, height: 28, borderRadius: '50%', background: 'rgba(245,158,11,0.2)', border: `2px solid ${BLUE}`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 14, fontWeight: 800, color: BLUE }}>1</div>
                  <div>
          ...
          >
            ...
            {showTakeoff && takeoffDims.length > 0 && (
              <div style={{
                ...
                {takeoffDims.map((d, i) => (
                  <div key={i} style={{ display: 'flex', justifyContent: 'space-between', padding: '3px 0', borderBottom: '1px solid rgba(0,0,0,0.04)' }}>
          ...
          <div style={{
            ...
            <button onClick={() => { setMeasuring(!measuring); setPendingPoint(null); }} style={{
              ...
              background: measuring ? GOLD : 'rgba(20,20,22,0.7)',
            ...
            <button onClick={() => { setMeasurements([]); setPendingPoint(null); }} style={{
              ...
              background: 'rgba(20,20,22,0.7)', color: RED,
              border: '1px solid rgba(0,0,0,0.06)',
            }}>
              Clear
            </button>
            ...
            <button onClick={undo} disabled={measurements.length === 0 && !pendingPoint} style={{
              padding: '10px 12px', borderRadius: 12, cursor: 'pointer', fontSize: 12, fontWeight: 700,
              background: 'rgba(20,20,22,0.7)', color: (measurements.length === 0 && !pendingPoint) ? 'rgba(139,170,200,0.3)' : DIM,
              border: '1px solid rgba(0,0,0,0.06)',
            }}>
              <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2} width={16} height={16}>
            ...
            <button onClick={saveMeasurements} disabled={saving || measurements.length === 0} style={{
              flex: 1, padding: '10px 0', borderRadius: 12, cursor: 'pointer', fontSize: 12, fontWeight: 700,
              display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 4,
              background: (saving || measurements.length === 0) ? 'rgba(20,20,22,0.7)' : GREEN,
            ...
            <button onClick={() => setUnit(u => u === 'ft' ? 'mm' : 'ft')} style={{
              ...
              background: 'rgba(20,20,22,0.7)', color: BLUE,
              border: '1px solid rgba(0,0,0,0.06)',
            }}>
              {unit}
            </button>
            ...
            <button onClick={() => setShowTakeoff(!showTakeoff)} style={{
              padding: '10px 12px', borderRadius: 12, cursor: 'pointer', fontSize: 12, fontWeight: 700,
              background: showTakeoff ? 'rgba(245,158,11,0.15)' : 'rgba(20,20,22,0.7)',
              color: showTakeoff ? BLUE : DIM,
              border: showTakeoff ? `1px solid rgba(245,158,11,0.3)` : '1px solid rgba(0,0,0,0.06)',
            }}>
              &#x1F4D0;
            </button>
          </div>
        ...
        <div style={{ padding: '0 16px' }}>
          <div style={{ ...glass, padding: 14, marginTop: 16 }}>
            ...
            {measurements.map((m, i) => (
              <div key={m.id} style={{ display: 'flex', justifyContent: 'space-between', padding: '8px 0', borderBottom: '1px solid rgba(0,0,0,0.04)' }}>
...
export default function FieldAROverlayPage() {
  return (
    <Suspense fallback={<div style={{ padding: 32, color: '#CBD5E1', textAlign: 'center', background: '#0a0a0a', minHeight: '100dvh' }}>Loading AR...</div>}>
      <AROverlayPage />
    </Suspense>
  );
}
