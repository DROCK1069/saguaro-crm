'use client';
/**
 * Saguaro Control Systems — Team Contacts
...
import { MagnifyingGlass, UsersThree, Phone, Envelope, ChatCircle } from '@phosphor-icons/react';
  ...
  return (
    <div style={{ padding: '18px 16px' }}>
      <FieldPageHeader
        ...
        icon={scopedFieldIcon('directory', 'ph')}
      />

      {/* Search */}
      <div style={{ position: 'relative', marginBottom: 16 }}>
        <span style={{ position: 'absolute', left: 12, top: '50%', transform: 'translateY(-50%)', color: DIM, fontSize: 16, display: 'inline-flex', alignItems: 'center' }}><MagnifyingGlass size={16} weight="regular" color={DIM} /></span>
        <input
          value={search}
      ...
      {loading ? (
        ...
        <div style={{ textAlign: 'center', padding: '40px 16px', color: DIM }}>
          <div style={{ fontSize: 40, marginBottom: 8 }}><UsersThree size={40} weight="regular" color={DIM} /></div>
      ...
      <div style={{ display: 'flex', gap: 8, flexShrink: 0 }}>
        {m.phone && (
          <a
            ...
            title={`Call ${m.name}`}
          >
            <Phone size={18} weight="regular" color={GREEN} />
          </a>
        )}
        {m.email && (
          <a
            ...
            title={`Email ${m.name}`}
          >
            <Envelope size={18} weight="regular" color={BLUE} />
          </a>
        )}
        {m.phone && (
          <a
            ...
            title={`Text ${m.name}`}
          >
            <ChatCircle size={18} weight="regular" color={GOLD} />
          </a>
        )}
      </div>
    </div>
