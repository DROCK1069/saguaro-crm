'use client';
/**
 * Saguaro Control Systems — Equipment & Tool Tracking
...
function EquipmentPage() {
  ...
  return (
    <div style={{ padding: '18px 16px', paddingBottom: 40 }}>
      <FieldPageHeader
        title="Equipment Log"
        subtitle={<>{projectName}{projectName ? ' · ' : ''}{todayDisplay()}</>}
        icon={scopedFieldIcon('equipment', 'ph')}
        actions={view === 'list' ? (
          <button
        ...
        <form onSubmit={submitEntry}>
          ...
          <div style={{ display: 'flex', gap: 10 }}>
            ...
            <button
              type="submit"
              disabled={saving}
              style={{ flex: 2, background: saving ? 'rgba(255,255,255,0.12)' : GOLD, border: 'none', borderRadius: 12, padding: '15px', color: saving ? DIM : '#000', fontSize: 15, fontWeight: 800, cursor: saving ? 'wait' : 'pointer' }}
            >
