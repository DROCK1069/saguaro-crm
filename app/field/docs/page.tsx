'use client';
/**
 * Saguaro Control Systems — Document Viewer with Markup
...
function DocsPage() {
  ...
  // Build the version list from REAL file metadata (was fabricated random
  // history with invented uploader names). project_files carries the real
  // uploader, size, timestamp and version_number, so each file shows its one
  // true current version until a new one is actually uploaded.
  useEffect(() => {
    const vMap: Record<string, VersionEntry[]> = {};
    files.forEach((f) => {
      const anyF = f as Record<string, any>;
      vMap[f.id] = [{
        version: Number(anyF.version_number) || 1,
        uploadedBy: anyF.uploaded_by_name || anyF.uploaded_by || '',
        date: getFileDate(f),
        fileSize: getFileSize(f),
        changeNotes: 'Original upload',
        isCurrent: true,
      }];
    });
    setDocVersions(vMap);
  ...
  async function handleVersionFileSelected(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    // REAL upload (was a local-only fake that toasted success without persisting).
    // Persists a new file row via the canonical upload route, then reloads the
    // project's files so the upload actually appears.
    try {
      const fd = new FormData();
      fd.append('file', file);
      if (projectId) fd.append('projectId', projectId);
      const cat = (selected as Record<string, any>).category;
      if (cat) fd.append('category', String(cat));
      const res = await fetch('/api/files/upload', { method: 'POST', body: fd });
      const data = await res.json().catch(() => ({} as any));
      if (!res.ok || data?.error) {
        console.error('doc upload failed', res.status, data?.error);
        showToast(humanError(data?.error, 'Upload failed. Please try again.'));
      } else {
        if (projectId) {
          const r = await fetch(`/api/projects/${projectId}/files`);
          if (r.ok) { const d = await r.json(); setFiles(d.files || []); }
        }
        showToast('New version uploaded to project files');
      }
    } catch {
  ...
  if (selected) {
    ...
    const containerStyle: React.CSSProperties = fullScreen
      ...
      : { padding: '18px 16px', minHeight: '100vh', background: '#0a0a0a' };
      ...
      <div style={containerStyle}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: fullScreen ? '12px 16px' : '0 0 12px', background: fullScreen ? 'rgba(6,12,21,.95)' : 'transparent' }}>
          ...
          <div style={{ display: 'flex', gap: 6 }}>
            ...
            <button onClick={() => { setShowVersionHistory(!showVersionHistory); setShowDocInfo(false); }} style={{ ...iconBtn, background: showVersionHistory ? 'rgba(245,158,11,0.15)' : undefined, color: showVersionHistory ? BLUE : DIM }} title="Version History">
              <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2} width={18} height={18}>
                <circle cx={12} cy={12} r={10}/><polyline points="12 6 12 12 16 14"/>
              </svg>
            </button>
  ...
  return (
    <div style={{ padding: '18px 16px', minHeight: '100vh', background: '#0a0a0a' }}>
      {/* Hidden file input for version uploads */}
      <input
        ref={fileInputRef}
        type="file"
...
const iconBtn: React.CSSProperties = {
  background: '#1c1c1e', border: `1px solid rgba(255,255,255,0.12)`, borderRadius: 8, padding: 6, color: '#CBD5E1', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center',
};
