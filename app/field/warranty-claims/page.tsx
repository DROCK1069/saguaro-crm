'use client';
/**
 * Saguaro Control Systems — Warranty Claims
...
function WarrantyClaimsInner() {
  ...
  const fetchClaims = useCallback(async () => {
    ...
    try {
      ...
      console.error(e); setError(humanError(e, 'Failed to load claims. Please try again.'));
    } finally {
      setLoading(false);
    }
  }, [projectId]);
  ...
  const handleStatusUpdate = async (claimId: string, newStatus: string) => {
    ...
    try {
      ...
      console.error(e); setError(humanError(e, 'Failed to update the status. Please try again.'));
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleResolution = async (claimId: string) => {
    ...
    try {
      ...
      console.error(e); setError(humanError(e, 'Failed to submit the resolution. Please try again.'));
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleAssign = async (claimId: string, name: string, email?: string, phone?: string) => {
    ...
    try {
      ...
      console.error(e); setError(humanError(e, 'Failed to assign. Please try again.'));
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleDelete = async (claimId: string) => {
    ...
    try {
      ...
      console.error(e); setError(humanError(e, 'Failed to delete. Please try again.'));
    } finally {
      setSaving(false);
