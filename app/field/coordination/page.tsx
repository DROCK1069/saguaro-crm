'use client';
/**
 * Saguaro Control Systems — Coordination Issues (BIM / Field Clash Tracking)
...
const TYPE_ICON: Record<string, React.ComponentType<any>> = {
  bim_clash:       Lightning,
  field_conflict:  Wrench,
  design_conflict: CompassTool,
  rfi_related:     Clipboard,
  scope_gap:       Ruler,
  coordination:    LinkSimple,
  other:           PushPin,
};
...
function typeIcon(t: string, size = 16): React.ReactNode {
  const Ico = TYPE_ICON[t] || PushPin;
  return <Ico size={size} weight="regular" style={{ verticalAlign: 'middle' }} />;
}
...
function CoordinationPage() {
        ...
        <div style={{ padding: '0 16px 8px', display: 'flex', flexWrap: 'wrap', gap: 6 }}>
          {Object.entries(summary.byType).map(([type, count]) => (
            <span key={type} style={{ fontSize: 11, background: `rgba(${hexRgb(BORDER)},0.5)`, borderRadius: 8, padding: '3px 8px', color: DIM }}>
              {typeIcon(type, 13)} {typeLabel(type)}: {count}
            </span>
          ))}
        </div>
      )}
      ...
      <div style={{ padding: '0 16px 12px', display: 'flex', gap: 6, alignItems: 'center' }}>
        ...
        {(['priority', 'date', 'cost'] as const).map(s => (
          <button key={s} onClick={() => { if (sortBy === s) setSortDir(d => d === 'asc' ? 'desc' : 'asc'); else { setSortBy(s); setSortDir('desc'); } }}
            ...
            {s.charAt(0).toUpperCase() + s.slice(1)} {sortBy === s ? (sortDir === 'desc' ? <ArrowDown size={12} weight="regular" style={{ verticalAlign: 'middle' }} /> : <ArrowUp size={12} weight="regular" style={{ verticalAlign: 'middle' }} />) : ''}
          </button>
        ))}
    ...
    return (
      <div style={{ minHeight: '100vh', background: '#0a0a0a', color: TEXT }}>
        ...
        <div style={{ padding: 16 }}>
          <button onClick={() => setView('list')} style={backBtn}><ArrowLeft size={16} weight="regular" color={DIM} style={{ marginRight: 4 }} />Back</button>

          {/* Header */}
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 12 }}>
            <h2 style={{ fontSize: 18, fontWeight: 700, margin: 0 }}>
              {typeIcon(selected.issue_type, 20)} {selected.issue_number ? `#${selected.issue_number} — ` : ''}{selected.title}
            </h2>
              ...
              <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                {avail.map(ns => (
                  <button key={ns} onClick={() => handleStatusChange(selected, ns)}
                    ...
                    <ArrowRight size={14} weight="regular" color={statusColor(ns)} style={{ verticalAlign: 'middle', marginRight: 4 }} />{STATUS_LABELS[ns]}
                  </button>
                ))}
              </div>
            </div>
          ...
          <button onClick={() => {
            ...
            <Printer size={16} weight="regular" color={TEXT} style={{ verticalAlign: 'middle', marginRight: 6 }} />Export Issue PDF
          </button>
        </div>
      </div>
    );
  ...
  if (view === 'create') return (
    <div style={{ minHeight: '100vh', background: '#0a0a0a', color: TEXT }}>
      {toast && <div style={toastStyle}>{toast}</div>}
      <div style={{ padding: 16 }}>
        <button onClick={() => setView('list')} style={backBtn}><ArrowLeft size={16} weight="regular" color={DIM} style={{ marginRight: 4 }} />Cancel</button>
          ...
          <div style={{ display: 'flex', gap: 4, flexWrap: 'wrap', marginTop: 6 }}>
            {form.trades_involved.map(t => (
              <span key={t} onClick={() => toggleTrade(t)} style={{ fontSize: 11, background: `rgba(${hexRgb(BLUE)},0.15)`, color: BLUE, borderRadius: 6, padding: '2px 8px', cursor: 'pointer' }}>
                {t} <X size={10} weight="bold" color={BLUE} style={{ verticalAlign: 'middle' }} />
              </span>
            ))}
          </div>
        )}
        ...
        <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginBottom: 8 }}>
          {photoPreview.map((src, i) => (
            <div key={i} style={{ position: 'relative' }}>
              ...
              <button onClick={() => removePhoto(i)} style={{ position: 'absolute', top: -6, right: -6, width: 20, height: 20, borderRadius: '50%', background: RED, color: '#fff', border: 'none', fontSize: 12, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><X size={12} weight="bold" color="#fff" /></button>
            </div>
          ))}
        </div>
        <label style={{ display: 'inline-flex', alignItems: 'center', gap: 6, padding: '8px 14px', background: `rgba(${hexRgb(BLUE)},0.15)`, border: `1px solid ${BLUE}`, borderRadius: 10, color: BLUE, fontSize: 13, cursor: 'pointer', fontWeight: 600 }}>
          <Camera size={16} weight="regular" color={BLUE} />Capture Photo
...
const filterSelect: React.CSSProperties = {
  background: '#1c1c1e', border: `1px solid ${BORDER}`, borderRadius: 8, padding: '6px 8px',
