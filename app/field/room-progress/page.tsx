...
const GOLD='#F59E0B',BASE='#0a0a0a',TEXT='#FFFFFF',DIM='#CBD5E1',GREEN='#22C55E',BLUE='#F59E0B',RED='#EF4444',BORDER='rgba(255,255,255,0.12)';
const glass:React.CSSProperties={background:'rgba(20,20,22,0.85)',backdropFilter:'blur(16px)',WebkitBackdropFilter:'blur(16px)',border:'1px solid rgba(255,255,255,0.08)',borderRadius:16};
...
<div style={{height:8,background:'rgba(255,255,255,0.08)',borderRadius:6,overflow:'hidden'}}><div style={{height:'100%',width:avg+'%',background:heatColor(avg),borderRadius:6,transition:'width 0.4s'}}/></div>
...
{loading?(<div style={{display:'flex',flexDirection:'column',gap:8}}>{[1,2,3].map(i=><div key={i} style={{...glass,padding:16,height:80}}><div style={{background:'rgba(255,255,255,0.06)',borderRadius:8,height:14,width:'50%',marginBottom:8}}/><div style={{background:'rgba(255,255,255,0.05)',borderRadius:6,height:8,width:'100%'}}/></div>)}</div>
):filtered.length===0?(<div style={{...glass,padding:'40px 24px',textAlign:'center'}}><div style={{marginBottom:10}}><Crane size={40} weight="duotone" color={DIM}/></div><p style={{margin:'0 0 4px',fontSize:15,fontWeight:700,color:TEXT}}>{rooms.length===0?'No Rooms Yet':'No Matching Rooms'}</p><p style={{margin:0,fontSize:12,color:DIM}}>{rooms.length===0?'Draw rooms on the floor plan or add them above.':'Try adjusting your filters.'}</p></div>
...
<div style={{...glass,padding:'10px 14px',marginTop:16}}><p style={{margin:'0 0 6px',fontSize:10,fontWeight:700,color:DIM}}>Legend</p><div style={{display:'flex',gap:8,flexWrap:'wrap'}}>{[{c:'#6E6E73',l:'Not started'},{c:'#EF4444',l:'1-25%'},{c:'#F97316',l:'26-50%'},{c:'#EAB308',l:'51-75%'},{c:'#F59E0B',l:'76-99%'},{c:'#22C55E',l:'Complete'}].map(h=>(<div key={h.l} style={{display:'flex',alignItems:'center',gap:4}}><div style={{width:12,height:12,borderRadius:3,background:h.c}}/><span style={{fontSize:10,color:DIM}}>{h.l}</span></div>))}</div></div>
<style>{String.raw`input[type=range]{-webkit-appearance:none;height:6px;border-radius:4px;background:rgba(255,255,255,0.12);outline:none}input[type=range]::-webkit-slider-thumb{-webkit-appearance:none;width:18px;height:18px;border-radius:50%;cursor:pointer}`}</style>
</div>
);
}
export default function FieldRoomProgressPage(){return(<Suspense fallback={<div style={{padding:32,color:'#CBD5E1',textAlign:'center'}}>Loading...</div>}><RoomProgressPage/></Suspense>);}
