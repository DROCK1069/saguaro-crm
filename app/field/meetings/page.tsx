'use client';
/**
 * Saguaro Control Systems — Meeting Minutes
...
const inputStyle: React.CSSProperties = {
  background: '#1c1c1e', border: `1px solid ${BORDER}`, borderRadius: 8, padding: '10px 14px',
  ...
  const renderList = () => (
    <div>
      <FieldPageHeader
        ...
        actions={!online ? <span style={badgeStyle(AMBER)}>Offline</span> : undefined}
      />
      ...
      <div style={{ display: 'flex', gap: 6, marginBottom: 16, background: '#141416', borderRadius: 10, padding: 4 }}>
        ...
        filtered.map(m => {
          ...
          return (
            <div key={m.id} onClick={() => selectMode ? toggleSelectId(m.id) : openDetail(m)} style={{
              ...
              background: isChecked ? 'rgba(245, 158, 11,.05)' : RAISED,
            }}>
    ...
    <div>
      ...
      <div style={cardStyle}>
        ...
        {form.attendees.map((att, idx) => (
          <div key={att.id} style={{ background: '#1c1c1e', borderRadius: 10, padding: 12, marginBottom: 8, border: `1px solid ${BORDER}` }}>
      ...
      <div style={cardStyle}>
        ...
        {form.agenda.map((item, idx) => (
          <div key={item.id} style={{ background: '#1c1c1e', borderRadius: 10, padding: 12, marginBottom: 8, border: `1px solid ${BORDER}` }}>
    ...
    return (
      <div>
        ...
        {seriesMeetings.length > 1 && (
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12, padding: '8px 12px', background: '#141416', borderRadius: 10 }}>
            <button
                ...
                {(item.decisions || []).length > 0 && (
                  <div style={{ marginLeft: 28, marginTop: 8 }}>
                    ...
                    {item.decisions.map(d => (
                      <div key={d.id} style={{ padding: '6px 10px', background: '#1c1c1e', borderRadius: 8, marginBottom: 4, border: `1px solid ${BORDER}` }}>
                ...
                {(item.action_items || []).length > 0 && (
                  <div style={{ marginLeft: 28, marginTop: 8 }}>
                    ...
                    {item.action_items.map(ai => {
                      ...
                      return (
                        <div key={ai.id} style={{ padding: '6px 10px', background: '#1c1c1e', borderRadius: 8, marginBottom: 4, border: `1px solid ${overdue ? RED : BORDER}` }}>
            ...
            {m.attendees.map((att, idx) => (
              <div
                ...
                style={{
                  ...
                  background: att.present ? `${GREEN}15` : '#1c1c1e', border: `1px solid ${att.present ? GREEN : BORDER}`,
          ...
          <div style={cardStyle}>
            ...
            <div style={{ marginBottom: 16 }}>
              ...
              {(currentItem.decisions || []).map((d, dIdx) => (
                <div key={d.id} style={{ background: '#1c1c1e', borderRadius: 10, padding: 12, marginBottom: 8, border: `1px solid ${BORDER}` }}>
                  <input
            ...
            <div>
              ...
              {(currentItem.action_items || []).map((ai, aiIdx) => (
                <div key={ai.id} style={{ background: '#1c1c1e', borderRadius: 10, padding: 12, marginBottom: 8, border: `1px solid ${BORDER}` }}>
                  <input
...
export default function MeetingsPageWrapper() {
  return (
    <Suspense fallback={<div style={{ padding: 40, textAlign: 'center', color: '#CBD5E1' }}>Loading...</div>}>
      <MeetingsPage />
    </Suspense>
  );
}
