'use client';
/**
 * Saguaro Control Systems — Specification Sections Viewer/Manager
...
import { FileText, MagnifyingGlass, Warning, Clock, BookOpen, Printer, Star } from '@phosphor-icons/react';
...
function SpecsPageInner() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        ...
        if (!res.ok) { console.error('specs request failed', res.status); throw new Error('request failed'); }
        ...
        console.error(err); if (!cancelled) setError(humanError(err, 'Failed to load specs. Please try again.'));
      } finally {
  ...
  if (selectedSection) {
    ...
    return (
      <div style={{ background: '#0a0a0a', minHeight: '100vh', color: TEXT, fontFamily: '-apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif' }}>
        {/* Detail header */}
        <div style={{ background: RAISED, borderBottom: `1px solid ${BORDER}`, padding: '12px 16px', display: 'flex', alignItems: 'center', gap: 12, position: 'sticky', top: 0, zIndex: 10 }}>
          ...
          >
            {isBookmarked ? <Star size={22} weight="fill" /> : <Star size={22} weight="regular" />}
          </button>
          <button
            onClick={handlePrint}
        ...
        <div style={{ position: 'relative', marginBottom: 10 }}>
          <input
            ...
            style={{
              ...
              background: '#1c1c1e', border: `1px solid ${BORDER}`, color: TEXT,
              outline: 'none', boxSizing: 'border-box',
            }}
          />
        ...
        <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', alignItems: 'center' }}>
          <button
            ...
            style={{
              ...
              color: showBookmarksOnly ? '#FFFFFF' : DIM,
            ...
            return (
              <div key={divCode} style={{ marginBottom: 2 }}>
                ...
                >
                  ...
                  {count > 0 && (
                    <span style={{
                      background: GOLD, color: '#FFFFFF', fontSize: 11, fontWeight: 800,
                    ...
                    {divSections.map(sec => {
                      ...
                      return (
                        ...
                        >
                          ...
                          >
                            {isBookmarked ? <Star size={18} weight="fill" /> : <Star size={18} weight="regular" />}
                          </button>
