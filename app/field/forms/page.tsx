'use client';
/**
 * Saguaro Control Systems — Custom Forms List + Builder
  ...
  return (
    <div style={{ display: 'flex', gap: 4 }}>
      {[1, 2, 3, 4, 5].map((star) => (
        ...
        >
          {star <= value ? <Star size={22} weight="fill" /> : <Star size={22} weight="regular" />}
        </button>
      ))}
    </div>
  );
...
export default function FormsPage() {
  ...
  const saveTemplate = async () => {
    ...
    try {
      // tenant_id is derived from the session server-side — never sent from the client.
      ...
      if (!res.ok) {
        showToast(humanError(payload, "Couldn't save the template. Please try again."), 'error');
        return;
      }
      ...
      console.error(e);
      showToast(humanError(e, "Couldn't save the template. Please try again."), 'error');
    }
  };
  ...
  const submitForm = async () => {
    ...
    setSubmitting(true);
    try {
      // tenant_id + submitted_by are derived from the session server-side. Answers go
      // in `responses`; the API keys off `template_id`.
      ...
      const payload = await res.json().catch(() => ({}));
      if (!res.ok) {
        showToast(humanError(payload, "Couldn't submit the form. Please try again."), 'error');
        return;
      }
      ...
      console.error(e);
      showToast(humanError(e, "Couldn't submit the form. Please try again."), 'error');
  ...
  if (view === 'list') {
    return (
      <div style={{ minHeight: '100vh', background: BASE, padding: '0 16px 100px' }}>
        <FieldPageHeader
          title="Custom Forms"
