'use client';
/**
 * Saguaro Control Systems — Commissioning / System Startup
...
function TextInput({ value, onChange, placeholder, type }: { value: string; onChange: (v: string) => void; placeholder?: string; type?: string }) {
  return (
    <input
      ...
      style={{ width: '100%', padding: '10px 12px', borderRadius: 10, border: `1px solid ${BORDER}`, background: '#1c1c1e', color: TEXT, fontSize: 14, outline: 'none', boxSizing: 'border-box' }}
    />
  );
}
...
function CommissioningInner() {
  ...
  const renderList = () => (
    <div>
      ...
      {/* Progress Bar */}
      <div style={{ background: '#1c1c1e', borderRadius: 8, height: 8, marginBottom: 16, overflow: 'hidden' }}>
  ...
  const renderCreateForm = () => (
    <div>
      ...
      <textarea
        ...
        style={{ width: '100%', padding: '10px 12px', borderRadius: 10, border: `1px solid ${BORDER}`, background: '#1c1c1e', color: TEXT, fontSize: 14, outline: 'none', resize: 'vertical', boxSizing: 'border-box', fontFamily: 'inherit' }}
      />
            ...
            {PHASES.map((p, i) => (
              <React.Fragment key={p}>
                <div style={{
                  ...
                  background: i <= phaseIdx ? `${PHASE_COLORS[p]}22` : '#1c1c1e',
  ...
  /* ── Main render ── */
  return (
    <div style={{ minHeight: '100vh', background: '#0a0a0a', color: TEXT, fontFamily: '-apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif' }}>
...
function ChecklistPanel({ items, onAdd, onToggle, onNote, onRemove }: {
  ...
  return (
    <div>
      ...
      {/* Progress bar */}
      {total > 0 && (
        <div style={{ background: '#1c1c1e', borderRadius: 6, height: 6, marginBottom: 12, overflow: 'hidden', display: 'flex' }}>
      ...
      {items.map((item) => (
        <div key={item.id} style={{ background: RAISED, border: `1px solid ${item.failed ? RED : item.checked ? GREEN : BORDER}`, borderRadius: 10, padding: 12, marginBottom: 8 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            ...
            >
              {item.checked ? <Check size={14} weight="bold" /> : null}
            </button>
            {/* Fail button */}
            <button
            ...
            >
              {item.failed ? <X size={14} weight="bold" /> : null}
            </button>
            <span style={{
            ...
            <button onClick={() => onRemove(item.id)} style={{ background: 'none', border: 'none', color: RED, cursor: 'pointer', fontSize: 14, padding: 0 }}><X size={13} weight="bold" /></button>
          </div>
          {expandedId === item.id && (
            <div style={{ marginTop: 8 }}>
              <textarea
                ...
                style={{ width: '100%', padding: '8px 10px', borderRadius: 8, border: `1px solid ${BORDER}`, background: '#1c1c1e', color: TEXT, fontSize: 13, outline: 'none', resize: 'vertical', boxSizing: 'border-box', fontFamily: 'inherit' }}
              />
            </div>
          )}
        </div>
        ...
        <div key={tr.id} style={{ background: RAISED, border: `1px solid ${tr.pass === true ? GREEN : tr.pass === false ? RED : BORDER}`, borderRadius: 10, padding: 12, marginBottom: 8 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
            ...
            <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
              ...
              <button onClick={() => onRemove(tr.id)} style={{ background: 'none', border: 'none', color: RED, cursor: 'pointer', fontSize: 13, padding: 0 }}><X size={13} weight="bold" /></button>
            </div>
          </div>
...
export default function CommissioningPage() {
  return (
    <Suspense fallback={<div style={{ minHeight: '100vh', background: '#0a0a0a', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#CBD5E1', fontFamily: 'sans-serif' }}>Loading Commissioning...</div>}>
      <CommissioningInner />
    </Suspense>
  );
}
