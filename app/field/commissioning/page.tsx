...
function TextInput({ value, onChange, placeholder, type }: { value: string; onChange: (v: string) => void; placeholder?: string; type?: string }) {
  return (
    <input
      ...
      style={{ width: '100%', padding: '10px 12px', borderRadius: 10, border: `1px solid ${BORDER}`, background: '#16243A', color: TEXT, fontSize: 14, outline: 'none', boxSizing: 'border-box' }}
    />
  );
}
...
function CommissioningInner() {
  ...
  const renderList = () => (
    <div>
      ...
      {/* Progress Bar */}
      <div style={{ background: '#16243A', borderRadius: 8, height: 8, marginBottom: 16, overflow: 'hidden' }}>
  ...
  const renderCreateForm = () => (
    <div>
      ...
      <textarea
        ...
        style={{ width: '100%', padding: '10px 12px', borderRadius: 10, border: `1px solid ${BORDER}`, background: '#16243A', color: TEXT, fontSize: 14, outline: 'none', resize: 'vertical', boxSizing: 'border-box', fontFamily: 'inherit' }}
      />
            ...
            {PHASES.map((p, i) => (
              <React.Fragment key={p}>
                <div style={{
                  ...
                  background: i <= phaseIdx ? `${PHASE_COLORS[p]}22` : '#16243A',
...
function ChecklistPanel({ items, onAdd, onToggle, onNote, onRemove }: {
  ...
  return (
    <div>
      ...
      {/* Progress bar */}
      {total > 0 && (
        <div style={{ background: '#16243A', borderRadius: 6, height: 6, marginBottom: 12, overflow: 'hidden', display: 'flex' }}>
          ...
          {expandedId === item.id && (
            <div style={{ marginTop: 8 }}>
              <textarea
                ...
                style={{ width: '100%', padding: '8px 10px', borderRadius: 8, border: `1px solid ${BORDER}`, background: '#16243A', color: TEXT, fontSize: 13, outline: 'none', resize: 'vertical', boxSizing: 'border-box', fontFamily: 'inherit' }}
              />
            </div>
          )}
        </div>
...
export default function CommissioningPage() {
  return (
    <Suspense fallback={<div style={{ minHeight: '100vh', background: '#0d1117', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#CBD5E1', fontFamily: 'sans-serif' }}>Loading Commissioning...</div>}>
      <CommissioningInner />
    </Suspense>
  );
}
