'use client';
/**
 * Saguaro Control Systems — Punch List
...
import { CaretRight, Circle, XCircle, X } from '@phosphor-icons/react';
...
function ConfirmDialog({ message, onConfirm, onCancel }: { message: string; onConfirm: () => void; onCancel: () => void }) {
  return (
    <div style={{ position: 'fixed', inset: 0, zIndex: 9999, display: 'flex', alignItems: 'center', justifyContent: 'center', background: '#0a0a0a' }}>
      <div style={{ background: '#141416', border: `1px solid ${BORDER}`, borderRadius: 16, padding: '24px', maxWidth: 340, width: '90%' }}>
...
function AssigneePickerDialog({ assignees, onSelect, onCancel }: { assignees: string[]; onSelect: (a: string) => void; onCancel: () => void }) {
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, zIndex: 9999, display: 'flex', alignItems: 'center', justifyContent: 'center', background: '#0a0a0a' }}>
      <div style={{ background: '#141416', border: `1px solid ${BORDER}`, borderRadius: 16, padding: '24px', maxWidth: 380, width: '90%', maxHeight: '70vh', overflow: 'auto' }}>
...
function PriorityPickerDialog({ onSelect, onCancel }: { onSelect: (p: string) => void; onCancel: () => void }) {
  return (
    <div style={{ position: 'fixed', inset: 0, zIndex: 9999, display: 'flex', alignItems: 'center', justifyContent: 'center', background: '#0a0a0a' }}>
      <div style={{ background: '#141416', border: `1px solid ${BORDER}`, borderRadius: 16, padding: '24px', maxWidth: 300, width: '90%' }}>
  ...
  return (
    <div style={{ marginBottom: 14 }}>
      <button onClick={() => setOpen(!open)} style={{ width: '100%', background: RAISED, border: `1px solid ${BORDER}`, borderRadius: 12, padding: '12px 14px', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        ...
        <span style={{ display: 'inline-flex', color: DIM, transform: open ? 'rotate(90deg)' : 'none', transition: 'transform .2s' }}><CaretRight size={18} weight="regular" color={DIM} /></span>
      </button>
      {open && (
...
function AdvancedFilterPanel({
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, zIndex: 9998, display: 'flex', flexDirection: 'column', background: '#0a0a0a' }}>
      ...
      <div style={{ flex: 1, overflow: 'auto', padding: '16px' }}>
            ...
            <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
              {savedPresets.map((p) => (
                <div key={p.name} style={{ display: 'flex', alignItems: 'center', gap: 0 }}>
                  ...
                  <button onClick={() => onDeletePreset(p.name)} style={{ background: 'transparent', border: `1px solid ${BORDER}`, borderRadius: '0 20px 20px 0', padding: '5px 8px', color: RED, fontSize: 11, fontWeight: 700, cursor: 'pointer', display: 'inline-flex', alignItems: 'center' }}><XCircle size={14} weight="regular" color={RED} /></button>
                </div>
              ))}
            </div>
          </div>
        ...
        <select value={filterAssignee} onChange={(e) => setFilterAssignee(e.target.value)} style={{ ...inp, marginBottom: 16 }}>
          <option value="" style={{ background: '#141416' }}>All Assignees</option>
          {assignees.map((a) => <option key={a} value={a} style={{ background: '#141416' }}>{a}</option>)}
        </select>
        ...
        <select value={filterDateField} onChange={(e) => setFilterDateField(e.target.value)} style={{ ...inp, marginBottom: 8 }}>
          <option value="" style={{ background: '#141416' }}>No Date Filter</option>
          <option value="created_at" style={{ background: '#141416' }}>Created Date</option>
          <option value="due_date" style={{ background: '#141416' }}>Due Date</option>
        </select>
        {filterDateField && (
    ...
    <div style={{ padding: '18px 16px', paddingBottom: selectMode && selectedIds.size > 0 ? 100 : 18 }}>
              ...
              >
                ...
                >
                  <div style={{ display: 'flex', alignItems: 'flex-start', gap: 10 }}>
                    ...
                    {!selectMode && <span style={{ display: 'inline-flex', color: DIM, flexShrink: 0 }}><CaretRight size={18} weight="regular" color={DIM} /></span>}
                  </div>
                </div>
              </div>
            ))}
      ...
      {view === 'list' && selectMode && selectedIds.size > 0 && (
        <div style={{
          ...
          background: '#141416', borderTop: `1px solid ${BORDER}`,
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
              ...
              <Fld label="Priority">
                <select value={priority} onChange={(e) => setPriority(e.target.value)} style={inp}>
                  {PRIORITIES.map((p) => <option key={p} value={p} style={{ background: '#141416' }}>{p}</option>)}
                </select>
              </Fld>
            </div>
            <Fld label="Assignee">
              ...
              <div style={{ display: 'flex', gap: 8, overflowX: 'auto', paddingBottom: 4 }}>
                {photoPreviews.map((src, i) => (
                  <div key={i} style={{ position: 'relative', flexShrink: 0 }}>
                    ...
                    <button type="button" onClick={() => removePhoto(i)} style={{ position: 'absolute', top: -6, right: -6, width: 20, height: 20, borderRadius: '50%', background: RED, border: 'none', color: '#fff', fontSize: 12, fontWeight: 900, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><X size={12} weight="bold" color="#fff" /></button>
                  </div>
                ))}
              </div>
            )}
        ...
        <div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            {STATUSES.map((s) => (
              ...
              >
                {selected.status === s
                  ? <Circle size={14} weight="fill" color={STATUS_COLORS[s]} style={{ verticalAlign: 'middle', marginRight: 8 }} />
                  : <Circle size={14} weight="regular" color={DIM} style={{ verticalAlign: 'middle', marginRight: 8 }} />}{STATUS_LABELS[s]}
              </button>
            ))}
          </div>
        </div>
