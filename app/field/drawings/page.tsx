'use client';
/**
 * Saguaro Control Systems — Drawing Viewer with Pindrops + Markup Persistence
...
import { PencilSimple, PushPin, Ruler, Eraser, Eye, EyeSlash } from '@phosphor-icons/react';

const GOLD = '#F59E0B', RAISED = '#141416', BORDER = 'rgba(255,255,255,0.12)', TEXT = '#FFFFFF', DIM = '#CBD5E1';
const GREEN = '#22C55E', RED = '#EF4444', AMBER = '#F59E0B', BLUE = '#F59E0B';

const PRESET_COLORS = [RED, BLUE, GREEN, AMBER, GOLD, '#8B5CF6', '#EC4899', '#06B6D4'];
const USER_COLORS = ['#EF4444', '#F59E0B', '#22C55E', '#F59E0B', '#8B5CF6', '#EC4899', '#06B6D4', '#F59E0B'];
...
function DrawingsPage() {
  ...
  const toolBtn = (tool: MarkupTool | null, label: string, icon: React.ReactNode): React.ReactNode => (
    <button
      onClick={() => {
  ...
  if (view === 'viewer' && selectedDrawing) {
    return (
      <div style={{ padding: '18px 16px', paddingBottom: 32 }}>
        <FieldPageHeader
          title={selectedDrawing.sheet}
          subtitle={selectedDrawing.name}
          icon={scopedFieldIcon('drawings', 'ph')}
          onBack={() => { setView('list'); setSelectedDrawing(null); setPins([]); setPendingPin(null); setShowPinForm(false); setSelectedPin(null); setActiveTool(null); setSavedMarkups([]); setSelectedMarkup(null); }}
          actions={
        ...
        <div style={{ background: RAISED, border: `1px solid ${BORDER}`, borderRadius: 12, padding: '10px', marginBottom: 10 }}>
          <div style={{ display: 'flex', gap: 4, flexWrap: 'wrap', marginBottom: 8 }}>
            {toolBtn('pen', 'Pen', <PencilSimple size={16} weight="fill" />)}
            {toolBtn('pin', 'Pin', <PushPin size={16} weight="fill" />)}
            {toolBtn('text', 'Text', 'T')}
            {toolBtn('rect', 'Rect', '\u25A1')}
            {toolBtn('circle', 'Circle', '\u25CB')}
            {toolBtn('arrow', 'Arrow', '\u2197')}
            {toolBtn('measure', 'Measure', <Ruler size={16} weight="fill" />)}
            {toolBtn('eraser', 'Eraser', <Eraser size={16} weight="fill" />)}
          </div>
          ...
          <div style={{ background: RAISED, border: `1px solid ${BORDER}`, borderRadius: 12, padding: '14px', marginBottom: 10 }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 10 }}>
              ...
              {uniqueCreators.length > 1 && (
                <select value={filterUser || ''} onChange={e => setFilterUser(e.target.value || null)} style={{ background: '#1c1c1e', border: `1px solid ${BORDER}`, borderRadius: 6, padding: '4px 8px', color: TEXT, fontSize: 12 }}>
                  ...
                  <div key={markup.id} style={{
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                      ...
                      <button onClick={() => toggleLayerVisibility(markup.id)} style={{ background: 'none', border: 'none', cursor: 'pointer', padding: 0, color: visibleLayers.has(markup.id) ? TEXT : DIM, fontSize: 16 }}>
                        {visibleLayers.has(markup.id) ? <Eye size={16} weight="fill" /> : <EyeSlash size={16} weight="regular" />}
                      </button>
                      {/* Color dot */}
