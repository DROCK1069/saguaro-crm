'use client';
/**
 * Saguaro Control Systems — Budget View
...
import { DotsThree, PencilSimple, ChartBar, Copy, Trash, CheckCircle } from '@phosphor-icons/react';
...
function BudgetPage() {
    ...
    return (
      <div style={{ padding: '18px 16px', maxWidth: 800, margin: '0 auto' }}>
        ...
        <div style={card}>
          ...
          <div style={{ height: 10, background: '#1c1c1e', borderRadius: 6, overflow: 'hidden' }}>
  ...
  return (
    <div style={{ padding: '0 16px 18px', maxWidth: 900, margin: '0 auto' }}>
      <FieldPageHeader title="Project Budget" icon={scopedFieldIcon('dashboard', 'ph')} />
                ...
                {filteredCodes.slice(0, 15).map((cc) => {
                  ...
                  return (
                    <div key={cc.id}>
                      ...
                      <div style={{ position: 'relative', height: 20, background: '#1c1c1e', borderRadius: 6, overflow: 'hidden' }}>
                      ...
                      <tr key={cc.id} style={{ cursor: 'pointer' }} onClick={() => { setSelectedCostCode(cc); setView('drilldown'); }}>
                        ...
                        <td style={{ ...tdStyle, textAlign: 'right', fontVariantNumeric: 'tabular-nums' }}>
                          <span style={{ display: 'inline-flex', alignItems: 'center', gap: 6 }}>
                            {formatUSD(cc.budget)}
                            <button onClick={(e) => { e.stopPropagation(); openActionSheet(cc.id, `${cc.code} — ${cc.description}`, cc.budget); }} style={{ background: 'none', border: 'none', color: DIM, cursor: 'pointer', padding: '2px 4px', fontSize: 18, lineHeight: 1, borderRadius: 6, display: 'inline-flex', alignItems: 'center' }} title="Actions"><DotsThree size={18} weight="bold" color={DIM} /></button>
                          </span>
                        </td>
                        ...
                        <td style={{ ...tdStyle, textAlign: 'center' }}>
                          <div style={{ width: 60, margin: '0 auto' }}>
                            <div style={{ height: 6, background: '#1c1c1e', borderRadius: 4, overflow: 'hidden' }}>
        ...
        <div style={{ padding: '8px 20px 24px' }}>
          ...
          {sheetMode === 'menu' && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
              {[
                { label: 'Edit Amount', icon: PencilSimple, action: () => setSheetMode('edit') },
                { label: 'Adjust %', icon: ChartBar, action: () => setSheetMode('adjust') },
                { label: 'Copy Amount', icon: Copy, action: () => actionSheet && handleFieldBudgetCopy(actionSheet.amount) },
              ].map(item => (
                <button key={item.label} onClick={item.action} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '14px 16px', background: 'transparent', border: `1px solid ${BORDER}`, borderRadius: 10, color: TEXT, fontSize: 15, fontWeight: 600, cursor: 'pointer', textAlign: 'left' }}>
                  <span style={{ display: 'inline-flex' }}><item.icon size={20} weight="regular" color={TEXT} /></span>{item.label}
                </button>
              ))}
            </div>
          )}
          {sheetMode === 'edit' && (
            <div>
              ...
              <input value={editVal} onChange={e => setEditVal(e.target.value)} type="number" inputMode="decimal" autoFocus style={{ width: '100%', padding: '14px 16px', background: '#1c1c1e', border: `1px solid ${GOLD}`, borderRadius: 10, color: TEXT, fontSize: 20, fontWeight: 700, outline: 'none', boxSizing: 'border-box', textAlign: 'right' }} />
      ...
      {deleteConfirm && (
        <div style={{ position: 'fixed', inset: 0, background: '#0a0a0a', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 700, padding: 24 }}>
          <div style={{ background: RAISED, border: `1px solid ${BORDER}`, borderRadius: 16, padding: 24, width: '100%', maxWidth: 340, textAlign: 'center' }}>
            <div style={{ marginBottom: 12, display: 'flex', justifyContent: 'center' }}><Trash size={36} weight="fill" color={RED} /></div>
...
export default function BudgetPageWrapper() {
  return (
    <Suspense fallback={<div style={{ padding: 40, textAlign: 'center', color: '#CBD5E1' }}>Loading budget...</div>}>
      <BudgetPage />
    </Suspense>
  );
}
