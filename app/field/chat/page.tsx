'use client';
/**
 * Saguaro Control Systems — Job Board / Crew Chat
...
function ChatPage() {
      ...
      <div style={{ padding: '18px 16px', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', minHeight: '60vh' }}>
        <div style={{ background: RAISED, border: `1px solid ${BORDER}`, borderRadius: 16, padding: 24, width: '100%', maxWidth: 360 }}>
          ...
          <input
            ...
            style={{ width: '100%', background: '#1c1c1e', border: '1px solid rgba(255,255,255,0.12)', borderRadius: 10, padding: '12px 14px', color: '#FFFFFF', fontSize: 16, outline: 'none', boxSizing: 'border-box', marginBottom: 12 }}
            autoFocus
          />
          <button
            onClick={saveName}
            disabled={!nameInput.trim()}
            style={{ width: '100%', background: nameInput.trim() ? GOLD : '#1c1c1e', border: 'none', borderRadius: 10, padding: '14px', color: nameInput.trim() ? '#000' : DIM, fontSize: 16, fontWeight: 800, cursor: nameInput.trim() ? 'pointer' : 'default' }}
          >
            Join Job Board
          </button>
        </div>
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100dvh', overflow: 'hidden' }}>
      ...
      {/* Input bar */}
      <div style={{ flexShrink: 0, padding: '10px 12px', borderTop: `1px solid ${BORDER}`, background: '#141416' }}>
        {!online && (
        ...
        <form onSubmit={sendMessage} style={{ display: 'flex', gap: 8, alignItems: 'flex-end' }}>
          ...
          <button
            ...
            style={{
              background: content.trim() && !sending ? GOLD : '#1c1c1e',
              border: 'none',
              borderRadius: 10,
              width: 44,
              height: 44,
