'use client';
/**
 * Saguaro Control Systems — Inspection
...
import { XCircle, X, Warning, Check } from '@phosphor-icons/react';
...
const RESULTS = [
  { value: 'passed',           label: 'Passed',           color: GREEN,  bg: 'rgba(34,197,94,.15)' },
  { value: 'conditional_pass', label: 'Conditional',      color: AMBER,  bg: 'rgba(245,158,11,.15)' },
  { value: 'failed',           label: 'Failed',           color: RED,    bg: 'rgba(239,68,68,.15)' },
  { value: 'pending',          label: 'Pending',          color: DIM,    bg: 'rgba(143,163,192,.1)' },
];
...
function PunchItemModal({
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, zIndex: 9999, display: 'flex', alignItems: 'center', justifyContent: 'center', background: '#0a0a0a', padding: 20 }}>
      <div style={{ background: '#141416', border: `1px solid ${BORDER}`, borderRadius: 16, padding: 24, maxWidth: 420, width: '100%', maxHeight: '80vh', overflowY: 'auto' }}>
        {created ? (
            ...
            <div style={{ marginBottom: 14 }}>
              {failedItems.map((item, i) => (
                <div key={i} style={{ background: 'rgba(239,68,68,.06)', border: `1px solid rgba(239,68,68,.15)`, borderRadius: 8, padding: '8px 10px', marginBottom: 6, fontSize: 13, color: TEXT }}>
                  <span style={{ color: RED, marginRight: 6, fontWeight: 700, display: 'inline-flex', verticalAlign: 'middle' }}><XCircle size={14} weight="fill" color={RED} /></span>
                  {item.item}
  ...
  return (
    <div style={{ ...card, borderColor: `rgba(245, 158, 11,.25)` }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 10 }}>
        ...
        <button type="button" onClick={() => setExpanded(false)} style={{ background: 'transparent', border: 'none', color: DIM, fontSize: 18, cursor: 'pointer', padding: 0, lineHeight: 1 }}><span style={{ display: 'inline-flex', verticalAlign: 'middle' }}><X size={18} weight="regular" color={DIM} /></span></button>
      </div>
      {/* Category tabs */}
...
function ReInspectionBanner({ originalInspType, failedItems }: { originalInspType: string; failedItems: CheckItem[] }) {
  return (
    <div style={{ background: 'rgba(245,158,11,.08)', border: `1px solid rgba(245,158,11,.25)`, borderRadius: 10, padding: '10px 14px', marginBottom: 12, fontSize: 13 }}>
...
function InspectionForm() {
  ...
  return (
    <div style={{ padding: '18px 16px' }}>
      ...
      <FieldPageHeader
        title={`Inspection${isReInspection ? ' (Re-inspection)' : ''}`}
      ...
      <form onSubmit={handleSubmit}>
        ...
        <div style={card}>
          ...
          <select value={inspType} onChange={(e) => changeType(e.target.value)} style={{ ...inp, marginBottom: 10 }}>
            {INSPECTION_TYPES.map((t) => <option key={t} value={t} style={{ background: '#141416' }}>{t}</option>)}
          </select>
        ...
        <div style={card}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 10 }}>
            ...
            <div style={{ display: 'flex', gap: 10, fontSize: 12 }}>
              ...
              {deficiencies > 0 && <span style={{ color: RED, fontWeight: 700 }}><span style={{ display: 'inline-flex', verticalAlign: 'middle' }}><Warning size={13} weight="fill" color={RED} /></span> {deficiencies} defect{deficiencies > 1 ? 's' : ''}</span>}
            </div>
          </div>
          {/* Progress bar */}
          <div style={{ height: 5, background: '#1c1c1e', borderRadius: 3, marginBottom: 12 }}>
            ...
            >
              <div style={{ display: 'flex', alignItems: 'flex-start', gap: 10, cursor: 'pointer' }} onClick={() => toggle(idx)}>
                <div style={{ width: 24, height: 24, borderRadius: 7, border: `2px solid ${item.checked ? GREEN : BORDER}`, background: item.checked ? GREEN : 'transparent', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, marginTop: 1, transition: 'all .12s' }}>
                  {item.checked && <span style={{ color: '#000', fontSize: 14, fontWeight: 900, lineHeight: 1, display: 'inline-flex' }}><Check size={16} weight="bold" color="#000" /></span>}
                </div>
                ...
                <button
                  ...
                  style={{ background: item.photoPreview ? 'rgba(245,158,11,.2)' : 'transparent', border: `1px solid ${item.photoPreview ? BLUE : BORDER}`, borderRadius: 6, padding: '2px 7px', fontSize: 11, color: item.photoPreview ? BLUE : DIM, cursor: 'pointer', flexShrink: 0 }}
                  title="Attach photo"
                >
                ...
                >
                  <span style={{ display: 'inline-flex', verticalAlign: 'middle' }}><Warning size={13} weight="fill" color={item.deficiency ? RED : DIM} /></span>
                </button>
              </div>
              {item.photoPreview && (
        ...
        <button
          type="submit"
          disabled={saving}
          style={{ width: '100%', background: saving ? '#1c1c1e' : res.color, border: 'none', borderRadius: 14, padding: '18px', color: saving ? DIM : (result === 'pending' ? TEXT : '#000'), fontSize: 17, fontWeight: 800, cursor: saving ? 'wait' : 'pointer', letterSpacing: 0.3 }}
        >
