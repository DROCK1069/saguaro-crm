'use client';
/**
 * Saguaro Control Systems — Purchase Orders
...
function PurchaseOrdersInner() {
  ...
  return (
    <div style={{
      ...
      <FieldPageHeader
        title={
          view === 'create' ? 'New Purchase Order'
          : view === 'detail' && selected ? (
              <span style={{ display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap' }}>
                PO #{selected.po_number}<Badge status={selected.status} />
              </span>
            )
          : view === 'receive' && selected ? `Receive Items — PO #${selected.po_number}`
          : 'Purchase Orders'
        }
        onBack={() => {
          if (view === 'receive') { setView('detail'); }
      ...
      <div style={{ padding: '4px 16px 0', maxWidth: 960, margin: '0 auto', boxSizing: 'border-box' }}>
            ...
            {sheetMode === 'menu' && (
              <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                {[
                  ...
                  { label: 'Copy Amount', icon: <Copy size={20} />, action: () => handleCopyPO(actionSheet.amount, actionSheet.poId) },
                ].map(item => (
                ...
                <button onClick={() => { closeActionSheet(); setDeleteConfirm(actionSheet.poId); }} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '12px 14px', background: 'rgba(239,68,68,.08)', border: `1px solid rgba(239,68,68,.2)`, borderRadius: 10, color: RED, fontSize: 14, cursor: 'pointer', width: '100%', textAlign: 'left' }}>
                  <span style={{ display: 'inline-flex' }}><Trash size={20} /></span>Void PO
                </button>
              </div>
            )}
            {sheetMode === 'edit' && (
