...
function MorePage(){
  ...
  const PAGE_LINKS=[
    ...
    {href:`/field/schedule${projectId?`?projectId=${projectId}`:''}`,icon:'calendar',label:'Schedule',desc:"Today's activities and deadlines",color:BLUE,bg:'rgba(245,158,11,.1)',border:'rgba(245,158,11,.25)'},
    ...
    {href:`/field/drawings${projectId?`?projectId=${projectId}`:''}`,icon:'plan',label:'Drawings',desc:'View plans, drop pins on issues',color:BLUE,bg:'rgba(245,158,11,.1)',border:'rgba(245,158,11,.25)'},
    ...
    {href:`/field/submittals${projectId?`?projectId=${projectId}`:''}`,icon:'document',label:'Submittals',desc:'Review and approve submittals',color:BLUE,bg:'rgba(245,158,11,.1)',border:'rgba(245,158,11,.25)'},
    ...
    {href:`/field/meetings${projectId?`?projectId=${projectId}`:''}`,icon:'users',label:'Meetings',desc:'Minutes, attendees, action items',color:BLUE,bg:'rgba(245,158,11,.1)',border:'rgba(245,158,11,.25)'},
    ...
    {href:`/field/search${projectId?`?projectId=${projectId}`:''}`,icon:'qr',label:'Global Search',desc:'Search across all modules',color:BLUE,bg:'rgba(245,158,11,.1)',border:'rgba(245,158,11,.25)'},
    ...
    {href:`/field/drone${projectId?`?projectId=${projectId}`:''}`,icon:'sparkle',label:'Drone Analysis',desc:'Upload drone photos, AI progress detection',color:BLUE,bg:'rgba(245,158,11,.1)',border:'rgba(245,158,11,.25)'},
    ...
    {href:`/field/floor-plan${projectId?`?projectId=${projectId}`:''}`,icon:'plan',label:'Floor Plan Pins',desc:'Tap drawings to pin locations, rooms & heatmap',color:BLUE,bg:'rgba(245,158,11,.1)',border:'rgba(245,158,11,.25)'},
    ...
    {href:`/field/purchase-orders${projectId?`?projectId=${projectId}`:''}`,icon:'box',label:'Purchase Orders',desc:'PO creation, line items & receiving',color:BLUE,bg:'rgba(245,158,11,.1)',border:'rgba(245,158,11,.25)'},
    ...
    {href:`/field/closeout${projectId?`?projectId=${projectId}`:''}`,icon:'folder',label:'Closeout',desc:'Warranties, O&M, as-builts & attic stock',color:BLUE,bg:'rgba(245,158,11,.1)',border:'rgba(245,158,11,.25)'},
  ...
  return(
    <div style={{padding:'18px 16px'}}>
      <FieldPageHeader
        title="More Tools"
        subtitle="All field tools in one place"
        icon={scopedFieldIcon('allTools', 'ph')}
      />
      ...
      {panel===null&&(
        <>
          ...
          {[
            { title: 'Daily Operations', Icon: ClipboardText, items: PAGE_LINKS.filter(l => ['/field/clock','/field/schedule','/field/equipment','/field/deliveries','/field/meetings'].some(p => l.href.startsWith(p))) },
      ...
      {panel==='safety'&&(
        <form onSubmit={submitSafety}>
          ...
          <S label="Incident Details">
            ...
            <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:10}}>
              <F label="Severity"><select value={safetySeverity} onChange={e=>setSafetySeverity(e.target.value)} style={inp}>{SEVERITY.map(s=><option key={s} value={s} style={{background:'#141416'}}>{s}</option>)}</select></F>
              <F label="Injury Type"><select value={safetyInjury} onChange={e=>setSafetyInjury(e.target.value)} style={inp}>{INJURY_TYPES.map(s=><option key={s} value={s} style={{background:'#141416'}}>{s}</option>)}</select></F>
            </div>
...
function Sub({saving,label,online,color,dark}:{saving:boolean;label:string;online:boolean;color:string;dark?:boolean}){
  return <button type="submit" disabled={saving} style={{width:'100%',background:saving?'#1c1c1e':color,border:'none',borderRadius:14,padding:'18px',color:saving?DIM:(dark?'#000':'#fff'),fontSize:17,fontWeight:800,cursor:saving?'wait':'pointer',marginTop:4}}>{saving?'Saving...':(online?`Save ${label}`:`${label} (Offline — will sync)`)}</button>;
}
