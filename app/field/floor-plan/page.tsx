'use client';
/**
 * Saguaro Control Systems -- Enhanced Floor Plan Viewer
...
import { MapPin, Camera, Warning, WarningCircle, Question, NotePencil, Ruler, MapTrifold, PushPin, Square, PencilSimple, ChartBar, Broadcast, Scribble, LineSegment, ArrowUpRight, Highlighter } from '@phosphor-icons/react';
...
const PIN_TYPES: Record<string, { color: string; icon: React.ElementType; label: string }> = {
  location: { color: BLUE, icon: MapPin, label: 'Location' },
  photo:    { color: GREEN, icon: Camera, label: 'Photo' },
  punch:    { color: RED, icon: Warning, label: 'Punch' },
  rfi:      { color: '#8B5CF6', icon: Question, label: 'RFI' },
  note:     { color: GOLD, icon: NotePencil, label: 'Note' },
  measurement: { color: '#06B6D4', icon: Ruler, label: 'Measurement' },
  issue:    { color: '#F97316', icon: WarningCircle, label: 'Issue' },
};
...
const HEAT_COLORS: { max: number; color: string; label: string }[] = [
  ...
  { max: 99, color: '#F59E0B', label: '76-99%' },
...
function FloorPlanPage() {
  ...
  const TBtn = ({ active, onClick, children, color }: { active?: boolean; onClick: () => void; children: React.ReactNode; color?: string }) => (
    <button onClick={onClick} style={{
      ...
      background: active ? (color || GOLD) : 'rgba(20,20,22,0.7)',
      ...
      {view === 'list' && (
        loading ? (
          ...
          <div style={{ ...glass, padding: '40px 24px', textAlign: 'center' }}>
            <div style={{ marginBottom: 12, display: 'flex', justifyContent: 'center' }}><MapTrifold size={48} weight="regular" color={DIM} /></div>
            ...
            {drawings.map(d => (
              <button key={d.id} onClick={() => openDrawing(d)} style={{ ...glass, padding: 14, display: 'flex', alignItems: 'center', gap: 14, cursor: 'pointer', width: '100%', textAlign: 'left', color: TEXT }}>
                {d.thumbnail_url ? (
                  ...
                  <div style={{ width: 56, height: 56, borderRadius: 10, background: 'rgba(245,158,11,0.1)', border: '1px solid rgba(245,158,11,0.2)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}><Ruler size={24} weight="regular" color={BLUE} /></div>
                )}
        ...
        <div>
          ...
          <div style={{ display: 'flex', gap: 6, marginBottom: 8, flexWrap: 'wrap' }}>
            <TBtn active={mode === 'pin'} onClick={() => { setMode(mode === 'pin' ? 'view' : 'pin'); setMarkupMode(false); setDrawingRoom(false); }}>
              <PushPin size={14} weight="fill" style={{ verticalAlign: 'middle' }} /> {mode === 'pin' ? 'Cancel' : 'Place Pin'}
            </TBtn>
            <TBtn active={drawingRoom} onClick={() => { if (drawingRoom) { setDrawingRoom(false); setRoomPoints([]); setMode('view'); } else { setMode('room'); setDrawingRoom(true); setRoomPoints([]); setMarkupMode(false); } }} color={BLUE}>
              <Square size={14} weight="fill" style={{ verticalAlign: 'middle' }} /> {drawingRoom ? 'Cancel Room' : 'Draw Room'}
            </TBtn>
            <TBtn active={markupMode} onClick={() => { setMarkupMode(!markupMode); if (!markupMode) setMode('markup'); else setMode('view'); setDrawingRoom(false); }} color="#F97316">
              <PencilSimple size={14} weight="fill" style={{ verticalAlign: 'middle' }} /> Markup
            </TBtn>
            <TBtn active={showProgress} onClick={() => setShowProgress(!showProgress)} color={GREEN}>
              <ChartBar size={14} weight="fill" style={{ verticalAlign: 'middle' }} /> Progress
            </TBtn>
            <TBtn onClick={getGPS} color={GREEN}>
              {gpsLoading ? '...' : <Broadcast size={14} weight="fill" style={{ verticalAlign: 'middle' }} />} GPS
            </TBtn>
          </div>
            ...
            <div style={{ ...glass, padding: '8px 12px', marginBottom: 8, display: 'flex', gap: 6, alignItems: 'center', flexWrap: 'wrap' }}>
              {(['freehand', 'line', 'arrow', 'text', 'highlight'] as MarkupTool[]).map(t => (
                <button key={t} onClick={() => setMarkupTool(t)} style={{
                  ...
                  {t === 'freehand' ? <Scribble size={13} weight="bold" style={{ verticalAlign: 'middle' }} /> : t === 'line' ? <LineSegment size={13} weight="bold" style={{ verticalAlign: 'middle' }} /> : t === 'arrow' ? <ArrowUpRight size={13} weight="bold" style={{ verticalAlign: 'middle' }} /> : t === 'text' ? 'Aa' : <Highlighter size={13} weight="bold" style={{ verticalAlign: 'middle' }} />} {t}
                </button>
              ))}
          ...
          {gpsPosition && (
            <div style={{ ...glass, padding: '8px 14px', marginBottom: 8, fontSize: 13 }}>
              <span style={{ fontWeight: 700, color: GREEN }}><MapPin size={13} weight="fill" style={{ verticalAlign: 'middle' }} /> Located</span>
                ...
                return (
                  <div key={pin.id} style={{ position: 'absolute', left: `${pin.x_pct}%`, top: `${pin.y_pct}%`, transform: 'translate(-50%, -100%)', zIndex: 10 }} title={`${pin.label} (${pType.label})`}>
                    ...
                    <div style={{
                      ...
                      <span style={{ transform: 'rotate(45deg)', display: 'inline-flex' }}>{React.createElement(pType.icon, { size: 14, weight: 'fill', color: '#fff' })}</span>
                    </div>
                    <div style={{
            ...
            {pendingPin && (
              <div style={{ position: 'absolute', left: `${pendingPin.x_pct}%`, top: `${pendingPin.y_pct}%`, transform: 'translate(-50%, -100%)', zIndex: 20 }}>
                <div style={{
                  ...
                  <span style={{ transform: 'rotate(45deg)', display: 'inline-flex' }}><PushPin size={16} weight="fill" color="#fff" /></span>
                </div>
              </div>
            )}
          </div>
                ...
                <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                  {Object.entries(PIN_TYPES).map(([key, cfg]) => (
                    <button key={key} type="button" onClick={() => setPinType(key)} style={{
                      ...
                      {React.createElement(cfg.icon, { size: 13, weight: 'fill', style: { verticalAlign: 'middle' } })} {cfg.label}
                    </button>
                  ))}
                </div>
              </div>
