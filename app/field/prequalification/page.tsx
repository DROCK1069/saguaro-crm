'use client';
/**
 * Saguaro Control Systems — Subcontractor Prequalification
...
import { FileText, Warning, Phone, ChatCircle, Envelope, Scroll, MagnifyingGlass, NotePencil, Star, CheckCircle, XCircle } from '@phosphor-icons/react';
...
function PrequalInner() {
  ...
  const fetchSubs = useCallback(async () => {
    ...
    try {
      ...
      if (!res.ok) { console.error('field prequal load failed', res.status); throw new Error('request failed'); }
      const data = await res.json();
      setSubs(Array.isArray(data) ? data : data.subs ?? []);
    } catch (e: unknown) {
      console.error(e); setError(humanError(e, 'Failed to load subcontractors. Please try again.'));
    } finally {
      setLoading(false);
    }
  }, [projectId]);
        ...
        <div style={{ padding: 16 }}>
          ...
          <div style={cardStyle}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 12 }}>
              <span style={{ display: 'inline-flex' }}>{insStatus === 'current' ? <CheckCircle size={18} weight="fill" color={INS_STATUS_COLORS[insStatus]} /> : insStatus === 'expiring' ? <Warning size={18} weight="fill" color={INS_STATUS_COLORS[insStatus]} /> : <XCircle size={18} weight="fill" color={INS_STATUS_COLORS[insStatus]} />}</span>
    ...
    <div style={containerStyle}>
      ...
      <div style={headerStyle}>
        <FieldPageHeader
          ...
          subtitle={loading ? 'Loading...' : `${filtered.length} subcontractor${filtered.length !== 1 ? 's' : ''}`}
    ...
    <Suspense fallback={
      <div style={{
        ...
        <div style={{ textAlign: 'center' }}>
          ...
          <div style={{ fontSize: 14, color: '#CBD5E1' }}>Loading Prequalification...</div>
        </div>
      </div>
    }>
      <PrequalInner />
