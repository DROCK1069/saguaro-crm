'use client';
/**
 * Saguaro Control Systems — Smart Escalation Dashboard
...
export default function EscalationDashboard() {
  ...
  return (
    <div style={{ background: BASE, minHeight: '100vh', color: TEXT, fontFamily: 'system-ui, -apple-system, sans-serif' }}>
      {/* Header */}
      <FieldPageHeader
        title="Smart Escalation Dashboard"
        subtitle="Overdue RFIs, change orders, and submittals auto-detected"
        icon={scopedFieldIcon('rfis', 'ph')}
        actions={
          <button
            onClick={runCheck}
            disabled={running}
        ...
        <div style={{ display: 'flex', gap: 10, marginBottom: 20, flexWrap: 'wrap', alignItems: 'center' }}>
          {(['all', 'high', 'medium', 'low'] as const).map(f => (
            <button
              ...
              onClick={() => setFilter(f)}
              style={{
                background: filter === f ? (f === 'all' ? 'rgba(0,0,0,0.1)' : severityBg(f)) : 'transparent',
                color: filter === f ? (f === 'all' ? TEXT : severityColor(f)) : DIM,
                border: `1px solid ${filter === f ? (f === 'all' ? 'rgba(0,0,0,0.15)' : severityColor(f) + '40') : 'rgba(0,0,0,0.06)'}`,
                borderRadius: 8,
                padding: '6px 16px',
                fontSize: 13,
                fontWeight: 600,
