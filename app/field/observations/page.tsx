'use client';
/**
 * Saguaro Control Systems — Proactive Safety Observations
...
const OBS_TYPES: { key: ObsType; label: string; color: string; icon: React.ElementType }[] = [
  { key: 'positive',  label: 'Positive',  color: GREEN, icon: CheckCircle },
  { key: 'negative',  label: 'Negative',  color: RED,   icon: XCircle },
  { key: 'condition', label: 'Condition', color: AMBER, icon: Warning },
];
...
const TEMPLATES: Template[] = [
  { name: 'Housekeeping', category: 'General Safety', icon: Broom, items: [
  ...
  { name: 'PPE Compliance', category: 'Personal Protection', icon: HardHat, items: [
  ...
  { name: 'Fall Protection', category: 'Fall Prevention', icon: Anchor, items: [
  ...
  { name: 'Scaffolding', category: 'Scaffolding Safety', icon: Buildings, items: [
  ...
  { name: 'Electrical Safety', category: 'Electrical', icon: Lightning, items: [
  ...
  { name: 'Excavation / Trenching', category: 'Excavation', icon: Shovel, items: [
  ...
  { name: 'Fire Prevention', category: 'Fire Safety', icon: Fire, items: [
  ...
  { name: 'Tool Safety', category: 'Tools & Equipment', icon: Wrench, items: [
  ...
  { name: 'Confined Space', category: 'Confined Space', icon: Door, items: [
  ...
  { name: 'Crane / Rigging', category: 'Lifting Operations', icon: Crane, items: [
...
const inputStyle: React.CSSProperties = {
  background: '#1c1c1e', border: `1px solid ${BORDER}`, borderRadius: 10,
...
function ObservationsPage() {
  ...
  /* Quick presets */
  const quickPresets: { label: string; icon: React.ElementType; apply: () => void }[] = [
    {
      label: 'Open Items', icon: FolderOpen,
      apply: () => setFilters({ ...defaultFilters, statuses: ['open'] }),
    },
    {
      label: 'Safety Issues', icon: HardHat,
      apply: () => setFilters({ ...defaultFilters, types: ['negative'], priorities: ['high', 'critical'] }),
    },
    {
      label: 'This Week', icon: CalendarBlank,
      apply: () => {
        const now = new Date();
    ...
    {
      label: 'Needs Action', icon: Lightning,
  ...
  const renderHeader = () => (
    <FieldPageHeader
      ...
      subtitle={view === 'list' ? 'Proactive Safety Program' : view === 'create' ? 'New Observation' : 'Observation Details'}
      icon={scopedFieldIcon('inspections', 'ph')}
      onBack={() => {
  ...
  /* ── FILTERS ── */
  const obsChipStyle: React.CSSProperties = { display: 'inline-flex', alignItems: 'center', gap: 4, background: 'rgba(245, 158, 11,.12)', border: `1px solid rgba(245, 158, 11,.3)`, borderRadius: 20, padding: '4px 10px', fontSize: 11, fontWeight: 600, color: GOLD, cursor: 'pointer', whiteSpace: 'nowrap' };
    ...
    <div style={{ marginBottom: 14 }}>
      ...
      <div style={{ display: 'flex', gap: 8, alignItems: 'center', marginBottom: 10 }}>
        <button onClick={() => setFilterDrawerOpen(!filterDrawerOpen)} style={{
          background: filterDrawerOpen || filtersActive ? 'rgba(245, 158, 11,.15)' : 'transparent',
      ...
      <div style={{
        ...
        <div style={{ background: RAISED, border: `1px solid ${GOLD}44`, borderRadius: 14, padding: 14, marginBottom: 14 }}>
            ...
            <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
              {OBS_TYPES.map(t => {
                ...
                return (
                  ...
                  }}><t.icon size={14} weight="fill" color={active ? t.color : DIM} style={{ verticalAlign: 'middle' }} /> {t.label}</button>
                );
              })}
            </div>
          </div>
          ...
          <div style={{ marginBottom: 12 }}>
            ...
            <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
              {quickPresets.map(qp => (
                <button key={qp.label} onClick={qp.apply} style={obsChipStyle}><qp.icon size={13} weight="regular" color={GOLD} style={{ verticalAlign: 'middle' }} /> {qp.label}</button>
              ))}
            </div>
          </div>
          {/* Save Preset */}
          <div style={{ borderTop: `1px solid ${BORDER}`, paddingTop: 10, marginTop: 6 }}>
            ...
            <div style={{ display: 'flex', gap: 8 }}>
              ...
              <button onClick={saveCurrentPreset} disabled={!presetName.trim()} style={{
                background: presetName.trim() ? GOLD : '#1c1c1e', border: 'none', borderRadius: 8,
              ...
              <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                {savedPresets.map((p) => (
                  <span key={p.name} style={{ ...obsChipStyle, gap: 6 }}>
                    ...
                    <span onClick={() => deletePreset(p.name)} style={{ cursor: 'pointer', opacity: 0.6, fontSize: 13, lineHeight: 1 }}><X size={13} weight="bold" style={{ verticalAlign: 'middle' }} /></span>
                  </span>
                ))}
              </div>
            </div>
        ...
        <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginBottom: 10 }}>
          {filters.types.map(t => (
            <span key={t} style={obsChipStyle}>
              {t} <span onClick={() => setFilters(prev => ({ ...prev, types: prev.types.filter(v => v !== t) }))} style={{ cursor: 'pointer', marginLeft: 2, fontSize: 13 }}><X size={13} weight="bold" style={{ verticalAlign: 'middle' }} /></span>
            </span>
          ))}
          {filters.priorities.map(p => (
            <span key={p} style={obsChipStyle}>
              {p} <span onClick={() => setFilters(prev => ({ ...prev, priorities: prev.priorities.filter(v => v !== p) }))} style={{ cursor: 'pointer', marginLeft: 2, fontSize: 13 }}><X size={13} weight="bold" style={{ verticalAlign: 'middle' }} /></span>
            </span>
          ))}
          {filters.statuses.map(s => (
            <span key={s} style={obsChipStyle}>
              {s} <span onClick={() => setFilters(prev => ({ ...prev, statuses: prev.statuses.filter(v => v !== s) }))} style={{ cursor: 'pointer', marginLeft: 2, fontSize: 13 }}><X size={13} weight="bold" style={{ verticalAlign: 'middle' }} /></span>
            </span>
          ))}
          {filters.observedBy && (
            <span style={obsChipStyle}>
              Observer: {filters.observedBy} <span onClick={() => setFilters(prev => ({ ...prev, observedBy: '' }))} style={{ cursor: 'pointer', marginLeft: 2, fontSize: 13 }}><X size={13} weight="bold" style={{ verticalAlign: 'middle' }} /></span>
            </span>
          )}
          {(filters.dateFrom || filters.dateTo) && (
            <span style={obsChipStyle}>
              Date: {filters.dateFrom || '...'} - {filters.dateTo || '...'} <span onClick={() => setFilters(prev => ({ ...prev, dateFrom: '', dateTo: '' }))} style={{ cursor: 'pointer', marginLeft: 2, fontSize: 13 }}><X size={13} weight="bold" style={{ verticalAlign: 'middle' }} /></span>
            </span>
          )}
        </div>
      )}
            ...
            >
              ...
              <div style={{ display: 'flex', gap: 12, fontSize: 11, color: DIM, flexWrap: 'wrap' }}>
                ...
                {obs.checklist && obs.checklist.length > 0 && (
                  <span><CheckCircle size={12} weight="fill" color={GREEN} style={{ verticalAlign: 'middle' }} /> {obs.checklist.filter(c => c.result === 'pass').length}/{obs.checklist.length}</span>
                )}
              </div>
            </div>
          );
    ...
    <div>
      ...
      <div style={{ display: 'flex', gap: 8, marginBottom: 16 }}>
        {OBS_TYPES.map(t => (
          <button key={t.key} onClick={() => setNewType(t.key)} style={{
            ...
            <div style={{ fontSize: 22, marginBottom: 4 }}><t.icon size={24} weight="fill" color={newType === t.key ? t.color : DIM} /></div>
        ...
        <div style={{
          ...
          {TEMPLATES.map(t => (
            <button key={t.name} onClick={() => applyTemplate(t)} style={{
              ...
              <span style={{ fontSize: 22, display: 'inline-flex' }}><t.icon size={24} weight="regular" color={GOLD} /></span>
              <div>
      ...
      <div style={{ marginBottom: 16 }}>
        {newChecklist.map(item => (
          <div key={item.id} style={{
            ...
            <button onClick={() => removeCheckItem(item.id)} style={{
              ...
              padding: '2px 6px',
            }}><X size={16} weight="bold" color={DIM} style={{ verticalAlign: 'middle' }} /></button>
          </div>
        ))}
        {/* Add custom item */}
        <div style={{ display: 'flex', gap: 6, marginTop: 8 }}>
          ...
          }}><Plus size={16} weight="bold" color={TEXT} style={{ verticalAlign: 'middle' }} /></button>
        </div>
      </div>
        ...
        <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
          {newPhotosPreviews.map((src, i) => (
            <div key={i} style={{ position: 'relative', width: 72, height: 72 }}>
              ...
              }}><X size={12} weight="bold" color="#fff" /></button>
            </div>
          ))}
          <button onClick={() => fileInputRef.current?.click()} style={{
            width: 72, height: 72, background: '#1c1c1e', border: `2px dashed ${BORDER}`,
        ...
        {selected.checklist && selected.checklist.length > 0 && (
          <div style={cardStyle}>
            ...
            {selected.checklist.map(item => {
              const cfg = { pass: { color: GREEN, icon: CheckCircle }, fail: { color: RED, icon: XCircle }, na: { color: DIM, icon: MinusCircle } };
              ...
              return (
                <div key={item.id} style={{
                  ...
                  }}><c.icon size={16} weight="fill" color={c.color} /></span>
                  <span style={{
        ...
        {selected.corrective_required && selected.corrective_action && (
          <div style={{
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 12 }}>
              <span style={{ color: AMBER, fontSize: 16, display: 'inline-flex' }}><Lightning size={16} weight="fill" color={AMBER} /></span>
  ...
  return (
    <div style={{
      minHeight: '100vh', background: '#0a0a0a', color: TEXT,
  ...
  return (
    <Suspense fallback={
      <div style={{
        ...
        <div style={{ color: '#F59E0B', fontSize: 16, fontWeight: 700 }}>Loading Observations...</div>
      </div>
    }>
      <ObservationsPage />
    </Suspense>
