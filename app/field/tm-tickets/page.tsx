'use client';
/**
 * Saguaro Control Systems — T&M Tickets (Time & Material)
  ...
  if (view === 'list') {
    return (
      <div style={{ padding: '18px 16px', maxWidth: 900, margin: '0 auto' }}>
        <FieldPageHeader
          title="T&M Tickets"
          subtitle={projectName || 'Time & Material Tracking'}
          icon={scopedFieldIcon('tm', 'ph')}
          actions={<>
            {tickets.length > 0 && (
              <button onClick={() => { setSelectMode(!selectMode); setSelectedIds(new Set()); }} style={{
                background: selectMode ? 'rgba(245, 158, 11,.2)' : 'transparent', border: `1px solid ${selectMode ? GOLD : BORDER}`,
        ...
        {selectMode && filtered.length > 0 && (
          <div style={{ display: 'flex', gap: 8, marginBottom: 12, alignItems: 'center' }}>
            <button onClick={selectedIds.size === filtered.length ? deselectAllTickets : selectAllTickets} style={{
              background: 'rgba(245, 158, 11,.12)', border: `1px solid rgba(245, 158, 11,.3)`, borderRadius: 8,
        ...
        {loading ? (
            ...
            {filtered.map((t) => {
              ...
              return (
                <button
                  ...
                  style={{
                    background: isChecked ? 'rgba(245, 158, 11,.06)' : RAISED,
                ...
                >
                  <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 10 }}>
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap' }}>
                        ...
                        <button
                          ...
                          title="Money actions"
                        >{copiedId === t.id ? <CheckCircle size={14} weight="fill" /> : <CaretDown size={14} weight="regular" />}</button>
                    ...
                    {!selectMode && <span style={{ color: DIM, fontSize: 18, flexShrink: 0, marginTop: 4, display: 'inline-flex' }}><CaretRight size={18} weight="regular" /></span>}
                  </div>
                </button>
              );
            })}
          ...
          {selectMode && selectedIds.size > 0 && (
            <div style={{
              position: 'fixed', bottom: 0, left: 0, right: 0, background: '#141416',
            ...
            <div style={{ padding: '16px 20px 24px' }}>
              ...
              {sheetMode === 'menu' && (
                <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                  {[
                    { label: 'Adjust Markup %', icon: <ChartBar size={18} weight="regular" />, action: () => setSheetMode('adjust') },
                    { label: 'Copy Amount', icon: <Clipboard size={18} weight="regular" />, action: () => handleCopyTM(actionSheet.amount, actionSheet.ticketId) },
                  ].map(item => (
  ...
  if (view === 'detail' && selected) {
    ...
    return (
      <div style={{ padding: '18px 16px', maxWidth: 900, margin: '0 auto' }}>
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            ...
            <p style={{ margin: '0 0 16px', fontSize: 28, fontWeight: 800, color: GOLD }}>
              ...
              <button
                ...
                title="Money actions"
              >{copiedId === selected.id ? <CheckCircle size={16} weight="fill" /> : <CaretDown size={16} weight="regular" />}</button>
            </p>
          </div>
          <button onClick={() => {
            const t = selected;
        ...
        {/* Grand Total Card */}
        <div style={{ ...card, background: 'linear-gradient(135deg, #141416 0%, #141416 100%)' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
              {selected.contractor_signature && (
                <div>
                  ...
                  <div style={{ background: '#1c1c1e', borderRadius: 8, padding: 4, border: `1px solid ${BORDER}` }}>
        ...
        <div style={card}>
          ...
          {selected.status === 'draft' && (
            <button onClick={() => handleStatusChange(selected, 'submitted')} disabled={actionLoading}
              style={{ ...actionBtnStyle, background: actionLoading ? '#1c1c1e' : AMBER, color: actionLoading ? DIM : '#000', marginBottom: 10 }}>
          ...
          {selected.status === 'submitted' && (
            <>
              <button onClick={() => handleStatusChange(selected, 'approved')} disabled={actionLoading}
                style={{ ...actionBtnStyle, background: actionLoading ? '#1c1c1e' : GREEN, color: actionLoading ? DIM : '#000', marginBottom: 10 }}>
                {actionLoading ? 'Processing...' : 'Approve Ticket'}
              </button>
              <button onClick={() => handleStatusChange(selected, 'disputed')} disabled={actionLoading}
                style={{ ...actionBtnStyle, background: actionLoading ? '#1c1c1e' : RED, color: actionLoading ? DIM : '#fff' }}>
          ...
          {selected.status === 'disputed' && (
            <button onClick={() => handleStatusChange(selected, 'submitted')} disabled={actionLoading}
              style={{ ...actionBtnStyle, background: actionLoading ? '#1c1c1e' : AMBER, color: actionLoading ? DIM : '#000' }}>
    ...
    <div style={{ padding: '18px 16px', maxWidth: 900, margin: '0 auto' }}>
      ...
      <form onSubmit={handleCreate}>
        ...
        <div style={card}>
          ...
          {formLabor.map((l, idx) => (
            <div key={l.id} style={{ background: '#1c1c1e', border: `1px solid ${BORDER}`, borderRadius: 10, padding: 12, marginBottom: 10, position: 'relative' }}>
              {formLabor.length > 1 && (
                <button type="button" onClick={() => removeLabor(l.id)} style={removeRowBtn} title="Remove"><X size={16} weight="bold" /></button>
              )}
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, marginBottom: 8 }}>
                ...
                <div>
                  ...
                  <select value={l.trade} onChange={(e) => updateLabor(l.id, 'trade', e.target.value)} style={inpSm}>
                    {TRADES.map((t) => <option key={t} value={t} style={{ background: '#141416' }}>{t}</option>)}
                  </select>
                </div>
              </div>
        ...
        <div style={card}>
          ...
          {formMaterials.map((m) => (
            <div key={m.id} style={{ background: '#1c1c1e', border: `1px solid ${BORDER}`, borderRadius: 10, padding: 12, marginBottom: 10, position: 'relative' }}>
              {formMaterials.length > 1 && (
                <button type="button" onClick={() => removeMaterial(m.id)} style={removeRowBtn} title="Remove"><X size={16} weight="bold" /></button>
              )}
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 8 }}>
                ...
                <div>
                  ...
                  <select value={m.unit} onChange={(e) => updateMaterial(m.id, 'unit', e.target.value)} style={inpSm}>
                    {UNITS.map((u) => <option key={u} value={u} style={{ background: '#141416' }}>{u}</option>)}
                  </select>
                </div>
                <div>
        ...
        <div style={card}>
          ...
          {formEquipment.map((eq) => (
            <div key={eq.id} style={{ background: '#1c1c1e', border: `1px solid ${BORDER}`, borderRadius: 10, padding: 12, marginBottom: 10, position: 'relative' }}>
              {formEquipment.length > 1 && (
                <button type="button" onClick={() => removeEquipment(eq.id)} style={removeRowBtn} title="Remove"><X size={16} weight="bold" /></button>
              )}
              <div style={{ marginBottom: 8 }}>
                <label style={lbl}>Description</label>
        ...
        <div style={card}>
          ...
          <label style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, background: '#1c1c1e', border: `1px dashed ${BORDER}`, borderRadius: 10, padding: '14px', cursor: 'pointer', color: DIM, fontSize: 14 }}>
...
const inp: React.CSSProperties = { width: '100%', background: '#1c1c1e', border: '1px solid rgba(255,255,255,0.12)', borderRadius: 10, padding: '11px 14px', color: '#FFFFFF', fontSize: 15, outline: 'none', boxSizing: 'border-box' };
const inpSm: React.CSSProperties = { width: '100%', background: '#1c1c1e', border: '1px solid rgba(255,255,255,0.12)', borderRadius: 8, padding: '9px 12px', color: '#FFFFFF', fontSize: 14, outline: 'none', boxSizing: 'border-box' };
...
const addRowBtn: React.CSSProperties = { background: 'rgba(245, 158, 11,.12)', border: `1px solid ${GOLD}`, borderRadius: 8, padding: '5px 12px', color: GOLD, fontSize: 12, fontWeight: 700, cursor: 'pointer' };
