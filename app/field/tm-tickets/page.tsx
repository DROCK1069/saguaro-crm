...
function SignaturePad({ label, value, onChange }: { label: string; value: string; onChange: (dataUrl: string) => void }) {
  ...
  return (
    <div style={{ marginBottom: 14 }}>
      ...
      <div style={{ background: '#16243A', border: `1px solid ${BORDER}`, borderRadius: 10, overflow: 'hidden', touchAction: 'none' }}>
        <canvas
          ref={canvasRef}
          width={600}
          height={180}
  ...
  if (view === 'detail' && selected) {
    ...
    return (
      <div style={{ padding: '18px 16px', maxWidth: 900, margin: '0 auto' }}>
        ...
        {/* Grand Total Card */}
        <div style={{ ...card, background: 'linear-gradient(135deg, #0F172A 0%, #0F172A 100%)' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
              {selected.contractor_signature && (
                <div>
                  ...
                  <div style={{ background: '#16243A', borderRadius: 8, padding: 4, border: `1px solid ${BORDER}` }}>
        ...
        <div style={card}>
          ...
          {selected.status === 'draft' && (
            <button onClick={() => handleStatusChange(selected, 'submitted')} disabled={actionLoading}
              style={{ ...actionBtnStyle, background: actionLoading ? '#16243A' : AMBER, color: actionLoading ? DIM : '#000', marginBottom: 10 }}>
          ...
          {selected.status === 'submitted' && (
            <>
              <button onClick={() => handleStatusChange(selected, 'approved')} disabled={actionLoading}
                style={{ ...actionBtnStyle, background: actionLoading ? '#16243A' : GREEN, color: actionLoading ? DIM : '#000', marginBottom: 10 }}>
                {actionLoading ? 'Processing...' : 'Approve Ticket'}
              </button>
              <button onClick={() => handleStatusChange(selected, 'disputed')} disabled={actionLoading}
                style={{ ...actionBtnStyle, background: actionLoading ? '#16243A' : RED, color: actionLoading ? DIM : '#fff' }}>
          ...
          {selected.status === 'disputed' && (
            <button onClick={() => handleStatusChange(selected, 'submitted')} disabled={actionLoading}
              style={{ ...actionBtnStyle, background: actionLoading ? '#16243A' : AMBER, color: actionLoading ? DIM : '#000' }}>
    ...
    <div style={{ padding: '18px 16px', maxWidth: 900, margin: '0 auto' }}>
      ...
      <form onSubmit={handleCreate}>
        ...
        <div style={card}>
          ...
          {formLabor.map((l, idx) => (
            <div key={l.id} style={{ background: '#16243A', border: `1px solid ${BORDER}`, borderRadius: 10, padding: 12, marginBottom: 10, position: 'relative' }}>
              {formLabor.length > 1 && (
        ...
        <div style={card}>
          ...
          {formMaterials.map((m) => (
            <div key={m.id} style={{ background: '#16243A', border: `1px solid ${BORDER}`, borderRadius: 10, padding: 12, marginBottom: 10, position: 'relative' }}>
        ...
        <div style={card}>
          ...
          {formEquipment.map((eq) => (
            <div key={eq.id} style={{ background: '#16243A', border: `1px solid ${BORDER}`, borderRadius: 10, padding: 12, marginBottom: 10, position: 'relative' }}>
        ...
        <div style={card}>
          ...
          <label style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, background: '#16243A', border: `1px dashed ${BORDER}`, borderRadius: 10, padding: '14px', cursor: 'pointer', color: DIM, fontSize: 14 }}>
...
const inpSm: React.CSSProperties = { width: '100%', background: '#16243A', border: '1px solid rgba(255,255,255,0.12)', borderRadius: 8, padding: '9px 12px', color: '#FFFFFF', fontSize: 14, outline: 'none', boxSizing: 'border-box' };
