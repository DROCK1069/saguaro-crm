'use client';
/**
 * Saguaro Control Systems — Correspondence
...
function typeIcon(t: CorrespondenceType, size = 16, color = GOLD): React.ReactNode {
  const p = { size, color, style: { verticalAlign: 'middle' as const } };
  switch (t) {
    case 'Letter': return <Envelope weight="regular" {...p} />;
    case 'Transmittal': return <Package weight="regular" {...p} />;
    case 'Notice': return <Warning weight="fill" {...p} />;
    case 'Memo': return <FileText weight="regular" {...p} />;
    case 'Email Record': return <EnvelopeSimple weight="regular" {...p} />;
    default: return <Envelope weight="regular" {...p} />;
  }
}
...
function CorrespondencePage() {
  ...
  const input: React.CSSProperties = {
    ...
    border: `1px solid ${BORDER}`, background: '#1c1c1e', color: TEXT,
  ...
  const renderHeader = () => (
    <FieldPageHeader
      ...
      title={view === 'create' ? 'New Correspondence' : view === 'detail' ? 'Correspondence Detail' : 'Correspondence'}
      onBack={() => {
        if (view !== 'list') { setView('list'); setSelected(null); setReplyMode(null); }
        else { router.back(); }
      }}
      actions={
        <>
  ...
  const renderList = () => (
    <>
      ...
      <div style={{ display: 'flex', gap: 8, marginBottom: 8, flexWrap: 'wrap', alignItems: 'center' }}>
        ...
        >
          <Gear size={14} weight="regular" color={filterDrawerOpen ? '#000' : (filtersAreActive ? GOLD : DIM)} style={{ verticalAlign: 'middle', marginRight: 4 }} />Filters {filtersAreActive ? '\u2022' : ''}
        </button>
        {filtersAreActive && (
      ...
      {filterDrawerOpen && (
        <div style={{
            ...
            <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
              {TYPES.map(t => (
                <button key={t} onClick={() => toggleAdvType(t)} style={{
                  ...
                  {typeIcon(t, 14, advFilters.types.includes(t) ? '#000' : DIM)} {t}
                </button>
              ))}
            </div>
          </div>
          ...
          <div style={{ display: 'flex', gap: 10, marginBottom: 14, flexWrap: 'wrap', alignItems: 'flex-end' }}>
            ...
            >
              {advFilters.sortDir === 'asc'
                ? <><ArrowUp size={14} weight="bold" color={GOLD} style={{ verticalAlign: 'middle', marginRight: 4 }} />Ascending</>
                : <><ArrowDown size={14} weight="bold" color={GOLD} style={{ verticalAlign: 'middle', marginRight: 4 }} />Descending</>}
            </button>
          </div>
              ...
              <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                {savedPresets.map(sp => (
                  <div key={sp.name} style={{
                    ...
                    background: '#1c1c1e', border: `1px solid ${BORDER}`, borderRadius: 8,
        ...
        <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginBottom: 10 }}>
          {advFilters.types.map(t => (
            <span key={t} style={{
              ...
              {t} <span onClick={() => toggleAdvType(t)} style={{ cursor: 'pointer', marginLeft: 2, fontSize: 13, display: 'inline-flex', verticalAlign: 'middle' }}><X size={13} weight="bold" color={GOLD} /></span>
            </span>
          ))}
          {advFilters.statuses.map(s => (
            <span key={s} style={{
              ...
              {s} <span onClick={() => toggleAdvStatus(s)} style={{ cursor: 'pointer', marginLeft: 2, fontSize: 13, display: 'inline-flex', verticalAlign: 'middle' }}><X size={13} weight="bold" color={GOLD} /></span>
            </span>
          ))}
          {advFilters.fromTo && (
            <span style={{
              ...
              From/To: {advFilters.fromTo} <span onClick={() => setAdvFilters(prev => ({ ...prev, fromTo: '' }))} style={{ cursor: 'pointer', marginLeft: 2, fontSize: 13, display: 'inline-flex', verticalAlign: 'middle' }}><X size={13} weight="bold" color={GOLD} /></span>
            </span>
          )}
          {(advFilters.dateFrom || advFilters.dateTo) && (
            <span style={{
              ...
              Date: {advFilters.dateFrom || '...'} - {advFilters.dateTo || '...'} <span onClick={() => setAdvFilters(prev => ({ ...prev, dateFrom: '', dateTo: '' }))} style={{ cursor: 'pointer', marginLeft: 2, fontSize: 13, display: 'inline-flex', verticalAlign: 'middle' }}><X size={13} weight="bold" color={GOLD} /></span>
            </span>
          )}
        </div>
      )}
      ...
      {loading ? (
        ...
        filtered.map(item => (
          <div key={item.id} onClick={() => { setSelected(item); setView('detail'); markRead(item); }} style={{
            ...
            <div style={{ fontSize: 24, lineHeight: 1, flexShrink: 0, marginTop: 2 }}>{typeIcon(item.type, 24, GOLD)}</div>
            <div style={{ flex: 1, minWidth: 0 }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginTop: 4 }}>
                ...
                {item.attachments?.length > 0 && (
                  <span style={{ fontSize: 10, color: DIM }}><Paperclip size={11} weight="regular" color={DIM} style={{ verticalAlign: 'middle', marginRight: 3 }} />{item.attachments.length}</span>
                )}
                {item.read_receipts?.length > 0 && (
                  <span title={`Read by ${item.read_receipts.length}`} style={{ fontSize: 12, color: GREEN, cursor: 'default' }}>
                    <Eye size={13} weight="regular" color={GREEN} style={{ verticalAlign: 'middle', marginRight: 3 }} />{formatDateTime(item.read_receipts[item.read_receipts.length - 1].read_at)}
                  </span>
                )}
              </div>
            </div>
      ...
      <>
        ...
        <div style={card}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: 8 }}>
            <div>
              <span style={{ fontSize: 26, marginRight: 8 }}>{typeIcon(selected.type, 26, GOLD)}</span>
            ...
            <div style={{ display: 'flex', gap: 6 }}>
              ...
              <button onClick={handlePrint} style={btn(BORDER, true)}><Printer size={14} weight="regular" color="#fff" style={{ verticalAlign: 'middle', marginRight: 4 }} />Print</button>
            </div>
          </div>
          ...
          <div style={{
            ...
            background: '#1c1c1e', borderRadius: 10, whiteSpace: 'pre-wrap',
          ...
          {selected.type === 'Transmittal' && (
            <div style={{ ...card, background: '#1c1c1e', marginBottom: 14 }}>
            ...
            <div style={{ marginBottom: 14 }}>
              ...
              {selected.attachments.map((att, i) => (
                <a key={i} href={att.url} download={att.name} target="_blank" rel="noopener noreferrer" style={{
                  ...
                  background: '#1c1c1e', borderRadius: 8, marginBottom: 4,
                  border: `1px solid ${BORDER}`, textDecoration: 'none', cursor: 'pointer',
                }}>
                  <span style={{ fontSize: 16 }}><Paperclip size={16} weight="regular" color={DIM} style={{ verticalAlign: 'middle' }} /></span>
          ...
          {selected.request_read_receipt && (
            <div style={{ marginBottom: 14 }}>
              <span style={label}><Eye size={14} weight="regular" color={DIM} style={{ verticalAlign: 'middle', marginRight: 4 }} />Read Receipts</span>
              {selected.read_receipts?.length > 0 ? (
                selected.read_receipts.map((rr, i) => (
                  <div key={i} style={{
                    ...
                    <span style={{ color: GREEN, fontSize: 14 }}><CheckCircle size={14} weight="fill" color={GREEN} style={{ verticalAlign: 'middle' }} /></span>
                  ...
                  <div style={{ marginTop: 6 }}>
                    {reply.attachments.map((att, i) => (
                      <a key={i} href={att.url} download style={{ color: BLUE, fontSize: 12, marginRight: 10, textDecoration: 'none' }}>
                        <Paperclip size={12} weight="regular" color={BLUE} style={{ verticalAlign: 'middle', marginRight: 3 }} />{att.name}
                      </a>
                    ))}
                  </div>
                )}
                  ...
                  <div key={i} style={{ display: 'flex', gap: 6, marginBottom: 4 }}>
                    ...
                    {forwardTo.length > 1 && (
                      <button onClick={() => removeRecipient(forwardTo, i, setForwardTo)}
                        style={{ ...btn(RED, true), padding: '6px 10px' }}><X size={14} weight="bold" color="#fff" /></button>
                    )}
                  </div>
                ))}
    ...
    <div style={card}>
        ...
        <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
          {TYPES.map(t => (
            <button key={t} onClick={() => setFormType(t)} style={{
              ...
              {typeIcon(t, 14, formType === t ? '#000' : DIM)} {t}
            </button>
          ))}
        </div>
      </div>
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          {(['Normal', 'Urgent'] as Priority[]).map(p => (
            <button key={p} onClick={() => setFormPriority(p)} style={{
              ...
              {p === 'Urgent' ? <Lightning size={13} weight="fill" color={formPriority === p ? '#fff' : DIM} style={{ verticalAlign: 'middle', marginRight: 4 }} /> : null}{p}
            </button>
          ))}
        </div>
      </div>
      ...
      <div style={{ marginBottom: 14 }}>
        ...
        <div style={{ display: 'flex', gap: 4, marginBottom: 4 }}>
          ...
          <button onClick={() => setFormBody(prev => prev + '\n- ')}
            style={{ ...btn(BORDER, true), fontSize: 13 }}><ListBullets size={13} weight="regular" color="#fff" style={{ verticalAlign: 'middle', marginRight: 4 }} />List</button>
          <button onClick={() => setFormBody(prev => prev + '\n---\n')}
          ...
          <div style={{ marginTop: 6 }}>
            {formAttachments.map((f, i) => (
              <div key={i} style={{
                ...
                background: '#1c1c1e', borderRadius: 6, marginBottom: 3,
              }}>
                <span style={{ color: TEXT, fontSize: 12, flex: 1 }}><Paperclip size={12} weight="regular" color={TEXT} style={{ verticalAlign: 'middle', marginRight: 3 }} />{f.name}</span>
      ...
      <div style={{ display: 'flex', gap: 10 }}>
        <button onClick={() => handleSubmit(false)} disabled={submitting}
          ...
          {submitting ? 'Sending...' : <><Envelope size={14} weight="regular" color="#fff" style={{ verticalAlign: 'middle', marginRight: 6 }} />Send</>}
        </button>
  ...
  /* ─── Main render ─── */
  return (
    <div style={{ minHeight: '100vh', background: '#0a0a0a', padding: '16px 12px 80px' }}>
