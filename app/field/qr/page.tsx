'use client';
/**
 * Saguaro Control Systems — QR Code Scanner + Generator
...
const GREEN = '#22C55E', BLUE = '#F59E0B';
...
function QRPage() {
  ...
  return (
    <div style={{ padding: '18px 16px', paddingBottom: 40 }}>
      <FieldPageHeader title="QR Scanner" icon={scopedFieldIcon('qr', 'ph')} />

      {/* Tabs */}
      <div style={{ display: 'flex', background: '#0a0a0a', borderRadius: 12, padding: 4, marginBottom: 20, border: `1px solid ${BORDER}` }}>
          ...
          <div style={{
            ...
            {scanning && (
              <>
                ...
                <div style={{ position: 'absolute', bottom: 12, left: '50%', transform: 'translateX(-50%)', fontSize: 12, color: 'rgba(255,255,255,.6)', background: '#1c1c1e', borderRadius: 8, padding: '4px 10px', whiteSpace: 'nowrap' }}>
                  Scanning...
                </div>
              </>
            )}
      ...
      {tab === 'generate' && (
        <div>
          ...
          <button
            ...
            style={{
              width: '100%',
              background: genText.trim() ? GOLD : 'rgba(255,255,255,0.12)',
              border: 'none',
              borderRadius: 12,
              padding: '15px',
