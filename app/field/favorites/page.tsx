'use client';
/**
 * Saguaro Control Systems — Favorites & Recents
...
import { Question, Hammer, NotePencil, Clipboard, MagnifyingGlass, Notebook, Ruler, Camera, Warning, FileText, CalendarBlank, Tractor, Envelope, Handshake, PushPin, Star, Clock } from '@phosphor-icons/react';
...
const MODULE_META: Record<string, { icon: React.ComponentType<any>; color: string; label: string }> = {
  rfi:           { icon: Question, color: BLUE,  label: 'RFIs' },
  punch:         { icon: Hammer, color: AMBER, label: 'Punch List' },
  change_order:  { icon: NotePencil, color: GOLD,  label: 'Change Orders' },
  submittal:     { icon: Clipboard, color: '#8B5CF6', label: 'Submittals' },
  inspection:    { icon: MagnifyingGlass, color: GREEN, label: 'Inspections' },
  daily_log:     { icon: Notebook, color: DIM,   label: 'Daily Logs' },
  drawing:       { icon: Ruler, color: '#06B6D4', label: 'Drawings' },
  photo:         { icon: Camera, color: '#EC4899', label: 'Photos' },
  safety:        { icon: Warning, color: RED,   label: 'Safety' },
  document:      { icon: FileText, color: DIM,   label: 'Documents' },
  schedule:      { icon: CalendarBlank, color: BLUE,  label: 'Schedule' },
  equipment:     { icon: Tractor, color: AMBER, label: 'Equipment' },
  correspondence:{ icon: Envelope, color: '#8B5CF6', label: 'Correspondence' },
  meeting:       { icon: Handshake, color: GREEN, label: 'Meetings' },
};

function getModuleMeta(mod: string) {
  return MODULE_META[mod] || { icon: PushPin, color: DIM, label: mod };
}
...
function FavoritesRecentsPage() {
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#0a0a0a', fontFamily: '-apple-system, BlinkMacSystemFont, sans-serif', color: TEXT, paddingBottom: 100 }}>
      {/* Header */}
      <div style={{ background: RAISED, borderBottom: `1px solid ${BORDER}`, position: 'sticky', top: 0, zIndex: 50 }}>
        <FieldPageHeader
          title={tab === 'favorites' ? 'Favorites' : 'Recents'}
          icon={scopedFieldIcon('allTools', 'ph')}
          actions={tab === 'recents' && recents.length > 0 ? (
          ...
          {(['favorites', 'recents'] as Tab[]).map(t => (
            ...
            >
              <span style={{ display: 'inline-flex', alignItems: 'center', gap: 6 }}>
                {t === 'favorites'
                  ? <Star size={14} weight="fill" color={tab === t ? GOLD : DIM} />
                  : <Clock size={14} weight="regular" color={tab === t ? GOLD : DIM} />}
          ...
          <div style={{ display: 'flex', gap: 8, overflowX: 'auto', paddingBottom: 4 }}>
            {frequentItems.map(item => {
              ...
              return (
                ...
                >
                  <span style={{ fontSize: 18, display: 'inline-flex' }}><meta.icon size={18} weight="fill" color={meta.color} /></span>
        ...
        <div style={{ padding: '12px 16px' }}>
          {groupedFavorites.length === 0 ? (
            <div style={{ textAlign: 'center', padding: 40, color: DIM }}>
              <div style={{ fontSize: 40, marginBottom: 12, display: 'flex', justifyContent: 'center' }}><Star size={40} weight="fill" color={GOLD} /></div>
              ...
              return (
                <div key={group.module} style={{ marginBottom: 20 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 10 }}>
                    <span style={{ fontSize: 16, display: 'inline-flex' }}><meta.icon size={16} weight="fill" color={meta.color} /></span>
      ...
      {!loading && tab === 'recents' && (
        <div style={{ padding: '12px 16px' }}>
          {timeGroups.length === 0 ? (
            <div style={{ textAlign: 'center', padding: 40, color: DIM }}>
              <div style={{ fontSize: 40, marginBottom: 12, display: 'flex', justifyContent: 'center' }}><Clock size={40} weight="regular" color={DIM} /></div>
