'use client';
/**
 * Saguaro Control Systems — Bid Management
...
import { PencilSimple, Trash, DotsThree, CheckCircle, ChartBar, Copy, NoteBlank } from '@phosphor-icons/react';
...
const thStyle: React.CSSProperties = { textAlign: 'left', padding: '10px 12px', fontSize: 12, fontWeight: 700, color: DIM, borderBottom: `1px solid ${BORDER}`, whiteSpace: 'nowrap', position: 'sticky' as const, top: 0, background: '#141416', zIndex: 1 };
              ...
              {ranked.map((b, i) => {
                ...
                return (
                  <div key={b.bidder_id} style={{ marginBottom: 12 }}>
                    ...
                    <div style={{ height: 24, background: '#1c1c1e', borderRadius: 8, overflow: 'hidden', position: 'relative' }}>
                      <div style={{
                        height: '100%',
                  ...
                  {bids.map((b, i) => {
                    ...
                    return (
                      <div key={b.name} style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4 }}>
                        ...
                        <div style={{ flex: 1, height: 14, background: '#1c1c1e', borderRadius: 4, overflow: 'hidden' }}>
            ...
            <div style={card}>
              {[...comparison.bidders].sort((a, b) => a.total - b.total).map((b, i) => {
                ...
                return (
                  <SwipeActionItem
                    key={b.bidder_id}
                    leftAction={{ label: 'Edit', color: GOLD, icon: <PencilSimple size={18} weight="regular" color="#fff" />, onAction: () => openActionSheet(b.bidder_id, b.company_name, b.total) }}
                    rightAction={{ label: 'Delete', color: RED, icon: <Trash size={18} weight="regular" color="#fff" />, onAction: () => setDeleteConfirm(b.bidder_id) }}
                  >
                    <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 0', borderBottom: i < comparison.bidders.length - 1 ? `1px solid rgba(255,255,255,0.06)` : 'none', background: RAISED }}>
                      <div style={{ width: 28, height: 28, borderRadius: '50%', background: i === 0 ? `${GREEN}22` : '#1c1c1e', display: 'flex', alignItems: 'center', justifyContent: 'center', color: i === 0 ? GREEN : DIM, fontWeight: 800, fontSize: 12, flexShrink: 0 }}>
                        #{i + 1}
                      </div>
                      <div style={{ flex: 1 }}>
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                        ...
                        <button onClick={(e) => { e.stopPropagation(); openActionSheet(b.bidder_id, b.company_name, b.total); }} style={{ background: 'none', border: 'none', color: DIM, cursor: 'pointer', padding: '4px 6px', fontSize: 16, lineHeight: 1, display: 'inline-flex', alignItems: 'center' }}><DotsThree size={20} weight="bold" color={DIM} /></button>
                      </div>
                    </div>
                  </SwipeActionItem>
                );
  ...
  return (
    <div style={{ padding: '18px 16px', maxWidth: 700, margin: '0 auto' }}>
      <FieldPageHeader
        ...
        icon={scopedFieldIcon('dashboard', 'ph')}
      />
      ...
      {deleteConfirm && (
        <div style={{ position: 'fixed', inset: 0, background: '#0a0a0a', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 700, padding: 24 }}>
          <div style={{ background: RAISED, border: `1px solid ${BORDER}`, borderRadius: 16, padding: 24, width: '100%', maxWidth: 340, textAlign: 'center' }}>
            <div style={{ marginBottom: 12, display: 'flex', justifyContent: 'center' }}><Trash size={36} weight="regular" color={RED} /></div>
      ...
      {fieldToast && (
        <div style={{ position: 'fixed', bottom: 90, left: '50%', transform: 'translateX(-50%)', background: RAISED, border: `1px solid ${BORDER}`, borderRadius: 10, padding: '10px 20px', color: GREEN, fontSize: 13, fontWeight: 600, zIndex: 800, boxShadow: '0 8px 24px rgba(0,0,0,.4)', display: 'flex', alignItems: 'center', gap: 8 }}>
          <CheckCircle size={16} weight="fill" color={GREEN} />{fieldToast}
        </div>
      )}
          ...
          <div key={pkg.id} style={{ ...card, cursor: 'pointer' }} onClick={() => openPackage(pkg)}>
            ...
            {invCount > 0 && (
              <div style={{ marginTop: 8 }}>
                <div style={{ height: 4, background: '#1c1c1e', borderRadius: 3, overflow: 'hidden' }}>
...
function MoneyActionSheet({
  ...
  const menuItems = [
    { label: 'Edit Amount', icon: <PencilSimple size={20} weight="regular" color="#FFFFFF" />, action: () => { setSheetMode('edit'); setEditVal(String(actionSheet.amount)); } },
    { label: 'Adjust %', icon: <ChartBar size={20} weight="regular" color="#FFFFFF" />, action: () => setSheetMode('adjust') },
    { label: 'Copy Amount', icon: <Copy size={20} weight="regular" color="#FFFFFF" />, action: () => onCopy() },
    { label: 'Add Note', icon: <NoteBlank size={20} weight="regular" color="#FFFFFF" />, action: () => setSheetMode('note') },
  ];
        ...
        {sheetMode === 'edit' && (
          <div>
            ...
            <input
              ...
              style={{ width: '100%', padding: '14px 16px', background: '#1c1c1e', border: '1px solid #F59E0B', borderRadius: 10, color: '#FFFFFF', fontSize: 20, fontWeight: 700, outline: 'none', boxSizing: 'border-box', textAlign: 'right' }}
            />
            <div style={{ display: 'flex', gap: 10, marginTop: 14 }}>
        ...
        {sheetMode === 'note' && (
          <div>
            ...
            <textarea
              ...
              style={{ width: '100%', padding: '12px 14px', background: '#1c1c1e', border: '1px solid #F59E0B', borderRadius: 10, color: '#FFFFFF', fontSize: 14, outline: 'none', resize: 'vertical', boxSizing: 'border-box' }}
            />
            <div style={{ display: 'flex', gap: 10, marginTop: 14 }}>
