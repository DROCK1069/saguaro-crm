'use client';
/**
 * Saguaro Control Systems — Daily Log
...
function DailyLogForm() {
        ...
        {selectMode && selectedLogIds.size > 0 && (
          <div style={{
            ...
            <button onClick={() => handleBulkEmailLogs()} disabled={bulkProcessing} style={{
              background: 'rgba(245,158,11,.15)', border: `1px solid rgba(245,158,11,.3)`, borderRadius: 10,
              padding: '10px 14px', color: '#F59E0B', fontSize: 12, fontWeight: 700, cursor: 'pointer', flex: 1,
            }}>
  ...
  return (
    <div style={{ padding: '18px 16px' }}>
      <FieldPageHeader
        title="Daily Log"
        subtitle={`${new Date().toLocaleDateString('en-US', { weekday: 'long', month: 'long', day: 'numeric' })}${projectName ? ` · ${projectName}` : ''}`}
        icon={scopedFieldIcon('dailyLog', 'ph')}
        actions={
      ...
      <form onSubmit={handleSubmit}>
        ...
        <Section label="Site Conditions">
          ...
          <Field label="Weather">
            <select value={weather} onChange={(e) => setWeather(e.target.value)} style={inp}>
              {WEATHER_OPTS.map((o) => <option key={o} value={o} style={{ background: '#141416' }}>{o}</option>)}
            </select>
          </Field>
          <Row>
        ...
        <Section label="Work Performed">
          ...
          <div style={{ display: 'flex', gap: 8, marginBottom: 10 }}>
            ...
            <button
              ...
              style={{ background: listening ? 'rgba(239,68,68,.2)' : 'rgba(245, 158, 11,.1)', border: `1px solid ${listening ? 'rgba(239,68,68,.4)' : 'rgba(245, 158, 11,.25)'}`, borderRadius: 10, padding: '10px 14px', color: listening ? RED : GOLD, fontSize: 18, cursor: 'pointer' }}
              title="Voice to text"
            >
              {listening ? (
          ...
          <div style={{ padding: '14px' }}>
            ...
            {activeTab === 'Notes/General' && (
              <div>
                ...
                <div style={{ background: 'rgba(245, 158, 11,.06)', border: `1px solid rgba(245, 158, 11,.15)`, borderRadius: 10, padding: 12 }}>
            ...
            {activeTab === 'Manpower' && (
              <div>
                ...
                {manpowerEntries.map((entry, idx) => (
                  <div key={entry.id} style={{ background: 'rgba(255,255,255,0.04)', border: `1px solid ${BORDER}`, borderRadius: 10, padding: 12, marginBottom: 10, position: 'relative' }}>
            ...
            <div style={{ display: 'flex', gap: 8, overflowX: 'auto', paddingBottom: 4 }}>
              {sitePhotoPreviews.map((src, i) => (
                <div key={i} style={{ position: 'relative', flexShrink: 0 }}>
                  ...
                  <img src={src} alt={`Site photo ${i + 1}`} style={{ width: 72, height: 72, objectFit: 'cover', borderRadius: 8, border: '1px solid rgba(255,255,255,0.12)' }} />
        ...
        <Section label="Superintendent Signature">
          {signatureData ? (
            <div style={{ textAlign: 'center' }}>
              {/* eslint-disable-next-line @next/next/no-img-element */}
              <img src={signatureData} alt="Signature" style={{ maxWidth: '100%', height: 80, borderRadius: 8, border: '1px solid rgba(255,255,255,0.12)' }} />
              <button type="button" onClick={() => { setSignatureData(''); setShowSignature(true); }} style={{ marginTop: 6, background: 'transparent', border: '1px solid rgba(255,255,255,0.12)', borderRadius: 8, padding: '6px 14px', color: DIM, fontSize: 12, cursor: 'pointer' }}>Re-sign</button>
            </div>
          ) : showSignature ? (
        ...
        <button
          type="submit"
          disabled={saving}
          style={{ width: '100%', background: saving ? '#1c1c1e' : GOLD, border: 'none', borderRadius: 14, padding: '18px', color: saving ? DIM : '#000', fontSize: 17, fontWeight: 800, cursor: saving ? 'wait' : 'pointer', marginTop: 4, letterSpacing: 0.3 }}
        >
...
export default function FieldLogPage() {
  return <Suspense fallback={<div style={{ padding: 32, color: '#CBD5E1', textAlign: 'center' }}>Loading...</div>}><DailyLogForm /></Suspense>;
}

// ── Shared UI helpers ─────────────────────────────────────
function Section({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <div style={{ background: '#141416', border: '1px solid rgba(255,255,255,0.12)', borderRadius: 14, padding: '14px 14px 6px', marginBottom: 12 }}>
      <p style={{ margin: '0 0 12px', fontSize: 11, fontWeight: 700, color: '#CBD5E1', textTransform: 'uppercase', letterSpacing: 0.8 }}>{label}</p>
      {children}
    </div>
  );
}
function Field({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <div style={{ marginBottom: 10 }}>
      <label style={{ display: 'block', fontSize: 12, color: '#CBD5E1', marginBottom: 5 }}>{label}</label>
      {children}
    </div>
  );
}
...
const inp: React.CSSProperties = { width: '100%', background: '#1c1c1e', border: '1px solid rgba(255,255,255,0.12)', borderRadius: 10, padding: '12px 14px', color: '#FFFFFF', fontSize: 15, outline: 'none' };
