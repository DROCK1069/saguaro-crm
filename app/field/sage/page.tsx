'use client';
/**
 * Saguaro Control Systems — Sage AI Chat
  ...
  const sendMessage = useCallback(async (text: string) => {
    ...
    try {
      ...
      const history = messages.map(m => ({
        role: (m.role === 'sage' || (m.role as string) === 'assistant') ? 'assistant' : 'user',
        content: m.content,
      }));
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', background: '#0a0a0a' }}>
      ...
      <div style={{ padding: '10px 12px', paddingBottom: 'max(10px, env(safe-area-inset-bottom))', background: RAISED, borderTop: `1px solid ${BORDER}`, flexShrink: 0 }}>
        <div style={{ display: 'flex', alignItems: 'flex-end', gap: 8, background: '#1c1c1e', border: `1px solid ${BORDER}`, borderRadius: 22, padding: '6px 6px 6px 14px' }}>
          <textarea
            ref={inputRef}
            value={input}
            onChange={e => {
