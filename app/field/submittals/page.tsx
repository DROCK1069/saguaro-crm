'use client';
/**
 * Saguaro Control Systems — Submittals Mobile Review
...
function SubmittalsPage() {
    ...
    return (
      <div style={{ padding: '18px 16px', minHeight: '100vh', background: '#0a0a0a' }}>
        ...
        <div style={{ background: RAISED, border: `1px solid ${BORDER}`, borderRadius: 14, padding: 16 }}>
          ...
          {newAttachment && (
            <div style={{ marginBottom: 14, padding: '6px 10px', background: '#1c1c1e', borderRadius: 8, border: `1px solid ${BORDER}`, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
      ...
      <div style={{ padding: '18px 16px', minHeight: '100vh', background: '#0a0a0a' }}>
        ...
        <div style={{ background: RAISED, border: `1px solid ${BORDER}`, borderRadius: 14, padding: 16, marginBottom: 14 }}>
          ...
          <button onClick={() => setShowEmail(true)} style={{
            marginTop: 12, background: 'rgba(245, 158, 11,.1)', border: '1px solid rgba(245, 158, 11,.3)',
              ...
              {selected.revisions.map((rev, i) => (
                <div key={i} style={{ position: 'relative', paddingBottom: i < selected.revisions!.length - 1 ? 18 : 0 }}>
                  ...
                  <div style={{
                    ...
                    background: getStatusColor(rev.status), border: '2px solid #141416',
                  }} />
        ...
        {!reviewAction ? (
          ...
          <div style={{ background: RAISED, border: `1px solid ${BORDER}`, borderRadius: 14, padding: 16 }}>
            ...
            <textarea
              ...
              style={{ width: '100%', padding: '10px 12px', background: '#1c1c1e', border: `1px solid ${BORDER}`, borderRadius: 10, color: TEXT, fontSize: 14, resize: 'vertical', outline: 'none', boxSizing: 'border-box' }}
            />
            <div style={{ display: 'flex', gap: 8, marginTop: 12 }}>
              <button
  ...
  return (
    <div style={{ padding: '18px 16px', minHeight: '100vh', background: '#0a0a0a' }}>
      <FieldPageHeader
        title="Submittals"
        subtitle="Review and manage submittals"
        icon={scopedFieldIcon('submittals', 'ph')}
        actions={
          <button
...
export default function FieldSubmittalsPage() {
  return <Suspense fallback={<div style={{ padding: 32, color: '#CBD5E1', textAlign: 'center' }}>Loading...</div>}><SubmittalsPage /></Suspense>;
}
