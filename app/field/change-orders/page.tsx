'use client';
/**
 * Saguaro Control Systems — Change Orders
...
function ChangeOrdersPage() {
  ...
  const drawerPanelStyle: React.CSSProperties = {
    ...
    background: '#141416', borderLeft: `1px solid ${BORDER}`,
  ...
  const EmailModal = () => {
    ...
    return (
      <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,.7)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16 }}>
        <div style={{ background: '#141416', border: `1px solid ${BORDER}`, borderRadius: 16, width: '100%', maxWidth: 500, maxHeight: '90vh', overflow: 'auto', padding: 20 }}>
          ...
          <div style={{ display: 'flex', gap: 10 }}>
            ...
            <button onClick={handleSendEmail} disabled={emailSending || !emailTo.trim()} style={{
              flex: 2, background: emailSending || !emailTo.trim() ? '#1c1c1e' : BLUE, border: 'none', borderRadius: 12,
                ...
                <div style={{ marginBottom: 14 }}>
                  ...
                  <div style={{ display: 'flex', gap: 8 }}>
                    ...
                    <button onClick={saveCoPreset} disabled={!coPresetName.trim()} style={{
                      background: coPresetName.trim() ? GOLD : '#1c1c1e', border: 'none', borderRadius: 10,
              ...
              {sheetMode === 'menu' && (
                <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                  {[
                    ...
                    { label: 'Copy Amount', icon: <Copy size={20} />, action: () => { handleCOCopy(actionSheet.amount, actionSheet.coId); } },
                    { label: 'Reject CO', icon: <Trash size={20} />, action: () => { closeMoneySheet(); setDeleteConfirm(actionSheet.coId); } },
                  ].map(item => (
              ...
              {sheetMode === 'edit' && (
                <div>
                  ...
                  <input
                    ...
                    style={{ width: '100%', padding: '14px 16px', background: '#1c1c1e', border: `1px solid ${GOLD}`, borderRadius: 10, color: TEXT, fontSize: 20, fontWeight: 700, outline: 'none', boxSizing: 'border-box', textAlign: 'right' }}
                  />
        ...
        {deleteConfirm && (
          <div style={{ position: 'fixed', inset: 0, background: '#0a0a0a', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 700, padding: 24 }}>
            <div style={{ background: RAISED, border: `1px solid ${BORDER}`, borderRadius: 16, padding: 24, width: '100%', maxWidth: 340, textAlign: 'center' }}>
              <div style={{ marginBottom: 12, display: 'flex', justifyContent: 'center' }}><Trash size={36} weight="regular" color="#EF4444" /></div>
  ...
  if (view === 'detail' && selected) {
    ...
    return (
      <div style={{ padding: '18px 16px' }}>
        ...
        {emailSentMsg && (
          <div style={{ background: 'rgba(245,158,11,.1)', border: '1px solid rgba(245,158,11,.3)', borderRadius: 10, padding: '12px 14px', marginBottom: 14, color: BLUE, fontSize: 13, fontWeight: 600, display: 'flex', alignItems: 'center', gap: 8 }}>
        ...
        {selected.status === 'pending' && (
          <div style={{ background: RAISED, border: `1px solid ${BORDER}`, borderRadius: 14, padding: '14px', marginBottom: 14 }}>
            ...
            <button
              ...
              style={{
                width: '100%', background: actionLoading ? '#1c1c1e' : GREEN, border: 'none', borderRadius: 12,
            ...
            <button
              ...
              style={{
                width: '100%', background: actionLoading || !rejectReason.trim() ? '#1c1c1e' : RED, border: 'none', borderRadius: 12,
      ...
      <div style={{ padding: '18px 16px' }}>
        ...
        <form onSubmit={handleCreatePCO}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginBottom: 10 }}>
              <div>
                ...
                <select value={pcoReason} onChange={(e) => setPcoReason(e.target.value)} style={inp}>
                  {REASONS.map((r) => <option key={r} value={r} style={{ background: '#141416' }}>{r}</option>)}
                </select>
              </div>
              <div>
          ...
          <div style={{ display: 'flex', gap: 10 }}>
            ...
            <button type="submit" disabled={pcoSaving} style={{ flex: 2, background: pcoSaving ? '#1c1c1e' : PURPLE, border: 'none', borderRadius: 12, padding: '16px', color: pcoSaving ? DIM : '#fff', fontSize: 15, fontWeight: 800, cursor: pcoSaving ? 'wait' : 'pointer' }}>
