'use client';
/**
 * Saguaro Control Systems — Delivery Tracking
...
const STATUS_CONFIG: Record<string, { color: string; label: string; icon: React.ReactNode }> = {
  in_transit: { color: BLUE, label: 'In Transit', icon: <Truck size={12} weight="fill" style={{ verticalAlign: 'middle' }} /> },
  arriving_soon: { color: GOLD, label: 'Arriving Soon', icon: <Alarm size={12} weight="fill" style={{ verticalAlign: 'middle' }} /> },
  delivered: { color: GREEN, label: 'Delivered', icon: <CheckCircle size={12} weight="fill" style={{ verticalAlign: 'middle' }} /> },
  delayed: { color: RED, label: 'Delayed', icon: <Warning size={12} weight="fill" style={{ verticalAlign: 'middle' }} /> },
};
        ...
        <>
          ...
          {deliveries.length > 0 && (
            <div style={{ display: 'flex', gap: 6, marginBottom: 14, overflowX: 'auto', paddingBottom: 4 }}>
              <button onClick={() => setFilterStatus('all')} style={{ ...glass, padding: '6px 12px', fontSize: 12, fontWeight: filterStatus === 'all' ? 700 : 400, color: filterStatus === 'all' ? GOLD : DIM, cursor: 'pointer', whiteSpace: 'nowrap', background: filterStatus === 'all' ? `rgba(${hr(GOLD)},0.15)` : 'rgba(20,20,22,0.7)' }}>
              ...
              {Object.entries(STATUS_CONFIG).map(([key, cfg]) => {
                ...
                return (
                  <button key={key} onClick={() => setFilterStatus(key)} style={{ ...glass, padding: '6px 12px', fontSize: 12, fontWeight: filterStatus === key ? 700 : 400, color: filterStatus === key ? cfg.color : DIM, cursor: 'pointer', whiteSpace: 'nowrap', background: filterStatus === key ? `rgba(${hr(cfg.color)},0.15)` : 'rgba(20,20,22,0.7)' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              {[1, 2, 3].map(i => (
                <div key={i} style={{ ...glass, padding: 16, height: 100, animation: 'pulse 1.5s ease-in-out infinite' }}>
                  <div style={{ background: 'rgba(255,255,255,0.06)', borderRadius: 8, height: 16, width: '60%', marginBottom: 8 }} />
                  <div style={{ background: 'rgba(255,255,255,0.045)', borderRadius: 8, height: 12, width: '40%', marginBottom: 8 }} />
                  <div style={{ background: 'rgba(255,255,255,0.03)', borderRadius: 8, height: 10, width: '30%' }} />
                </div>
              ))}
            </div>
          ) : filteredDeliveries.length === 0 ? (
            <div style={{ ...glass, padding: '40px 24px', textAlign: 'center' }}>
              <div style={{ marginBottom: 12, display: 'flex', justifyContent: 'center' }}><Package size={48} weight="regular" color={DIM} /></div>
              ...
              {filteredDeliveries.map(delivery => {
                ...
                return (
                  <div key={delivery.id} style={{
                    ...glass, padding: 14,
                    border: isOverdue ? `1px solid rgba(${hr(RED)},0.3)` : '1px solid rgba(255,255,255,0.08)',
                  }}>
                    {/* Header */}
                      ...
                      <div style={{ fontSize: 12, color: DIM }}>
                        ...
                        {delivery.contact_phone && (
                          <span style={{ marginLeft: 8 }}>
                            <Phone size={12} weight="fill" style={{ verticalAlign: 'middle' }} /> {delivery.contact_phone}
                          </span>
                        )}
                      </div>
                    </div>
                    ...
                    {delivery.status !== 'delivered' && (
                      <button
                        ...
                        style={{
                          ...
                          background: markingId === delivery.id ? 'rgba(255,255,255,0.06)' : `rgba(${hr(GREEN)},0.1)`,
                      ...
                      >
                        {markingId === delivery.id ? 'Updating...' : <><CheckCircle size={14} weight="fill" /> Mark Arrived</>}
                      </button>
                    )}
                  </div>
                );
