'use client';
/**
 * Saguaro Control Systems — OSHA Incident Tracking
...
const TYPE_ICONS: Record<string, React.ComponentType<any>> = {
  injury: Bandaids, illness: Thermometer, near_miss: Warning, property_damage: Buildings,
  environmental: Leaf, vehicle: Car, fire: Fire, other: Clipboard,
};
function renderTypeIcon(type: string, size: number, color?: string) {
  const Icon = TYPE_ICONS[type] || Clipboard;
...
function IncidentsPage() {
  ...
  const inputStyle: React.CSSProperties = {
    width: '100%', padding: '10px 12px', background: '#1c1c1e', border: `1px solid ${BORDER}`,
  ...
  /* ═══════════════ RENDER ═══════════════ */
  return (
    <div style={{ minHeight: '100vh', background: '#0a0a0a', color: TEXT, fontFamily: "-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif" }}>
      {/* ─── Toast ─── */}
      {toast && (
      ...
      {/* ─── Header ─── */}
      <FieldPageHeader
        title={view === 'dashboard' ? 'Incident Tracking' : view === 'list' ? 'All Incidents' : view === 'create' ? `Report Incident (${step}/3)` : view === 'osha300' ? 'OSHA 300 Log' : selected?.title || 'Incident Detail'}
        subtitle="OSHA Compliance & Safety"
        onBack={() => {
          if (view === 'detail') setView('list');
          else if (view === 'create') { setView('list'); setStep(1); setForm(blankForm()); }
          else if (view === 'osha300') setView('dashboard');
        ...
        icon={scopedFieldIcon('safety', 'ph')}
      />

      <div style={{ padding: 16, maxWidth: 600, margin: '0 auto' }}>
        {loading ? (
          <div style={{ textAlign: 'center', padding: 60, color: DIM }}>
            <div style={{ fontSize: 32, marginBottom: 12 }}><Hourglass size={32} weight="regular" color={DIM} /></div>
            Loading incidents...
          </div>
        ) : (
          <>
              ...
              <>
                  ...
                  {SEVERITIES.map(s => {
                    ...
                    return (
                      <div key={s} style={{ marginBottom: 8 }}>
                        ...
                        <div style={{ height: 6, background: '#1c1c1e', borderRadius: 3 }}>
                ...
                <div style={cardStyle}>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
                    {INCIDENT_TYPES.map(t => (
                      <div key={t} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '6px 10px', background: '#1c1c1e', borderRadius: 8 }}>
                        <span style={{ fontSize: 18, display: 'inline-flex' }}>{renderTypeIcon(t, 18, TYPE_COLORS[t])}</span>
                        <div>
                ...
                <div style={cardStyle}>
                  ...
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
                    {STATUSES.map(s => (
                      <div key={s} style={{ padding: '6px 12px', background: '#1c1c1e', borderRadius: 8, textAlign: 'center' }}>
              ...
              <>
                ...
                {step === 1 && (
                  <div style={cardStyle}>
                    ...
                    {/* GPS */}
                    <div style={{ marginTop: 12, padding: 10, background: '#1c1c1e', borderRadius: 8 }}>
                  ...
                  <div style={cardStyle}>
                    {(form.incident_type === 'injury' || form.incident_type === 'illness') ? (
                      <>
                        ...
                        <div style={{ marginTop: 16, padding: 12, background: isLikelyRecordable ? 'rgba(239,68,68,.12)' : 'rgba(34,197,94,.08)',
                          ...
                          <div style={{ fontSize: 13, fontWeight: 700, color: isLikelyRecordable ? RED : GREEN, marginBottom: 6 }}>
                            {isLikelyRecordable ? <><span style={{ display: 'inline-flex', verticalAlign: 'middle' }}><Warning size={16} weight="fill" color={RED} /></span> Likely OSHA Recordable</> : <><span style={{ display: 'inline-flex', verticalAlign: 'middle' }}><CheckCircle size={16} weight="fill" color={GREEN} /></span> Likely NOT Recordable</>}
                          </div>
                      ...
                      <div style={{ textAlign: 'center', padding: 30, color: DIM }}>
                        <div style={{ fontSize: 32, marginBottom: 8 }}>{renderTypeIcon(form.incident_type, 32, TYPE_COLORS[form.incident_type])}</div>
                ...
                {step === 3 && (
                  <div style={cardStyle}>
                    ...
                    {form.witnesses.length > 0 && (
                      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginTop: 8 }}>
                        {form.witnesses.map((w, i) => (
                          <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 4, background: '#1c1c1e', padding: '4px 10px', borderRadius: 16, fontSize: 12 }}>
                            <span>{w}</span>
                            <button onClick={() => setForm(f => ({ ...f, witnesses: f.witnesses.filter((_, j) => j !== i) }))}
                              style={{ background: 'none', border: 'none', color: RED, cursor: 'pointer', fontSize: 12, padding: 0 }}><X size={12} weight="bold" color={RED} /></button>
                          </div>
                        ))}
                      </div>
                    )}
                    ...
                    <div style={{ padding: 10, background: '#1c1c1e', borderRadius: 8, marginBottom: 8 }}>
                    ...
                    {form.corrective_actions.length > 0 && form.corrective_actions.map((ca, i) => (
                      <div key={ca.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '8px 10px', background: '#1c1c1e', borderRadius: 6, marginBottom: 4 }}>
                        <div>
                        ...
                        <button onClick={() => setForm(f => ({ ...f, corrective_actions: f.corrective_actions.filter((_, j) => j !== i) }))}
                          style={{ background: 'none', border: 'none', color: RED, cursor: 'pointer', fontSize: 14 }}><X size={14} weight="bold" color={RED} /></button>
                      </div>
                    ))}
                    ...
                    <div style={{ display: 'flex', gap: 10, marginTop: 16 }}>
                      <button onClick={() => setStep(2)} style={{ ...btnStyle(BORDER), flex: 1, justifyContent: 'center' }}><ArrowLeft size={16} weight="bold" color="#fff" />Back</button>
                      <button onClick={saveIncident} disabled={saving}
                        style={{ ...btnStyle(GREEN), flex: 1, justifyContent: 'center', opacity: saving ? 0.6 : 1 }}>
                        {saving ? 'Saving...' : <><CheckCircle size={16} weight="bold" color="#fff" />Submit Report</>}
                      </button>
                    </div>
                  </div>
                )}
              ...
              <>
                ...
                <div style={cardStyle}>
                  ...
                  <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginBottom: 10 }}>
                    <span style={badgeStyle(TYPE_COLORS[selected.incident_type])}><span style={{ display: 'inline-flex', verticalAlign: 'middle' }}>{renderTypeIcon(selected.incident_type, 12, '#fff')}</span> {TYPE_LABELS[selected.incident_type]}</span>
                ...
                <div style={cardStyle}>
                  ...
                  {selected.investigation_notes && (
                    <div style={{ marginTop: 10, padding: 10, background: '#1c1c1e', borderRadius: 8 }}>
                ...
                {selected.witnesses && selected.witnesses.length > 0 && (
                  <div style={cardStyle}>
                    ...
                    {selected.witnesses.map((w, i) => (
                      <div key={i} style={{ padding: '6px 10px', background: '#1c1c1e', borderRadius: 6, marginBottom: 4, fontSize: 13, display: 'flex', alignItems: 'center', gap: 8 }}>
                ...
                {selected.corrective_actions.length > 0 && (
                  <div style={cardStyle}>
                    ...
                    {selected.corrective_actions.map(ca => (
                      <div key={ca.id} style={{ padding: 10, background: '#1c1c1e', borderRadius: 8, marginBottom: 8, borderLeft: `3px solid ${ca.status === 'closed' ? GREEN : ca.status === 'in_progress' ? AMBER : RED}` }}>
                  ...
                  <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                    {(STATUS_FLOW[selected.status] || []).map(nextStatus => (
                      <button key={nextStatus} onClick={() => updateStatus(selected.id, nextStatus)}
                        ...
                        <ArrowRight size={16} weight="bold" color="#fff" />{STATUS_LABELS[nextStatus]}
                      </button>
                    ))}
                  </div>
                </div>
...
export default function IncidentsPageWrapper() {
  return (
    <Suspense fallback={<div style={{ minHeight: '100vh', background: '#0a0a0a', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#CBD5E1' }}>Loading...</div>}>
      <IncidentsPage />
    </Suspense>
  );
}
