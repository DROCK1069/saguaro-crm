      ...
      <div style={{ padding: '16px 20px', maxWidth: 1200, margin: '0 auto' }}>
        ...
        {view === 'list' && (
          <>
            ...
            {/* Batch actions */}
            {selected.size > 0 && (
              <div style={{ ...cardStyle, display: 'flex', alignItems: 'center', gap: 12, flexWrap: 'wrap', background: '#141416' }}>
            ...
            {sheetMode === 'menu' && (
              <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                {[
                  ...
                  { label: 'Copy Amount', icon: <Copy size={20} />, action: () => { handleFieldCopy(actionSheet.amount, actionSheet.invoiceId); } },
                  { label: 'Void Invoice', icon: <Trash size={20} />, action: () => { closeActionSheet(); setDeleteConfirm(actionSheet.invoiceId); } },
                ].map(item => (
...
export default function InvoicesPageWrapper() {
  return (
    <Suspense fallback={<div style={{ padding: 40, textAlign: 'center', color: '#CBD5E1', background: '#0a0a0a', minHeight: '100vh' }}>Loading...</div>}>
      <InvoicesPage />
    </Suspense>
  );
}
