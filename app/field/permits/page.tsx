'use client';
/**
 * Saguaro Control Systems — Construction Permit Tracking
...
const exportPDF = (title: string, content: string) => {
  ...
  pw.document.write(`<!DOCTYPE html><html><head><title>${title}</title>
    ...
    <div class="footer">Saguaro Control Systems &mdash; Permit Report &mdash; Generated ${new Date().toLocaleString()}</div>
    </body></html>`);
  pw.document.close();
  pw.focus();
  pw.print();
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (!res.ok) { console.error('permits request failed', res.status); throw new Error('request failed'); }
        const data = await res.json();
        if (!cancelled) setPermits(data.permits || []);
      } catch (e: unknown) {
        console.error(e); if (!cancelled) setError(humanError(e, 'Failed to load permits. Please try again.'));
      } finally {
  ...
  return (
    <Suspense fallback={
      <div style={{ minHeight: '100vh', background: '#0a0a0a', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        <span style={{ color: '#CBD5E1', fontSize: 16 }}>Loading...</span>
      </div>
    }>
      <PermitsPageInner />
    </Suspense>
