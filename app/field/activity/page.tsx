'use client';
/**
 * Saguaro Control Systems — Activity Log / Audit Trail
...
import { Plus, PencilSimple, X, ArrowRight, ChatCircle, CheckCircle, Circle, Clipboard } from '@phosphor-icons/react';
...
const ACTION_META: Record<string, { icon: React.ElementType; color: string; label: string }> = {
  ...
  updated:       { icon: PencilSimple, color: BLUE,  label: 'Updated' },
  deleted:       { icon: X,            color: RED,   label: 'Deleted' },
  status_change: { icon: ArrowRight,   color: AMBER, label: 'Status Change' },
  comment:       { icon: ChatCircle,   color: BLUE,  label: 'Comment' },
  approval:      { icon: CheckCircle,  color: GREEN, label: 'Approval' },
};

function getActionMeta(action: string) {
  return ACTION_META[action] || { icon: Circle, color: DIM, label: action };
}
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#0a0a0a', fontFamily: '-apple-system, BlinkMacSystemFont, sans-serif', color: TEXT, paddingBottom: 100 }}>
      ...
      <div style={{ background: RAISED, borderBottom: `1px solid ${BORDER}`, position: 'sticky', top: 0, zIndex: 50 }}>
        ...
        {/* Filter Panel */}
        {showFilters && (
          <div style={{ marginTop: 12, padding: 14, background: '#1c1c1e', borderRadius: 12, border: `1px solid ${BORDER}` }}>
      ...
      {!loading && entries.length === 0 && (
        <div style={{ textAlign: 'center', padding: 40, color: DIM }}>
          <div style={{ fontSize: 40, marginBottom: 12, display: 'flex', justifyContent: 'center' }}><Clipboard size={40} weight="regular" color={DIM} /></div>
                    ...
                    <div style={{ ...card, flex: 1, padding: '12px 14px', marginBottom: 8 }}>
                      ...
                      >
                        ...
                        <div style={{
                          ...
                          <ActionIcon size={14} weight="fill" color={actionMeta.color} />
                        </div>
                      ...
                      {hasChanges && (
                        <div style={{ marginTop: 8 }}>
                          ...
                          {isExpanded && (
                            <div style={{ marginTop: 8, padding: 10, background: '#1c1c1e', borderRadius: 8, border: `1px solid ${BORDER}` }}>
