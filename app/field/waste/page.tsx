'use client';
/**
 * Saguaro Control Systems — Waste & Recycling Tracking
...
const WASTE_ICONS: Record<WasteType, React.ElementType> = {
  concrete: Cube, wood: TreeEvergreen, metal: Nut, drywall: SquareHalf,
  roofing: House, masonry: Wall, plastic: Recycle, cardboard: Package,
  mixed_c_d: Stack, hazardous: Radioactive, soil: Mountains, asphalt: Path,
  glass: Square, insulation: Spiral, other: ClipboardText,
};
...
function WasteTrackingPage() {
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#0a0a0a', color: TEXT, fontFamily: '-apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif' }}>
      {/* Header */}
      <div style={{ background: RAISED, borderBottom: `1px solid ${BORDER}`, padding: '0 16px 12px', position: 'sticky', top: 0, zIndex: 100 }}>
        <FieldPageHeader
          title="Waste Tracking"
          icon={scopedFieldIcon('deliveries', 'ph')}
          actions={
            <>
                ...
                <div key={r.id} style={{ ...cardStyle, cursor: 'pointer' }} onClick={() => startEdit(r)}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                    <div style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
                      <span style={{ display: 'inline-flex' }}>{React.createElement(WASTE_ICONS[r.waste_type] || ClipboardText, { size: 24, weight: 'fill', color: DIM })}</span>
                      <div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 6, marginBottom: 12 }}>
                {WASTE_TYPES.map(t => (
                  <button key={t} onClick={() => setFType(t)}
                    ...
                    {React.createElement(WASTE_ICONS[t], { size: 14, weight: 'fill', style: { verticalAlign: 'middle' } })} {fmt(t)}
                  </button>
                ))}
              </div>
          ...
          <>
            ...
            <div style={{ ...cardStyle, borderColor: compliance.issues > 0 ? RED : GREEN }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 }}>
                <span style={{ display: 'inline-flex' }}>{compliance.issues === 0 ? <CheckCircle size={20} weight="fill" color={GREEN} /> : <Warning size={20} weight="fill" color={AMBER} />}</span>
                  ...
                  <div style={{ marginTop: 12, borderTop: `1px solid ${BORDER}`, paddingTop: 12 }}>
                    {costByType.map(([type, cost]) => (
                      <div key={type} style={{ display: 'flex', justifyContent: 'space-between', padding: '6px 0', borderBottom: `1px solid ${BORDER}` }}>
                        <span style={{ color: TEXT, fontSize: 13 }}>{React.createElement(WASTE_ICONS[type as WasteType], { size: 14, weight: 'fill', style: { verticalAlign: 'middle' } })} {fmt(type)}</span>
            ...
            {hazRecords.length === 0 ? (
              ...
              hazRecords.map(r => (
                <div key={r.id} style={{ ...cardStyle, borderColor: r.manifest_number ? '#A855F7' : RED, cursor: 'pointer' }} onClick={() => startEdit(r)}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                    <div>
                      <div style={{ fontWeight: 700, color: TEXT }}><Radioactive size={16} weight="fill" style={{ verticalAlign: 'middle' }} /> {fmt(r.waste_type)}</div>
            ...
            {leedTargets.map((target, i) => {
              ...
              return (
                <div key={i} style={{ ...cardStyle, borderColor: met ? GREEN : BORDER }}>
                  ...
                  <div style={{ background: '#1c1c1e', borderRadius: 6, height: 20, overflow: 'hidden', position: 'relative' }}>
              ...
              {WASTE_TYPES.map(t => {
                ...
                return (
                  <div key={t} style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 }}>
                    <span style={{ width: 24, display: 'inline-flex', justifyContent: 'center' }}>{React.createElement(WASTE_ICONS[t], { size: 16, weight: 'fill', color: DIM })}</span>
                    <div style={{ flex: 1 }}>
                      ...
                      <div style={{ background: '#1c1c1e', borderRadius: 4, height: 8, overflow: 'hidden' }}>
