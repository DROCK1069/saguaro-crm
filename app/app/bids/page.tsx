...
const GOLD='#F59E0B',DARK='#0d1117',RAISED='#0F172A',BORDER='rgba(255,255,255,0.12)',DIM='#CBD5E1',TEXT='#FFFFFF',RED='#ef4444',GREEN='#3dd68c';
    ...
    <div style={{padding:'24px 28px',maxWidth:1300,margin:'0 auto'}}>
      <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',marginBottom:24}}>
        ...
        <div style={{display:'flex',gap:10}}>
          ...
          <button style={{padding:'9px 18px',background:`linear-gradient(135deg,${GOLD},#FBBF24)`,border:'none',borderRadius:8,color:'#1C1C1E',fontSize:13,fontWeight:800,cursor:'pointer'}} onClick={()=>setShowScore(true)}>+ Score a Bid</button>
        </div>
      </div>
      ...
      {tab==='pipeline'&&<div>
        {pipelineLoading ? (
          ...
          <div style={{textAlign:'center' as const,padding:60,color:DIM}}>
            ...
            <button onClick={()=>router.push('/app/projects')} style={{padding:'10px 22px',background:`linear-gradient(135deg,${GOLD},#FBBF24)`,border:'none',borderRadius:8,color:'#1C1C1E',fontSize:13,fontWeight:800,cursor:'pointer'}}>Go to Projects</button>
          </div>
        ) : (
            ...
            <tbody>{opportunities.map(op=>(
              <tr key={op.id} style={{borderBottom:`1px solid rgba(229,229,234,.5)`}}>
                ...
                <td style={{padding:'12px 14px'}}>
                  <button onClick={()=>router.push(`/app/projects/${op.project_id}/bid-packages/${op.id}`)} style={{background:`linear-gradient(135deg,${GOLD},#FBBF24)`,border:'none',borderRadius:5,color:'#1C1C1E',fontSize:11,padding:'4px 10px',fontWeight:700,cursor:'pointer'}}>View →</button>
                </td>
              </tr>
            ))}</tbody>
          </table>
      ...
      {tab==='active'&&<div style={{textAlign:'center' as const,padding:60,color:DIM}}>
        ...
        <button
          ...
          style={{padding:'10px 22px',background:`linear-gradient(135deg,${GOLD},#FBBF24)`,border:'none',borderRadius:8,color:'#1C1C1E',fontSize:13,fontWeight:800,cursor:'pointer'}}
      ...
      {/* ── Score a Bid Modal ─────────────────────────────────────────────────── */}
      {showScore&&<div style={{position:'fixed',inset:0,background:'#0d1117',backdropFilter:'blur(4px)',display:'flex',alignItems:'center',justifyContent:'center',zIndex:1000}}>
          ...
          {scoreResult ? (
            <div>
              ...
              <div style={{display:'flex',gap:10}}>
                ...
                <button onClick={()=>setTab('pipeline')} style={{flex:1,padding:'9px 16px',background:`linear-gradient(135deg,${GOLD},#FBBF24)`,border:'none',borderRadius:7,color:'#1C1C1E',fontSize:13,fontWeight:800,cursor:'pointer'}}>View Pipeline</button>
              </div>
            </div>
          ) : (
            <div>
              <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:14,marginBottom:14}}>
                {([['Project Name','projectName','e.g. Scottsdale Office Buildout'],['Bid Amount ($)','bidAmount','e.g. 2,500,000'],['Target Margin (%)','margin','e.g. 15'],['Trade Type','tradeType','e.g. Commercial']]).map(([lbl,key,ph])=>(
                  <div key={key}>
                    ...
                    <input value={(scoreForm as any)[key]} onChange={e=>setScoreForm(f=>({...f,[key]:e.target.value}))} placeholder={ph} style={{width:'100%',padding:'8px 12px',background:'#16243A',border:`1px solid ${BORDER}`,borderRadius:7,color:TEXT,fontSize:13,outline:'none',boxSizing:'border-box' as const}}/>
                  </div>
                ))}
              </div>
              <div style={{marginBottom:16}}>
                ...
                <textarea value={scoreForm.notes} onChange={e=>setScoreForm(f=>({...f,notes:e.target.value}))} rows={3} placeholder="Any context for the AI…" style={{width:'100%',padding:'8px 12px',background:'#16243A',border:`1px solid ${BORDER}`,borderRadius:7,color:TEXT,fontSize:13,outline:'none',resize:'vertical' as const,boxSizing:'border-box' as const}}/>
              </div>
              <div style={{display:'flex',gap:10}}>
