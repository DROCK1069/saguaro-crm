...
export default function ContractsPage(){
  ...
  const rows=[['C-001','Desert Electric LLC','Electrical — complete scope','$185,000','2026-01-10','Active'],['C-002','AZ Plumbing & Mech','Plumbing & mechanical','$122,000','2026-01-10','Active'],['C-003','Southwest Concrete','Concrete work','$98,500','2026-01-12','Active']];
  ...
  return <div>
    <div style={{padding:'16px 24px',borderBottom:'1px solid '+BORDER,display:'flex',alignItems:'center',justifyContent:'space-between',background:'#0d1117'}}>
      <div><h2 style={{margin:0,fontSize:20,fontWeight:800,color:TEXT}}>📋 Contracts</h2><div style={{fontSize:12,color:DIM,marginTop:3}}>Subcontractor and vendor contracts</div></div>
    ...
    {showNew&&<div style={{margin:24,background:RAISED,border:'1px solid rgba(212,160,23,.3)',borderRadius:10,padding:24}}>
      <div style={{fontSize:13,fontWeight:700,color:TEXT,marginBottom:12}}>Add New Contracts</div>
    ...
    <div style={{padding:'0 24px 24px'}}>
      <table style={{width:'100%',borderCollapse:'collapse',fontSize:13,marginTop:24}}>
        <thead><tr style={{background:'#0a1117'}}>
          {['Contract #','Party','Scope','Value','Signed','Status'].map(h=><th key={h} style={{padding:'10px 14px',textAlign:'left',fontSize:11,fontWeight:700,textTransform:'uppercase',letterSpacing:.5,color:DIM,borderBottom:'1px solid '+BORDER}}>{h}</th>)}
        </tr></thead>
