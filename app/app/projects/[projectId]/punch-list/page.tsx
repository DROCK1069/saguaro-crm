...
export default function PunchListPage(){
  ...
  const rows=[['P-001','Touch up paint — master bedroom','Paint crew','2026-09-15','Low','Open'],['P-002','Adjust door hardware — unit 3','Finish crew','2026-09-10','Medium','Open'],['P-003','Install missing outlet cover — kitchen','Electrician','2026-09-08','High','Open']];
  ...
  return <div>
    <div style={{padding:'16px 24px',borderBottom:'1px solid '+BORDER,display:'flex',alignItems:'center',justifyContent:'space-between',background:'#0d1117'}}>
      <div><h2 style={{margin:0,fontSize:20,fontWeight:800,color:TEXT}}>✅ Punch List</h2><div style={{fontSize:12,color:DIM,marginTop:3}}>Punch list items before substantial completion</div></div>
    ...
    {showNew&&<div style={{margin:24,background:RAISED,border:'1px solid rgba(212,160,23,.3)',borderRadius:10,padding:24}}>
      <div style={{fontSize:13,fontWeight:700,color:TEXT,marginBottom:12}}>Add New Punch List</div>
    ...
    <div style={{padding:'0 24px 24px'}}>
      <table style={{width:'100%',borderCollapse:'collapse',fontSize:13,marginTop:24}}>
        <thead><tr style={{background:'#0a1117'}}>
          {['#','Description','Assigned To','Due','Priority','Status'].map(h=><th key={h} style={{padding:'10px 14px',textAlign:'left',fontSize:11,fontWeight:700,textTransform:'uppercase',letterSpacing:.5,color:DIM,borderBottom:'1px solid '+BORDER}}>{h}</th>)}
        </tr></thead>
