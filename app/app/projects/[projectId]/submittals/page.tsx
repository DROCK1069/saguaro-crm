...
export default function SubmittalsPage(){
  ...
  const rows=[['S-001','03 31 00','Concrete mix design','Southwest Concrete','2026-01-20','Approved'],['S-002','06 10 00','Framing lumber grade certification','Framing crew','2026-01-25','Approved'],['S-003','26 05 00','Electrical panel schedule','Desert Electric','2026-02-05','In Review']];
  ...
  return <div>
    <div style={{padding:'16px 24px',borderBottom:'1px solid '+BORDER,display:'flex',alignItems:'center',justifyContent:'space-between',background:'#0d1117'}}>
      <div><h2 style={{margin:0,fontSize:20,fontWeight:800,color:TEXT}}>📬 Submittals</h2><div style={{fontSize:12,color:DIM,marginTop:3}}>Shop drawings, product data, and submittals</div></div>
    ...
    {showNew&&<div style={{margin:24,background:RAISED,border:'1px solid rgba(212,160,23,.3)',borderRadius:10,padding:24}}>
      <div style={{fontSize:13,fontWeight:700,color:TEXT,marginBottom:12}}>Add New Submittals</div>
    ...
    <div style={{padding:'0 24px 24px'}}>
      <table style={{width:'100%',borderCollapse:'collapse',fontSize:13,marginTop:24}}>
        <thead><tr style={{background:'#0a1117'}}>
          {['#','Spec Section','Description','Submitted By','Date','Status'].map(h=><th key={h} style={{padding:'10px 14px',textAlign:'left',fontSize:11,fontWeight:700,textTransform:'uppercase',letterSpacing:.5,color:DIM,borderBottom:'1px solid '+BORDER}}>{h}</th>)}
        </tr></thead>
