...
export default function PurchaseOrdersPage(){
  ...
  const rows=[['PO-001','Desert Steel Supply','Structural steel','$28,400','2026-01-20','Delivered'],['PO-002','Phoenix Lumber Co','Framing lumber','$15,880','2026-02-01','Delivered'],['PO-003','Window World AZ','Windows — 18 units','$6,930','2026-02-10','In Transit']];
  ...
  return <div>
    <div style={{padding:'16px 24px',borderBottom:'1px solid '+BORDER,display:'flex',alignItems:'center',justifyContent:'space-between',background:'#0d1117'}}>
      <div><h2 style={{margin:0,fontSize:20,fontWeight:800,color:TEXT}}>🛒 Purchase Orders</h2><div style={{fontSize:12,color:DIM,marginTop:3}}>Purchase orders to suppliers and vendors</div></div>
    ...
    {showNew&&<div style={{margin:24,background:RAISED,border:'1px solid rgba(212,160,23,.3)',borderRadius:10,padding:24}}>
      <div style={{fontSize:13,fontWeight:700,color:TEXT,marginBottom:12}}>Add New Purchase Orders</div>
    ...
    <div style={{padding:'0 24px 24px'}}>
      <table style={{width:'100%',borderCollapse:'collapse',fontSize:13,marginTop:24}}>
        <thead><tr style={{background:'#0a1117'}}>
          {['PO #','Vendor','Description','Amount','Issued','Status'].map(h=><th key={h} style={{padding:'10px 14px',textAlign:'left',fontSize:11,fontWeight:700,textTransform:'uppercase',letterSpacing:.5,color:DIM,borderBottom:'1px solid '+BORDER}}>{h}</th>)}
        </tr></thead>
