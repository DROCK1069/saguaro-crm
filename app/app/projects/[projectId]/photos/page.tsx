...
export default function PhotosPage(){
  ...
  const rows=[['2026-01-15','Chad D.','Site prep — foundation excavation','Foundation area','foundation,progress'],['2026-02-01','Chad D.','Framing — Level 1 walls','Building perimeter','framing,progress'],['2026-02-28','Crew Lead','Roof framing complete','Roof','framing,roof,milestone']];
  ...
  return <div>
    <div style={{padding:'16px 24px',borderBottom:'1px solid '+BORDER,display:'flex',alignItems:'center',justifyContent:'space-between',background:'#0d1117'}}>
      <div><h2 style={{margin:0,fontSize:20,fontWeight:800,color:TEXT}}>📸 Photos</h2><div style={{fontSize:12,color:DIM,marginTop:3}}>Site photos and progress documentation</div></div>
    ...
    {showNew&&<div style={{margin:24,background:RAISED,border:'1px solid rgba(212,160,23,.3)',borderRadius:10,padding:24}}>
      <div style={{fontSize:13,fontWeight:700,color:TEXT,marginBottom:12}}>Add New Photos</div>
    ...
    <div style={{padding:'0 24px 24px'}}>
      <table style={{width:'100%',borderCollapse:'collapse',fontSize:13,marginTop:24}}>
        <thead><tr style={{background:'#0a1117'}}>
          {['Date','Taken By','Description','Location','Tags'].map(h=><th key={h} style={{padding:'10px 14px',textAlign:'left',fontSize:11,fontWeight:700,textTransform:'uppercase',letterSpacing:.5,color:DIM,borderBottom:'1px solid '+BORDER}}>{h}</th>)}
        </tr></thead>
