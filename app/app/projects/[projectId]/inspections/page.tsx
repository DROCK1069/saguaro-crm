...
export default function InspectionsPage(){
  ...
  const rows=[['Foundation','2026-02-10','John Smith — City','Passed','No issues','Complete'],['Framing','2026-02-28','John Smith — City','Passed','Minor correction — corrected on site','Complete'],['Rough Electrical','2026-03-05','Mary Jones — City','Pending','-','Scheduled']];
  ...
  return <div>
    <div style={{padding:'16px 24px',borderBottom:'1px solid '+BORDER,display:'flex',alignItems:'center',justifyContent:'space-between',background:'#0d1117'}}>
      <div><h2 style={{margin:0,fontSize:20,fontWeight:800,color:TEXT}}>🔍 Inspections</h2><div style={{fontSize:12,color:DIM,marginTop:3}}>Building inspections and approvals</div></div>
    ...
    {showNew&&<div style={{margin:24,background:RAISED,border:'1px solid rgba(212,160,23,.3)',borderRadius:10,padding:24}}>
      <div style={{fontSize:13,fontWeight:700,color:TEXT,marginBottom:12}}>Add New Inspections</div>
    ...
    <div style={{padding:'0 24px 24px'}}>
      <table style={{width:'100%',borderCollapse:'collapse',fontSize:13,marginTop:24}}>
        <thead><tr style={{background:'#0a1117'}}>
          {['Type','Date','Inspector','Result','Notes','Status'].map(h=><th key={h} style={{padding:'10px 14px',textAlign:'left',fontSize:11,fontWeight:700,textTransform:'uppercase',letterSpacing:.5,color:DIM,borderBottom:'1px solid '+BORDER}}>{h}</th>)}
        </tr></thead>
