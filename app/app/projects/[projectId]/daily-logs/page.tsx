...
export default function DailyLogsPage(){
  ...
  const rows=[['2026-03-10','Mike T.','Clear, 72°F','14','Framing Level 2 — east wing walls','None'],['2026-03-09','Mike T.','Partly cloudy, 68°F','12','Framing Level 2 — started east wing','None'],['2026-03-08','Mike T.','Clear, 75°F','14','Framing Level 2 — north wall complete','RFI submitted for window location']];
  ...
  return <div>
    <div style={{padding:'16px 24px',borderBottom:'1px solid '+BORDER,display:'flex',alignItems:'center',justifyContent:'space-between',background:'#0d1117'}}>
      <div><h2 style={{margin:0,fontSize:20,fontWeight:800,color:TEXT}}>📝 Daily Logs</h2><div style={{fontSize:12,color:DIM,marginTop:3}}>Daily construction logs and reports</div></div>
    ...
    {showNew&&<div style={{margin:24,background:RAISED,border:'1px solid rgba(212,160,23,.3)',borderRadius:10,padding:24}}>
      <div style={{fontSize:13,fontWeight:700,color:TEXT,marginBottom:12}}>Add New Daily Logs</div>
    ...
    <div style={{padding:'0 24px 24px'}}>
      <table style={{width:'100%',borderCollapse:'collapse',fontSize:13,marginTop:24}}>
        <thead><tr style={{background:'#0a1117'}}>
          {['Date','Superintendent','Weather','Crew Count','Work Performed','Issues'].map(h=><th key={h} style={{padding:'10px 14px',textAlign:'left',fontSize:11,fontWeight:700,textTransform:'uppercase',letterSpacing:.5,color:DIM,borderBottom:'1px solid '+BORDER}}>{h}</th>)}
        </tr></thead>
