...
export default function DrawingsPage(){
  ...
  const rows=[['A1.0','Site Plan','Rev 2','2026-01-15','Architectural','Current'],['A2.0','Floor Plan — Level 1','Rev 3','2026-02-01','Architectural','Current'],['S1.0','Foundation Plan','Rev 1','2026-01-20','Structural','Current'],['E1.0','Electrical Site Plan','Rev 1','2026-01-25','Electrical','Current']];
  ...
  return <div>
    <div style={{padding:'16px 24px',borderBottom:'1px solid '+BORDER,display:'flex',alignItems:'center',justifyContent:'space-between',background:'#0d1117'}}>
      <div><h2 style={{margin:0,fontSize:20,fontWeight:800,color:TEXT}}>📐 Drawings</h2><div style={{fontSize:12,color:DIM,marginTop:3}}>Architectural and engineering drawings</div></div>
    ...
    {showNew&&<div style={{margin:24,background:RAISED,border:'1px solid rgba(212,160,23,.3)',borderRadius:10,padding:24}}>
      <div style={{fontSize:13,fontWeight:700,color:TEXT,marginBottom:12}}>Add New Drawings</div>
    ...
    <div style={{padding:'0 24px 24px'}}>
      <table style={{width:'100%',borderCollapse:'collapse',fontSize:13,marginTop:24}}>
        <thead><tr style={{background:'#0a1117'}}>
          {['Sheet','Title','Revision','Date','Discipline','Status'].map(h=><th key={h} style={{padding:'10px 14px',textAlign:'left',fontSize:11,fontWeight:700,textTransform:'uppercase',letterSpacing:.5,color:DIM,borderBottom:'1px solid '+BORDER}}>{h}</th>)}
        </tr></thead>
