...
export default function TodosPage(){
  ...
  const rows=[['Submit pay application #3','Chad D.','2026-04-01','High','Open'],['Request updated COI from Mesa Roofing','Admin','2026-03-25','High','Open'],['Review and approve framing submittals','Chad D.','2026-03-20','Medium','Open'],['Schedule rough MEP inspection','Admin','2026-04-05','Medium','Open']];
  ...
  return <div>
    <div style={{padding:'16px 24px',borderBottom:'1px solid '+BORDER,display:'flex',alignItems:'center',justifyContent:'space-between',background:'#0d1117'}}>
      <div><h2 style={{margin:0,fontSize:20,fontWeight:800,color:TEXT}}>✔️ Todos</h2><div style={{fontSize:12,color:DIM,marginTop:3}}>Project tasks and action items</div></div>
    ...
    {showNew&&<div style={{margin:24,background:RAISED,border:'1px solid rgba(212,160,23,.3)',borderRadius:10,padding:24}}>
      <div style={{fontSize:13,fontWeight:700,color:TEXT,marginBottom:12}}>Add New Todos</div>
    ...
    <div style={{padding:'0 24px 24px'}}>
      <table style={{width:'100%',borderCollapse:'collapse',fontSize:13,marginTop:24}}>
        <thead><tr style={{background:'#0a1117'}}>
          {['Task','Assigned To','Due Date','Priority','Status'].map(h=><th key={h} style={{padding:'10px 14px',textAlign:'left',fontSize:11,fontWeight:700,textTransform:'uppercase',letterSpacing:.5,color:DIM,borderBottom:'1px solid '+BORDER}}>{h}</th>)}
        </tr></thead>
