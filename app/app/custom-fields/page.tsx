      ...
      {options.map((opt, i) => (
        ...
        >
          ...
          <button
            ...
            disabled={i === 0}
          >
            <span style={{ display: 'inline-flex', verticalAlign: 'middle' }}><CaretUp size={12} weight="regular" color={DIM} /></span>
          </button>
          <button
            style={{ ...btn('transparent', DIM), padding: '2px 6px', fontSize: 11 }}
            onClick={() => moveOption(i, 1)}
          ...
          >
            <span style={{ display: 'inline-flex', verticalAlign: 'middle' }}><CaretDown size={12} weight="regular" color={DIM} /></span>
          </button>
          <button
...
export default function CustomFieldsPage() {
  ...
  const loadFields = useCallback(async () => {
    ...
    try {
      ...
      if (!r.ok) { console.error('load custom fields failed', r.status); throw new Error('request failed'); }
      const data = await r.json();
      setFields(data.fields || data || []);
    } catch (err: unknown) {
      console.error(err);
      setError(humanError(err, 'Failed to load custom fields. Please try again.'));
      setFields([]);
    } finally {
      setLoading(false);
    }
