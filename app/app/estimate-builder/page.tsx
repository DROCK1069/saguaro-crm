...
export default function EstimateBuilderPage() {
  ...
  const openTakeoffPicker = useCallback(() => {
    ...
    fetch('/api/takeoff-projects/list')
      ...
      .catch((e: unknown) => { console.error(e); setImportError(humanError(e, 'Failed to load takeoff projects.')); })
  ...
  const importFromTakeoff = useCallback(async (takeoffId: string) => {
    ...
    try {
      ...
      console.error(e); setImportError(humanError(e, 'Failed to import takeoff line items.'));
    } finally {
      setSaving(false);
    }
  }, []);
                            ...
                            <td style={{ ...tdStyle, fontWeight: 600, color: GOLD, position: 'relative' as const }}>
                              {deleteId === item.id ? (
                                ...
                                <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
                                  <input value={editVal} onChange={e => setEditVal(e.target.value)} type="number" autoFocus onKeyDown={e => { if (e.key === 'Enter') handleEditLine(div.code, item.id); if (e.key === 'Escape') setEditId(null); }} style={{ width: 100, padding: '4px 8px', background: '#1c1c1e', border: `1px solid ${GOLD}`, borderRadius: 5, color: TEXT, fontSize: 12, outline: 'none', textAlign: 'right' }} />
