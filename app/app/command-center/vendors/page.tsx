...
import { C, font, fmtDate, fmtMoneyShort, useFranchiseGate, GateLoading, PageHeader, Tile, SevBadge, EmptyState } from '@/components/franchise/kit';
...
export default function VendorsPage() {
  ...
  return (
    <div style={{ padding: '28px 24px 60px', maxWidth: 1200, margin: '0 auto', fontFamily: font, color: C.text }}>
      <PageHeader title="Vendor & Install Schedule" subtitle="Every vendor delivery and equipment install across all sites, on one timeline — coordinate Full-Swing, AV, and long-lead so nothing lands late." />

      <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap', marginBottom: 20 }}>
        <Tile label="Scheduled Items" value={summary.total} />
        <Tile label="Next 30 Days" value={summary.upcoming} color={C.blue} />
        <Tile label="At Risk" value={summary.atRisk} color={summary.atRisk ? C.red : C.green} />
        <Tile label="Delivered" value={summary.delivered} color={C.green} />
        <Tile label="Committed Value" value={fmtMoneyShort(summary.value)} />
      </div>

      {loading ? <div style={{ color: C.dim, padding: 40, textAlign: 'center' }}>Loading…</div>
      : rows.length === 0 ? (
        <EmptyState icon="🚚" title="No vendor deliveries scheduled" body="Long-lead procurement and equipment installs (Full-Swing, AV, switchgear) appear here as a coordinated cross-site timeline." />
      ) : (
            ...
            return (
              <div key={b.key}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 10 }}>
                  ...
                  <span style={{ fontSize: 13, fontWeight: 800, textTransform: 'uppercase', letterSpacing: 0.5 }}>{b.label}</span>
                  <span style={{ fontSize: 12, color: C.dim }}>{list.length}</span>
                </div>
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(340px, 1fr))', gap: 12 }}>
                  {list.map(({ it, h, bd }) => (
                    <div key={it.id} style={{ background: C.card, border: `1px solid ${C.border}`, borderLeft: `4px solid ${h.severity === 'red' ? C.red : h.severity === 'yellow' ? C.yellow : C.green}`, borderRadius: 12, padding: '13px 15px', boxShadow: '0 1px 2px rgba(0,0,0,0.04)' }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', gap: 8, alignItems: 'flex-start' }}>
                        <div style={{ minWidth: 0 }}>
                          <div style={{ fontSize: 14, fontWeight: 800 }}>{isInstall(it.item_description) ? '🔧 ' : '📦 '}{it.item_description}</div>
