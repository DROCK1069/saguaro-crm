      ...
      {loading ? (
        ...
        <div style={{ display: 'flex', gap: 14, overflowX: 'auto', paddingBottom: 12, alignItems: 'flex-start' }}>
          {ROLLOUT_STAGES.map((stage) => {
            ...
            return (
              <div key={stage.id} style={{ flex: '0 0 288px', width: 288, background: '#16243A', border: `1px solid ${C.border}`, borderRadius: 14, padding: 10 }}>
                  ...
                  {items.length === 0 ? (
                    ...
                    return (
                      <div key={p.id} style={{ background: C.card, border: `1px solid ${C.border}`, borderLeft: `3px solid ${stage.color}`, borderRadius: 10, padding: '10px 11px', boxShadow: '0 1px 2px rgba(0,0,0,0.04)' }}>
                        ...
                        {/* progress */}
                        <div style={{ height: 4, borderRadius: 2, background: '#16243A', overflow: 'hidden', margin: '8px 0 9px' }}>
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                          <button onClick={() => shift(p, -1)} disabled={busy[p.id] || STAGE_META[stageOf(p)].order === 0}
                            style={arrowStyle(busy[p.id] || STAGE_META[stageOf(p)].order === 0)}>←</button>
                          <button onClick={() => shift(p, 1)} disabled={busy[p.id] || STAGE_META[stageOf(p)].order === STAGE_IDS.length - 1}
                            style={arrowStyle(busy[p.id] || STAGE_META[stageOf(p)].order === STAGE_IDS.length - 1)}>Advance →</button>
                        </div>
                      </div>
                    );
                  })}
...
function arrowStyle(disabled: boolean): React.CSSProperties {
  return {
    ...
    border: `1px solid ${C.border}`, background: disabled ? '#0d1117' : '#16243A', color: disabled ? C.faint : C.text,
  };
}
  ...
  return (
    <div onClick={onClose} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.4)', zIndex: 1000, display: 'flex', alignItems: 'flex-start', justifyContent: 'center', padding: '8vh 16px', overflow: 'auto' }}>
      <div onClick={(e) => e.stopPropagation()} style={{ background: C.card, borderRadius: 16, padding: 24, width: 'min(560px, 100%)', boxShadow: '0 24px 80px rgba(0,0,0,0.3)', fontFamily: font }}>
        {done ? (
          <div>
            <div style={{ fontSize: 20, fontWeight: 900, marginBottom: 6 }}>🌵 {f.name} launched</div>
            ...
            <div style={{ display: 'flex', gap: 10 }}>
              ...
              <button onClick={onClose} style={{ padding: '10px 18px', borderRadius: 10, border: `1px solid ${C.border}`, background: '#16243A', color: C.dim, fontWeight: 700, cursor: 'pointer' }}>Done</button>
            </div>
          </div>
        ) : (
          <>
