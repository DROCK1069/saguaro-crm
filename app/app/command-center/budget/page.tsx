...
export default function BudgetPage() {
  ...
  return (
    <div style={{ padding: '28px 24px 60px', maxWidth: 1200, margin: '0 auto', fontFamily: font, color: C.text }}>
      ...
      : tab === 'budget' ? (
        (budgets as any[]).length === 0 ? <EmptyState icon="💰" title="No budgets yet" body="Site budgets roll up here — original, approved & pending change orders, committed, actual, and forecast variance." />
        : <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(360px, 1fr))', gap: 14 }}>
          {(budgets as any[]).map((b) => (
            <div key={b.project_id} style={{ background: C.card, border: `1px solid ${C.border}`, borderLeft: `4px solid ${b.variance < 0 ? C.red : C.green}`, borderRadius: 14, padding: '16px 18px', boxShadow: '0 1px 2px rgba(0,0,0,0.05)' }}>
              ...
              <div style={{ height: 6, borderRadius: 3, background: '#16243A', overflow: 'hidden', margin: '10px 0 12px' }}>
        ...
        (changeOrders as any[]).length === 0 ? <EmptyState icon="📝" title="No change orders" body="Every change routes the same way: Issued → GC Pricing → PM Review → Architect Review → Owner Approval → Executed → Schedule Updated → Budget Updated."
              ...
              <div key={co.id} style={{ background: C.card, border: `1px solid ${C.border}`, borderLeft: `3px solid ${st.color}`, borderRadius: 12, padding: '13px 15px' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', gap: 10, alignItems: 'flex-start', flexWrap: 'wrap' }}>
                  ...
                  <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                    ...
                    {!final && <>
                      <button onClick={() => act(co.id, 'advance')} disabled={busy[co.id]} style={{ padding: '6px 11px', borderRadius: 8, border: 'none', background: C.green, color: '#fff', fontWeight: 700, fontSize: 12, cursor: 'pointer' }}>Advance →</button>
                      <button onClick={() => act(co.id, 'reject')} disabled={busy[co.id]} style={{ padding: '6px 11px', borderRadius: 8, border: `1px solid ${C.red}`, background: '#0F172A', color: C.red, fontWeight: 700, fontSize: 12, cursor: 'pointer' }}>Reject</button>
                    </>}
                  </div>
                </div>
              </div>
