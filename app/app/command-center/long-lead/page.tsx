  ...
  return (
    <div style={{ padding: '28px 24px 60px', maxWidth: 1280, margin: '0 auto', fontFamily: font, color: C.text }}>
      ...
      {loading ? (
        ...
        rows.length === 0 ? (
          <EmptyState icon="📦" title="No long-lead items yet"
          ...
          {filtered.map(({ it, h }) => (
            <LiftCard key={it.id} sev={h.severity}>
                ...
                return (
                  <div style={{ marginTop: 12 }}>
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 8, marginBottom: 5 }}>
                      ...
                      {!isLast && (
                        <button onClick={() => advance(it)} disabled={busy[it.id]} style={{ fontSize: 11, fontWeight: 700, color: busy[it.id] ? C.dim : C.blue, background: 'none', border: 'none', cursor: busy[it.id] ? 'default' : 'pointer' }}>Advance →</button>
                      )}
                    </div>
                    <div style={{ display: 'flex', gap: 4 }}>
                      {LONGLEAD_LIFECYCLE.map((s, i) => (
                        <div key={i} title={s} style={{ flex: 1, height: 5, borderRadius: 3, background: i <= lc.index ? fill : '#16243A' }} />
                      ))}
                    </div>
                  </div>
                );
              ...
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 5, marginTop: 12, alignItems: 'center' }}>
                {it.po_number && <span style={{ fontSize: 11, fontWeight: 600, padding: '3px 8px', borderRadius: 6, background: '#16243A', color: C.dim }}>PO {it.po_number}</span>}
                {it.unit_cost != null && <span style={{ fontSize: 11, fontWeight: 600, padding: '3px 8px', borderRadius: 6, background: '#16243A', color: C.dim }}>{fmtMoney((num(it.unit_cost) ?? 0) * (num(it.quantity) ?? 1))}</span>}
