...
export default function PortalsPage() {
  ...
  return (
    <div style={{ padding: '28px 24px 60px', maxWidth: 1000, margin: '0 auto', fontFamily: font, color: C.text }}>
      ...
      : list.length === 0 ? (
        <EmptyState icon="🔗" title="No owner portals yet" body="Generate a private link for each franchisee — they'll see live progress, milestones, and budget status for their location without touching your operations." />
      ) : (
            ...
            <div key={p.id} style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 12, padding: '14px 16px' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', gap: 12, alignItems: 'flex-start', flexWrap: 'wrap' }}>
                ...
                <div style={{ display: 'flex', gap: 8, flexShrink: 0 }}>
                  <Link href={p.url} target="_blank" style={{ padding: '7px 12px', borderRadius: 8, border: `1px solid ${C.border}`, background: '#16243A', color: C.blue, fontWeight: 700, fontSize: 13, textDecoration: 'none' }}>Preview</Link>
                  <button onClick={() => copy(p.url, p.id)} style={{ padding: '7px 12px', borderRadius: 8, border: 'none', background: copied === p.id ? C.green : C.gold, color: '#fff', fontWeight: 700, fontSize: 13, cursor: 'pointer' }}>{copied === p.id ? '✓ Copied' : 'Copy Link'}</button>
                  <button onClick={() => revoke(p.id)} style={{ padding: '7px 12px', borderRadius: 8, border: `1px solid ${C.border}`, background: '#16243A', color: C.red, fontWeight: 700, fontSize: 13, cursor: 'pointer' }}>Revoke</button>
                </div>
              </div>
            </div>
          ))}
