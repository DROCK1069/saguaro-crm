...
const METHOD_META: Record<string, { label: string; icon: string; linkLabel: string }> = {
  photo:      { label: 'Photo',            icon: '📷', linkLabel: 'Evidence photo URL' },
  drone:      { label: 'Drone',            icon: '🛩️', linkLabel: 'Drone footage URL' },
  '360':      { label: '360° Walkthrough', icon: '🌐', linkLabel: '360° walkthrough URL' },
  matterport: { label: 'Matterport',       icon: '🏠', linkLabel: 'Matterport scan URL' },
  facetime:   { label: 'Live Video',       icon: '📹', linkLabel: 'Video-call / FaceTime link' },
  video:      { label: 'Video',            icon: '🎥', linkLabel: 'Video URL' },
  site_visit: { label: 'On-Site Visit',    icon: '📍', linkLabel: 'Scheduling / calendar link' },
};
    ...
    <div style={{ padding: '28px 24px 60px', maxWidth: 1200, margin: '0 auto', fontFamily: font, color: C.text }}>
      <PageHeader
        ...
        right={<div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', justifyContent: 'flex-end' }}>
          <button onClick={() => setWalkthrough(true)} style={{ padding: '9px 14px', borderRadius: 10, border: `1px solid ${C.blue}`, background: '#0F172A', color: C.blue, fontWeight: 800, fontSize: 14, cursor: 'pointer' }}>📹 Start Live Walkthrough</button>
          <button onClick={() => setRequesting({ method: 'site_visit' })} style={{ padding: '9px 14px', borderRadius: 10, border: `1px solid ${C.border}`, background: '#0F172A', color: C.text, fontWeight: 800, fontSize: 14, cursor: 'pointer' }}>📍 Request On-Site Visit</button>
      ...
      {loading ? (
        ...
        (verifications as any[]).length === 0 ? (
          <EmptyState icon="📸" title="No verifications yet"
            ...
            return (
              <div key={v.id} style={{ background: C.card, border: `1px solid ${C.border}`, borderLeft: `4px solid ${STATUS_COLOR[st]}`, borderRadius: 14, padding: '15px 16px', boxShadow: '0 1px 2px rgba(0,0,0,0.05)' }}>
                ...
                <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginTop: 10, alignItems: 'center' }}>
                  {v.method && METHOD_META[v.method] && <span style={{ fontSize: 11, fontWeight: 700, color: C.text, background: '#16243A', padding: '3px 9px', borderRadius: 6 }}>{METHOD_META[v.method].icon} {METHOD_META[v.method].label}</span>}
                ...
                {v.media_url && <a href={v.media_url} target="_blank" rel="noreferrer" style={{ display: 'inline-block', fontSize: 12, fontWeight: 700, color: C.blue, marginTop: 8, textDecoration: 'none' }}>{METHOD_META[v.method]?.icon || '🔗'} View {METHOD_META[v.method]?.label?.toLowerCase() || 'evidence'}</a>}
                {v.photo_url && <a href={v.photo_url} target="_blank" rel="noreferrer" style={{ display: 'inline-block', fontSize: 12, fontWeight: 700, color: C.blue, marginTop: 8, marginLeft: v.media_url ? 12 : 0, textDecoration: 'none' }}>📷 View evidence photo</a>}

                {st === 'pending' ? (
                  <div style={{ display: 'flex', gap: 8, marginTop: 14 }}>
                    <button onClick={() => decide(v, 'verified')} disabled={busy[v.id]} style={{ flex: 1, padding: '8px', borderRadius: 9, border: 'none', background: C.green, color: '#fff', fontWeight: 800, fontSize: 13, cursor: 'pointer' }}>✓ Verify</button>
                    <button onClick={() => decide(v, 'rejected')} disabled={busy[v.id]} style={{ flex: 1, padding: '8px', borderRadius: 9, border: `1px solid ${C.red}`, background: '#0F172A', color: C.red, fontWeight: 800, fontSize: 13, cursor: 'pointer' }}>✕ Reject</button>
                  </div>
                ) : (
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: 12, fontSize: 12, color: C.dim }}>
                    <span>{st === 'verified' ? '✓ Verified' : '✕ Rejected'}{v.verified_by ? ` by ${v.verified_by}` : ''}{v.verified_at ? ` · ${fmtDate(v.verified_at)}` : ''}</span>
...
function LiveWalkthroughModal({ projects, onClose }: { projects: any[]; onClose: () => void }) {
  ...
  const inp: React.CSSProperties = { padding: '9px 11px', borderRadius: 8, border: `1px solid ${C.border}`, fontSize: 14, outline: 'none', background: '#16243A', width: '100%' };
  ...
  const PLATFORMS: Record<string, { label: string; icon: string; instant: boolean; url?: string }> = {
    meet:     { label: 'Google Meet (new instant meeting)', icon: '📹', instant: true, url: 'https://meet.google.com/new' },
    zoom:     { label: 'Zoom (start instant meeting)',      icon: '🔵', instant: true, url: 'https://zoom.us/start/videomeeting' },
    facetime: { label: 'FaceTime (call an address)',        icon: '🍏', instant: false },
    custom:   { label: 'Paste a meeting link',              icon: '🔗', instant: false },
  };
  ...
  return (
    <div onClick={onClose} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.4)', zIndex: 1000, display: 'flex', alignItems: 'flex-start', justifyContent: 'center', padding: '8vh 16px', overflow: 'auto' }}>
      <div onClick={(e) => e.stopPropagation()} style={{ background: C.card, borderRadius: 16, padding: 24, width: 'min(540px, 100%)', boxShadow: '0 24px 80px rgba(0,0,0,0.3)', fontFamily: font }}>
        <div style={{ fontSize: 19, fontWeight: 900, marginBottom: 4 }}>📹 Start Live Walkthrough</div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
          ...
          <div><label style={lbl}>Platform</label><select style={inp} value={f.platform} onChange={(e) => set('platform', e.target.value)}>{Object.entries(PLATFORMS).map(([k, v]) => <option key={k} value={k}>{v.icon} {v.label}</option>)}</select></div>
        ...
        <div style={{ display: 'flex', gap: 10, marginTop: 18 }}>
          <button onClick={launch} disabled={busy} style={{ padding: '10px 20px', borderRadius: 10, border: 'none', background: busy ? C.dim : C.blue, color: '#fff', fontWeight: 800, cursor: busy ? 'default' : 'pointer' }}>{busy ? 'Launching…' : `${p.icon} Launch & log`}</button>
          <button onClick={onClose} style={{ padding: '10px 20px', borderRadius: 10, border: `1px solid ${C.border}`, background: '#0F172A', color: C.dim, fontWeight: 700, cursor: 'pointer' }}>Cancel</button>
        </div>
      </div>
    </div>
  );
    ...
    <div onClick={onClose} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.4)', zIndex: 1000, display: 'flex', alignItems: 'flex-start', justifyContent: 'center', padding: '8vh 16px', overflow: 'auto' }}>
      <div onClick={(e) => e.stopPropagation()} style={{ background: C.card, borderRadius: 16, padding: 24, width: 'min(540px, 100%)', boxShadow: '0 24px 80px rgba(0,0,0,0.3)', fontFamily: font }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
          ...
          <div><label style={lbl}>Method</label><select style={inp} value={f.method || 'photo'} onChange={(e) => set('method', e.target.value)}>{METHOD_OPTIONS.map((m) => <option key={m} value={m}>{METHOD_META[m].icon} {METHOD_META[m].label}</option>)}</select></div>
          <div><label style={lbl}>Gates stage</label><select style={inp} value={f.stage || ''} onChange={(e) => set('stage', e.target.value)}><option value="">None</option>{ROLLOUT_STAGES.map((s) => <option key={s.id} value={s.id}>{s.label}</option>)}</select></div>
          <div><label style={lbl}>Requested by</label><input style={inp} value={f.requested_by || ''} onChange={(e) => set('requested_by', e.target.value)} /></div>
