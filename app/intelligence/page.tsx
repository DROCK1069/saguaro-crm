...
const GOLD='#C8881C',EYEBROW='#B07A12',DARK='#FBF3E4',RAISED='linear-gradient(180deg,#FFFBF2,#FBF3E4)',BORDER='rgba(176,122,18,0.16)',DIM='#6B5B43',TEXT='#2A1B06',GREEN='#1a8a4a',RED='#c03030';
...
export default function IntelligencePage() {
  ...
  return (
    <div style={{minHeight:'100vh',background:PAGE_BG,fontFamily:"system-ui,-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif"}}>
    <div style={{padding:'24px 28px',maxWidth:1300,margin:'0 auto'}}>
      <div style={{position:'relative',background:'linear-gradient(160deg, #FFFBF2, #FDF3E2, #FBEAD2)',borderRadius:18,padding:'28px 32px',marginBottom:24,overflow:'hidden'}}>
        ...
        <div>
          <div style={{fontSize:11,fontWeight:700,letterSpacing:2,textTransform:'uppercase' as const,color:EYEBROW}}>AI Learning Engine</div>
          <h1 style={{fontSize:30,fontWeight:800,margin:'6px 0',background:'linear-gradient(135deg,#D89A1E,#A86A0C)',WebkitBackgroundClip:'text',WebkitTextFillColor:'transparent',backgroundClip:'text'}}>Bid Intelligence</h1>
          <div style={{fontSize:13,color:DIM}}>Saguaro learns from every bid you win or lose. No competitor has this.</div>
        </div>
        <button onClick={()=>setScoring(!scoring)} style={{padding:'11px 20px',background:'linear-gradient(135deg,#E8B84B,#C98A1A)',border:'none',borderRadius:10,color:'#2A1B06',fontSize:13,fontWeight:700,cursor:'pointer',boxShadow:'0 6px 18px rgba(201,138,26,0.28)'}}>
      ...
      {scoring&&<div style={{background:RAISED,border:`1px solid rgba(212,160,23,.3)`,borderRadius:10,padding:24,marginBottom:24}}>
        ...
        <div style={{display:'flex',gap:10}}>
          <button onClick={scoreOpportunity} disabled={loading} style={{padding:'9px 20px',background:'linear-gradient(135deg,#E8B84B,#C98A1A)',border:'none',borderRadius:7,color:'#2A1B06',fontSize:13,fontWeight:800,cursor:'pointer'}}>🤖 {loading?'Scoring...':'Score with AI'}</button>
        ...
        {result&&!('error' in result)&&<div style={{marginTop:20,background:'rgba(212,160,23,.06)',border:'1px solid rgba(212,160,23,.2)',borderRadius:10,padding:20}}>
          ...
          <div style={{marginTop:12,display:'flex',gap:10}}>
            <button style={{padding:'8px 16px',background:'linear-gradient(135deg,#E8B84B,#C98A1A)',border:'none',borderRadius:7,color:'#2A1B06',fontSize:12,fontWeight:800,cursor:'pointer'}}>✅ Add to Pipeline</button>
        ...
        <div style={{background:RAISED,border:`1px solid ${BORDER}`,borderRadius:10,padding:24}}>
          ...
          {[{trade:'Residential',bids:7,wins:5,wr:71,margin:18.2},{trade:'Commercial',bids:3,wins:0,wr:0,margin:12},{trade:'Addition/Remodel',bids:2,wins:2,wr:100,margin:20}].map(t=>(
            <div key={t.trade} style={{marginBottom:12}}>
              ...
              <div style={{height:5,background:'rgba(176,122,18,0.12)',borderRadius:3}}>
        ...
        <div style={{background:RAISED,border:`1px solid ${BORDER}`,borderRadius:10,overflow:'hidden'}}>
          ...
          <table style={{width:'100%',borderCollapse:'collapse' as const,fontSize:12}}>
            ...
            <tbody>{outcomes.map((o,i)=>(
              <tr key={i} style={{borderBottom:`1px solid rgba(176,122,18,0.12)`}}>
                ...
                <td style={{padding:'11px 14px'}}>
                  ...
                  {o.outcome==='lost'&&o.reason&&<div style={{fontSize:10,color:DIM,marginTop:2}}>{o.reason}</div>}
                </td>
              </tr>
            ))}</tbody>
          </table>
