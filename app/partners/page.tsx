...
const STATUS_STYLES: Record<string, { color: string; bg: string; border: string }> = {
  ...
  Beta: { color: C.gold, bg: 'rgba(245, 158, 11,0.12)', border: 'rgba(245, 158, 11,0.30)' },
};
    ...
    <div style={{ minHeight: '100vh', background: C.dark, color: C.text, fontFamily: C.font }}>
      ...
      <div style={{ paddingTop: 64 }}>
        ...
        <section style={{ textAlign: 'center', padding: '88px 24px 56px', background: `radial-gradient(ellipse 900px 500px at 50% 0%, rgba(245, 158, 11,0.07) 0%, transparent 70%)` }}>
          <div style={{ display: 'inline-flex', alignItems: 'center', gap: 8, padding: '6px 16px', background: 'rgba(245, 158, 11,0.10)', border: `1px solid rgba(245, 158, 11,0.30)`, borderRadius: 20, fontSize: 12, fontWeight: 700, color: C.gold, letterSpacing: 1.2, textTransform: 'uppercase', marginBottom: 24 }}>
          ...
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', justifyContent: 'center', marginBottom: 36 }}>
            {CATEGORIES.map(cat => (
              <button key={cat} onClick={() => setFilter(cat)} style={{
                ...
                background: filter === cat ? 'rgba(245, 158, 11,0.14)' : 'rgba(255,255,255,0.03)',
                border: `1px solid ${filter === cat ? 'rgba(245, 158, 11,0.35)' : C.border}`,
            ...
            {PARTNER_TIERS.map(tier => (
              <div key={tier.name} style={{
                ...
                <div style={{ padding: '30px 28px 28px' }}>
                  <div style={{ width: 52, height: 52, borderRadius: 13, background: 'rgba(245, 158, 11,0.10)', border: '1px solid rgba(245, 158, 11,0.25)', display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: 18 }}>
                  ...
                  <a href="mailto:partners@saguarocontrol.net" style={{
                    ...
                    boxShadow: tier.popular ? `0 4px 14px rgba(245, 158, 11,0.30)` : 'none',
                  }}>
                    {tier.cta}
                  </a>
                </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: 0 }}>
              {STEPS.map((s, i, arr) => (
                <div key={s.step} style={{ padding: '0 28px', borderRight: i < arr.length - 1 ? `1px solid ${C.border}` : 'none' }}>
                  <div style={{ width: 44, height: 44, borderRadius: 12, background: 'rgba(245, 158, 11,0.10)', border: '1px solid rgba(245, 158, 11,0.22)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 18, fontWeight: 900, color: C.gold, marginBottom: 14 }}>
                    {s.step}
                  </div>
        ...
        <section style={{ padding: '0 24px 96px', maxWidth: 900, margin: '0 auto' }}>
          <div style={{ background: 'linear-gradient(135deg, rgba(245, 158, 11,0.10) 0%, rgba(11,11,15,0) 100%)', border: `1px solid rgba(245, 158, 11,0.30)`, borderRadius: 14, padding: '40px 48px', display: 'flex', gap: 48, alignItems: 'center', flexWrap: 'wrap', boxShadow: '0 4px 14px rgba(0,0,0,.45)' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 12, alignItems: 'center', minWidth: 220 }}>
              ...
              <a href="mailto:partners@saguarocontrol.net" style={{ padding: '14px 28px', background: `linear-gradient(135deg, ${C.gold}, ${C.goldBright})`, borderRadius: 10, color: '#0B0B0F', fontWeight: 800, fontSize: 14, textDecoration: 'none', boxShadow: `0 4px 14px rgba(245, 158, 11,0.30)`, textAlign: 'center' }}>
                Apply to Partner
              </a>
              <Link href="/signup" style={{ fontSize: 13, color: C.gold, textDecoration: 'none', fontWeight: 600 }}>Or start a free trial first →</Link>
            </div>
          </div>
        </section>
