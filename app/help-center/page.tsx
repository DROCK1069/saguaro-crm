...
const ARTICLES: Article[] = [
  ...
  // ---- Field App ----
  { cat: 'field-app', title: 'Download & log in to Saguaro Field', body: 'Install Saguaro Field from the App Store or Google Play and sign in with your Saguaro account — Face ID gets you in fast. Your assigned projects sync automatically.', tags: ['download', 'login', 'field app', 'install'] },
  ...
  { cat: 'field-app', title: 'Work offline — automatic sync on reconnect', body: 'Saguaro Field is offline-first. Daily logs, photos, punch items, and time entries queue locally with no signal and sync the moment you reconnect — nothing is lost in a dead zone.', tags: ['offline', 'sync', 'no signal'] },
...
function Chevron({ open }: { open: boolean }) {
  return (
    <svg width={18} height={18} viewBox="0 0 24 24" fill="none" stroke={C.gold} strokeWidth={2} strokeLinecap="round" strokeLinejoin="round"
      style={{ flexShrink: 0, transform: open ? 'rotate(90deg)' : 'rotate(0deg)', transition: 'transform .15s', opacity: 0.85 }}>
    ...
    <div style={{ minHeight: '100vh', background: C.dark, color: C.text, fontFamily: C.font }}>
      ...
      <div style={{ paddingTop: 64 }}>
        ...
        <section style={{ textAlign: 'center', padding: '88px 24px 56px', background: `radial-gradient(ellipse 900px 500px at 50% 0%, rgba(245, 158, 11,0.07) 0%, transparent 70%)` }}>
          ...
          <p style={{ fontSize: 'clamp(15px, 2vw, 19px)', color: C.dim, maxWidth: 600, margin: '0 auto 32px', lineHeight: 1.6 }}>
          ...
          <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', justifyContent: 'center', maxWidth: 700, margin: '20px auto 0' }}>
            ...
            {POPULAR.map(p => (
              <button key={p} onClick={() => setQuery(p)} style={{ padding: '6px 14px', background: 'rgba(255,255,255,0.04)', border: `1px solid ${C.border}`, borderRadius: 999, fontSize: 12.5, color: C.dim, fontWeight: 500, cursor: 'pointer', fontFamily: C.font }}
                onMouseEnter={e => { e.currentTarget.style.color = C.gold; e.currentTarget.style.borderColor = 'rgba(245, 158, 11,0.30)'; }}
        ...
        {searching ? (
          ...
          <section style={{ padding: '0 24px 88px', maxWidth: 820, margin: '0 auto' }}>
            <div style={{ marginBottom: 24, display: 'flex', alignItems: 'baseline', gap: 10, flexWrap: 'wrap' }}>
              <h2 style={{ fontSize: 'clamp(20px, 3vw, 26px)', fontWeight: 800, margin: 0, letterSpacing: -0.4 }}>
            ...
            {totalResults === 0 ? (
              <div style={{ textAlign: 'center', padding: '56px 24px', background: C.raisedAlt, border: `1px solid ${C.border}`, borderRadius: 14, color: C.dim, fontSize: 15, lineHeight: 1.7 }}>
              ...
              <>
                {matchedArticles.length > 0 && (
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 12, marginBottom: matchedFaqs.length ? 36 : 0 }}>
                    {matchedArticles.map(a => {
                      const open = openArticle === a.title;
                      return (
                        <div key={a.title} style={{ background: C.raisedAlt, border: `1px solid ${open ? 'rgba(245, 158, 11,0.35)' : C.border}`, borderRadius: 12, overflow: 'hidden', transition: 'border-color .15s' }}>
                          <button onClick={() => setOpenArticle(open ? null : a.title)}
                            style={{ width: '100%', textAlign: 'left', padding: '18px 20px', background: 'none', border: 'none', cursor: 'pointer', display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 14, fontFamily: C.font }}>
                            <span style={{ minWidth: 0 }}>
                              <span style={{ display: 'block', fontSize: 11, fontWeight: 700, color: C.gold, letterSpacing: 0.8, textTransform: 'uppercase', marginBottom: 4 }}>{catTitle(a.cat)}</span>
                              <span style={{ display: 'block', fontSize: 16, fontWeight: 600, color: C.text, lineHeight: 1.35 }}>{a.title}</span>
                            </span>
                            <Chevron open={open} />
                          </button>
                          {open && (
                            <div style={{ padding: '0 20px 20px', fontSize: 15, color: C.dim, lineHeight: 1.75 }}>{a.body}</div>
                          )}
                        </div>
                      );
                    })}
                ...
                {matchedFaqs.length > 0 && (
                  <div>
                    <h3 style={{ fontSize: 14, fontWeight: 700, color: C.dim, letterSpacing: 1, textTransform: 'uppercase', margin: '0 0 14px' }}>Related questions</h3>
          ...
          <>
                  ...
                  return (
                    <div key={cat.key} style={{ background: C.raisedAlt, border: `1px solid ${C.border}`, borderRadius: 14, padding: '28px 26px', boxShadow: '0 1px 2px rgba(0,0,0,.4)' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 14, marginBottom: 14 }}>
                        <div style={{ width: 46, height: 46, borderRadius: 12, background: 'rgba(245, 158, 11,0.10)', border: '1px solid rgba(245, 158, 11,0.22)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                          {cat.icon}
                        </div>
                        <div>
                        ...
                        {arts.map(a => {
                          ...
                          return (
                            <div key={a.title} style={{ borderBottom: `1px solid ${C.border}` }}>
                              <button onClick={() => setOpenArticle(open ? null : a.title)}
                                style={{ width: '100%', textAlign: 'left', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 10, padding: '12px 0', background: 'none', border: 'none', cursor: 'pointer', color: open ? C.gold : C.text, fontFamily: C.font }}
                                onMouseEnter={e => { if (!open) e.currentTarget.style.color = C.gold; }}
                                onMouseLeave={e => { if (!open) e.currentTarget.style.color = C.text; }}>
                                <span style={{ fontSize: 13.5, fontWeight: 500, lineHeight: 1.4 }}>{a.title}</span>
                                <Chevron open={open} />
                              </button>
                              {open && (
            ...
            <section style={{ padding: '0 24px 88px', maxWidth: 800, margin: '0 auto' }}>
              <div style={{ textAlign: 'center', marginBottom: 44 }}>
                <div style={{ display: 'inline-block', padding: '5px 14px', background: 'rgba(245, 158, 11,0.12)', border: `1px solid rgba(245, 158, 11,0.25)`, borderRadius: 999, fontSize: 11, fontWeight: 700, color: C.gold, letterSpacing: 1.5, textTransform: 'uppercase', marginBottom: 16 }}>Common Questions</div>
        ...
        <section style={{ padding: '0 24px 96px', maxWidth: 980, margin: '0 auto' }}>
          <div style={{ background: 'linear-gradient(135deg, rgba(245, 158, 11,0.10) 0%, rgba(11,11,15,0) 100%)', border: `1px solid rgba(245, 158, 11,0.30)`, borderRadius: 14, padding: '44px 48px', display: 'flex', gap: 40, alignItems: 'center', flexWrap: 'wrap', boxShadow: '0 4px 14px rgba(0,0,0,.45)' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 12, alignItems: 'stretch', minWidth: 220 }}>
              <Link href="/contact" style={{ padding: '15px 28px', background: `linear-gradient(135deg, ${C.gold}, ${C.goldBright})`, borderRadius: 10, color: '#0B0B0F', fontWeight: 800, fontSize: 15, textDecoration: 'none', boxShadow: `0 4px 14px rgba(245, 158, 11,0.30)`, textAlign: 'center' }}>
                Contact Support
              </Link>
              <Link href="/signup" style={{ padding: '15px 28px', background: 'rgba(255,255,255,0.05)', border: `1px solid ${C.border}`, borderRadius: 10, color: C.text, fontWeight: 700, fontSize: 15, textDecoration: 'none', textAlign: 'center' }}>
                Start Free Trial
              </Link>
