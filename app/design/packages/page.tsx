...
const BLUE = '#F59E0B', PURPLE = '#AF52DE', AMBER = '#FF9500';
...
export default function PackagesPage() {
  ...
  const handleCustomize = async () => {
    ...
    setCustomizing(true);
    setCustomizeError('');
    // Real local-pricing: resolve ZIP -> state utility rates and adjust the
    // package numbers server-side. No fake success — if the ZIP can't be
    // resolved we surface the actual error instead of flipping to "customized".
    try {
      const res = await fetch('/api/smart-packages/customize-public', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ zip: zipCode, packages }),
      });
      const data = await res.json().catch(() => null);
      if (!res.ok) {
        setCustomizeError(data?.error || 'Could not get local pricing. Please try again.');
        return;
      }
      if (Array.isArray(data?.packages) && data.packages.length) setPackages(data.packages);
      setGeoData(data?.geo || null);
      setLocalApplied(!!data?.applied);
      setCustomized(true);
    } catch {
      setCustomizeError('Could not get local pricing — check your connection and try again.');
    } finally {
    ...
    <div style={{ minHeight: '100vh', background: BG, color: TEXT }}>
      ...
      <div style={{ maxWidth: 1200, margin: '0 auto', padding: '0 20px 80px' }}>
        ...
        <div style={{
          ...
          {customized && geoData && localApplied && (
            <span style={{ fontSize: 13, color: GREEN }}>
              Showing local rates for {geoData.city || 'your area'}{geoData.state ? `, ${geoData.state}` : ''}
            </span>
          )}
          {customized && geoData && !localApplied && (
            <span style={{ fontSize: 13, color: DIM }}>
              Located {geoData.city || 'your area'}{geoData.state ? `, ${geoData.state}` : ''} — no local rate data yet, showing national averages
            </span>
          )}
          {customizeError && (
            <span style={{ fontSize: 13, color: AMBER }}>{customizeError}</span>
          )}
        </div>
