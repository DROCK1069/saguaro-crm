      ...
      <div style={{ maxWidth: 440, width: '100%' }}>
        ...
        <div style={{ background: '#0F172A', backdropFilter: 'blur(40px)', border: '1px solid rgba(255,255,255,0.12)', borderRadius: 16, padding: 32 }}>
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            <input type="text" value={token} onChange={(e) => setToken(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && token.trim() && router.push(`/portals/w9/${token.trim()}`)} placeholder="Paste token from email" style={{ flex: 1, padding: '12px 16px', background: '#16243A', border: '1px solid rgba(255,255,255,0.12)', borderRadius: 10, color: '#FFFFFF', fontSize: 14, outline: 'none' }} />
