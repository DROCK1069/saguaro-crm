...
export async function createCheckoutHandler(request: NextRequest) {
  ...
  // Resolve tenant + email from the authenticated session (mirrors
  // getSubscriptionHandler) so the client never has to supply them. Body
  // values remain a fallback for server-to-server / scripted callers.
  const sessionUser = await getUser(request);

  const planId   = String(body.planId   ?? '');
  const interval = String(body.interval ?? 'monthly') as 'monthly' | 'annual';
  const tenantId = sessionUser?.tenantId ?? String(body.tenantId ?? '');
  const email    = (sessionUser?.email || String(body.email ?? '')).toLowerCase().trim();

  if (!planId) return err('planId is required');
  if (!tenantId || !email) {
    return err('You must be signed in to start checkout', 401);
  }
...
export async function billingPortalHandler(request: NextRequest) {
  const body = await request.json().catch(() => null);
  // Resolve tenant from the session (mirrors getSubscriptionHandler); body is fallback.
  const sessionUser = await getUser(request);
  const tenantId = sessionUser?.tenantId ?? String(body?.tenantId ?? '');
  if (!tenantId) return err('You must be signed in to manage billing', 401);
...
export async function cancelHandler(request: NextRequest) {
  ...
  const sessionUser = await getUser(request);
  const tenantId = sessionUser?.tenantId ?? String(body?.tenantId ?? '');
  if (!tenantId) return err('You must be signed in to cancel', 401);
  ...
  const r = resend();
  const cancelEmail = (body?.email as string | undefined) ?? sessionUser?.email;
  if (r && cancelEmail) {
    await r.emails.send({
      from: FROM,
      to:   cancelEmail,
