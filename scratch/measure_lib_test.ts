...
import {
  ...
  snapEndpoint, snapOrtho, snapGrid, toSY, toSquares, toCF, measureCondition,
...
const p = measureCondition('pitch', { points: rect40x60, ppf: 1, riseOver12: 6 });
ok('measureCondition(pitch) 2400 plan → sloped 2683.28', near(p.value, 2683.28, 0.1), p.value);
const v = measureCondition('volume', { points: rect40x60, ppf: 1, depthFt: 0.5 });
ok('measureCondition(volume) → 44.44 CY', v.value === 44.44, v.value);
const a1 = measureCondition('area', { points: rect40x60, holes: [hole10], ppf: 1 });
const a2 = measureCondition('area', { points: rect40x60, holes: [hole10], ppf: 1 });
ok('measureCondition(area) net = 2300 + deterministic', a1.value === 2300 && JSON.stringify(a1) === JSON.stringify(a2));
