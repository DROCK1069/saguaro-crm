...
export function netAreaSF(outer: Pt[], holes: Pt[][], ppf: number) {
  ...
  for (const h of holes) {
    holesSF += grossSF(h, ppf);
    contained.push(h.every((p) => pointInPolygon(p, outer)) || pointInPolygon(centroid(h), outer));
  }
  const net = Math.max(0, r2(gross - holesSF));
...
export type MeasureKind = 'linear' | 'area' | 'count' | 'pitch' | 'volume' | 'arc' | 'deduction';
...
export function measure(kind: MeasureKind, input: MeasureInput): MeasureResult {
