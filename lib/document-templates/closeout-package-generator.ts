...
export async function generateCloseoutPackage(projectId: string): Promise<{
  ...
  coverPage.drawText(toWinAnsi('SAGUARO CRM'), {
    x: 40,
    y: height - 50,
    size: 14,
    font: bold,
  ...
  coverPage.drawText(toWinAnsi('PROJECT CLOSEOUT PACKAGE'), {
    x: 40,
    y: height - 80,
    size: 24,
    font: bold,
  ...
  coverPage.drawText(toWinAnsi((project as any)?.name || 'Project'), {
    x: 40,
    y: height - 108,
    size: 16,
    font: bold,
  ...
  coverPage.drawText(toWinAnsi((project as any)?.address || ''), {
    x: 40,
    y: height - 126,
    size: 10,
    font,
  ...
  coverPage.drawText(toWinAnsi(`Prepared: ${new Date().toLocaleDateString()}`), {
    x: 40,
    y: height - 144,
    size: 10,
    font,
  ...
  detailRows.forEach(([label, value]) => {
    coverPage.drawText(toWinAnsi(label), {
      x: 40,
      y,
      size: 10,
      font: bold,
    ...
    coverPage.drawText(toWinAnsi(value), {
      x: 220,
      y,
      size: 10,
      font,
  ...
  // Checklist table
  y -= 50;
  coverPage.drawText(toWinAnsi('DOCUMENT CHECKLIST'), {
    x: 40,
    y,
    size: 12,
    font: bold,
  ...
  for (const category of categories) {
    y -= 18;
    coverPage.drawText(toWinAnsi(category.toUpperCase()), {
      x: 40,
      y,
      size: 8,
      font: bold,
    ...
    for (const item of catItems) {
      ...
      coverPage.drawText(toWinAnsi(item.title), {
        x: 55,
        y,
        size: 8.5,
      ...
      coverPage.drawText(
        toWinAnsi(item.completed ? 'INCLUDED' : item.required ? 'MISSING' : 'N/A'),
        {
          x: width - 100,
          y,
          size: 8,
