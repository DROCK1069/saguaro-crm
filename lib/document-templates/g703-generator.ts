...
export async function generateG703(
  ...
  while (pageItems.length > 0) {
    ...
    page.drawText(toWinAnsi('AIA G703 \u2014 CONTINUATION SHEET'), {
      x: 10,
      y: height - 25,
      size: 11,
      font: bold,
    ...
    page.drawText(
      toWinAnsi(`${(project as any)?.name || 'Project'}  |  App #${appNumber}  |  Period To: ${periodTo}  |  Page ${pageNum}`),
    ...
    batch.forEach((item, i) => {
      ...
      const pct =
        item.scheduledValue > 0
          ...
          : '0';
      page.drawText(toWinAnsi(item.itemNo), { x: 12, y: rowY - 10, size: 8, font });
      page.drawText(toWinAnsi(item.description.slice(0, 28)), {
        x: 50,
        y: rowY - 10,
        size: 8,
        font,
      ...
      page.drawText(toWinAnsi(fmtCurrency(item.scheduledValue)), {
        x: 213,
        y: rowY - 10,
        size: 8,
        font,
      ...
      page.drawText(toWinAnsi(fmtCurrency(item.prevCompleted)), {
        x: 286,
        y: rowY - 10,
        size: 8,
        font,
      ...
      page.drawText(toWinAnsi(fmtCurrency(item.thisPeriod)), {
        x: 354,
        y: rowY - 10,
        size: 8,
        font: bold,
      ...
      page.drawText(toWinAnsi(fmtCurrency(item.storedMaterials || 0)), {
        x: 422,
        y: rowY - 10,
        size: 8,
        font,
      ...
      page.drawText(toWinAnsi(`${fmtCurrency(total)} (${pct}%)`), {
        x: 480,
        y: rowY - 10,
        size: 8,
        font: bold,
    ...
    page.drawText(toWinAnsi('TOTALS'), {
      x: 50,
      y: totY - 10,
      size: 8,
      font: bold,
    ...
    page.drawText(toWinAnsi(fmtCurrency(totSched)), {
      x: 213,
      y: totY - 10,
      size: 8,
      font: bold,
    ...
    page.drawText(toWinAnsi(fmtCurrency(totPrev)), {
      x: 286,
      y: totY - 10,
      size: 8,
      font: bold,
    ...
    page.drawText(toWinAnsi(fmtCurrency(totThis)), {
      x: 354,
      y: totY - 10,
      size: 8,
      font: bold,
    ...
    page.drawText(toWinAnsi(fmtCurrency(totPrev + totThis + totStored)), {
      x: 480,
      y: totY - 10,
      size: 8,
      font: bold,
    ...
    page.drawText(
      toWinAnsi(`Generated by Saguaro CRM  \u2022  ${new Date().toLocaleDateString()}`),
