...
export async function generatePrevailingWage(input: PrevailingWageInput): Promise<{
  ...
  page.drawText(toWinAnsi('PREVAILING WAGE RATE DETERMINATION'), {
    x: 10,
    y: height - 26,
    size: 12,
    font: bold,
  ...
  // Subtitle
  let y = height - 55;
  page.drawText(toWinAnsi('Davis-Bacon Act \u2014 State of Arizona'), {
  ...
  page.drawText(toWinAnsi('TRADE CLASSIFICATION'), {
    x: 15, y: y + 1, size: 7.5, font: bold, color: rgb(1, 1, 1),
  });
  page.drawText(toWinAnsi('BASE RATE'), {
    x: 250, y: y + 1, size: 7.5, font: bold, color: rgb(1, 1, 1),
  });
  page.drawText(toWinAnsi('FRINGE BENEFITS'), {
    x: 350, y: y + 1, size: 7.5, font: bold, color: rgb(1, 1, 1),
  });
  page.drawText(toWinAnsi('TOTAL RATE'), {
  ...
  for (let i = 0; i < ARIZONA_RATES.length; i++) {
    ...
    page.drawText(toWinAnsi(rate.trade), {
      x: 15, y: y + 2, size: 9, font, color: rgb(0.1, 0.1, 0.1),
    });
    page.drawText(toWinAnsi(fmtCurrency(rate.baseRate)), {
      x: 250, y: y + 2, size: 9, font, color: rgb(0.2, 0.2, 0.2),
    });
    page.drawText(toWinAnsi(fmtCurrency(rate.fringe)), {
      x: 350, y: y + 2, size: 9, font, color: rgb(0.2, 0.2, 0.2),
    });
    page.drawText(toWinAnsi(fmtCurrency(rate.total)), {
  ...
  y -= 14;
  page.drawText(
    toWinAnsi('Rates are approximate and subject to change. Verify current rates at sam.gov before use.'),
    { x: 10, y, size: 8.5, font, color: rgb(0.3, 0.3, 0.3) }
  );
  y -= 12;
  page.drawText(
    toWinAnsi('These rates are based on publicly available Davis-Bacon wage determinations for the State of Arizona.'),
    { x: 10, y, size: 8.5, font, color: rgb(0.3, 0.3, 0.3) }
  );
  y -= 12;
  page.drawText(
    toWinAnsi(`County: ${county}. Contractors must comply with all applicable federal and state prevailing wage requirements.`),
    { x: 10, y, size: 8.5, font, color: rgb(0.3, 0.3, 0.3) }
  );

  // Footer
  page.drawText(
    toWinAnsi(`Generated by Saguaro CRM  \u2022  ${new Date().toLocaleDateString()}  \u2022  Prevailing Wage`),
