...
export async function generateA101(input: A101Input): Promise<{
  ...
  function wrapText(page: any, text: string, x: number, startY: number, maxChars: number = 95): number {
    ...
    for (const word of words) {
      if ((line + ' ' + word).length > maxChars) {
        page.drawText(toWinAnsi(line.trim()), { x, y, size: 8.5, font, color: rgb(0.2, 0.2, 0.2) });
        y -= 12;
        line = word;
      } else {
        line = line + ' ' + word;
  ...
  page1.drawText(toWinAnsi('AIA DOCUMENT A101 \u2014 OWNER-CONTRACTOR AGREEMENT'), {
    x: 10,
    y: height - 26,
    size: 12,
    font: bold,
  ...
  page1.drawText(toWinAnsi('SAGUARO CONSTRUCTION INTELLIGENCE PLATFORM'), {
    x: 10,
    y: height - 36,
    size: 7,
    font,
  ...
  // Preamble
  y -= 18;
  page1.drawText(toWinAnsi('AGREEMENT'), {
    x: 10,
    y,
    size: 11,
    font: bold,
  ...
  // Article 1
  y -= 10;
  page1.drawText(toWinAnsi('ARTICLE 1 \u2014 THE CONTRACT DOCUMENTS'), {
    x: 10,
    y,
    size: 10,
    font: bold,
  ...
  // Article 2
  y -= 10;
  page1.drawText(toWinAnsi('ARTICLE 2 \u2014 THE WORK'), {
    x: 10,
    y,
    size: 10,
    font: bold,
  ...
  // Article 3
  y -= 10;
  page1.drawText(toWinAnsi('ARTICLE 3 \u2014 DATE OF COMMENCEMENT AND SUBSTANTIAL COMPLETION'), {
    x: 10,
    y,
    size: 10,
    font: bold,
  ...
  dateRows.forEach(([label, value], i) => {
    ...
    page1.drawText(toWinAnsi(label), {
      x: 15,
      y: y + 2,
      size: 9,
      font: bold,
    ...
    page1.drawText(toWinAnsi(value), {
      x: 280,
      y: y + 2,
      size: 9,
      font,
  ...
  // Article 4
  y -= 25;
  page1.drawText(toWinAnsi('ARTICLE 4 \u2014 CONTRACT SUM'), {
    x: 10,
    y,
    size: 10,
    font: bold,
  ...
  page1.drawText(toWinAnsi('CONTRACT SUM:'), {
    x: 20,
    y: y + 7,
    size: 11,
    font: bold,
  ...
  page1.drawText(toWinAnsi(fmtCurrency(p?.contract_amount || 0)), {
    x: 380,
    y: y + 7,
    size: 13,
    font: bold,
  ...
  page1.drawText(toWinAnsi(`Retainage: ${retainagePct}%`), {
    x: 15,
    y,
    size: 9,
    font,
  ...
  // Footer page 1
  page1.drawText(
    toWinAnsi(`Generated by Saguaro CRM  \u2022  ${new Date().toLocaleDateString()}  \u2022  AIA A101`),
  ...
  // Article 5
  page2.drawText(toWinAnsi('ARTICLE 5 \u2014 PAYMENTS'), {
    x: 10,
    y,
    size: 10,
    font: bold,
  ...
  // Article 6
  y -= 15;
  page2.drawText(toWinAnsi('ARTICLE 6 \u2014 INSURANCE AND BONDS'), {
    x: 10,
    y,
    size: 10,
    font: bold,
  ...
  y -= 15;
  page2.drawText(toWinAnsi('IN WITNESS WHEREOF, the parties have executed this Agreement.'), {
    x: 10,
    y,
    size: 9,
    font,
  ...
  y -= 25;
  // Owner signature
  page2.drawText(toWinAnsi('OWNER:'), { x: 10, y, size: 9, font: bold, color: rgb(0, 0, 0) });
  page2.drawText(toWinAnsi((ctx.owner as any)?.name || ''), {
    x: 10,
    y: y - 12,
    size: 8,
    font,
  ...
  page2.drawText(toWinAnsi('CONTRACTOR:'), {
    x: 310,
    y,
    size: 9,
    font: bold,
  ...
  page2.drawText(toWinAnsi(p?.gc_entity?.name || ''), {
    x: 310,
    y: y - 12,
    size: 8,
    font,
