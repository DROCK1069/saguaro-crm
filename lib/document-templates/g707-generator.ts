...
export async function generateG707(input: G707Input): Promise<{
  ...
  page.drawText(toWinAnsi('AIA DOCUMENT G707 \u2014 CONSENT OF SURETY TO FINAL PAYMENT'), {
    x: 10,
    y: height - 26,
    size: 10,
    font: bold,
  ...
  // Body heading
  y -= 18;
  page.drawText(toWinAnsi('CONSENT OF SURETY'), {
  ...
  y -= 15;
  page.drawText(toWinAnsi('SURETY COMPANY'), {
    x: 10, y, size: 9, font: bold, color: rgb(0, 0, 0),
  });
  y -= 20;
  page.drawText(toWinAnsi(input.suretyName || '___________________________'), {
  ...
  y -= 10;
  page.drawText(toWinAnsi('By: Attorney-in-Fact (Signature)'), {
  ...
  y -= 18;
  page.drawText(toWinAnsi('STATE OF ________________     COUNTY OF ________________'), {
    x: 10, y, size: 8.5, font, color: rgb(0.2, 0.2, 0.2),
  });
  ...
  y -= 10;
  page.drawText(toWinAnsi('Notary Public'), {
    x: 10, y, size: 8, font, color: rgb(0.5, 0.5, 0.5),
  });
  page.drawText(toWinAnsi('Commission Expiration'), {
    x: 310, y, size: 8, font, color: rgb(0.5, 0.5, 0.5),
  });

  // Footer
  page.drawText(
    toWinAnsi(`Generated by Saguaro CRM  \u2022  ${new Date().toLocaleDateString()}  \u2022  AIA G707`),
