...
export async function generateJHA(input: JHAInput): Promise<{
  ...
  page.drawText(toWinAnsi('JOB HAZARD ANALYSIS (JHA)'), {
    x: 10,
    y: height - 26,
    size: 12,
    font: bold,
  ...
  page.drawText(toWinAnsi('TASK / ACTIVITY'), {
    x: 15, y: y + 1, size: 7.5, font: bold, color: rgb(1, 1, 1),
  });
  page.drawText(toWinAnsi('POTENTIAL HAZARD'), {
    x: 210, y: y + 1, size: 7.5, font: bold, color: rgb(1, 1, 1),
  });
  page.drawText(toWinAnsi('CONTROLS / MITIGATION'), {
  ...
  for (let i = 0; i < maxRows; i++) {
    ...
    page.drawText(toWinAnsi(row.task.slice(0, 28)), {
      x: 15, y: y + 1, size: 8, font, color: rgb(0.1, 0.1, 0.1),
    });
    page.drawText(toWinAnsi(row.hazard.slice(0, 28)), {
      x: 210, y: y + 1, size: 8, font, color: rgb(0.2, 0.2, 0.2),
    });
    page.drawText(toWinAnsi(row.control.slice(0, 35)), {
  ...
  y -= 15;
  page.drawText(toWinAnsi('REQUIRED PPE'), {
  ...
  y -= 10;
  page.drawText(toWinAnsi('EMERGENCY CONTACTS'), {
    x: 10, y, size: 10, font: bold, color: rgb(0, 0, 0),
  });
  y -= 14;
  ...
  // Footer
  page.drawText(
    toWinAnsi(`Generated by Saguaro CRM  \u2022  ${new Date().toLocaleDateString()}  \u2022  JHA`),
