...
export async function generatePreliminaryNotice(input: PreliminaryNoticeInput): Promise<{
  ...
  page.drawText(toWinAnsi('PRELIMINARY TWENTY-DAY NOTICE'), {
    x: 10,
    y: height - 26,
    size: 13,
    font: bold,
  ...
  // Legal citation
  let y = height - 60;
  page.drawText(toWinAnsi('Pursuant to Arizona Revised Statutes \u00A733-992.01'), {
    x: 10,
    y,
    size: 9,
    font: bold,
  ...
  // Notice body
  y -= 18;
  page.drawText(toWinAnsi('NOTICE IS HEREBY GIVEN THAT:'), {
    x: 10,
    y,
    size: 11,
    font: bold,
  ...
  for (const word of noticeWords) {
    if ((noticeLine + ' ' + word).length > 95) {
      page.drawText(toWinAnsi(noticeLine.trim()), {
        x: 10,
        y,
        size: 8.5,
        font,
  ...
  y -= 5;
  page.drawText(toWinAnsi('WARNING'), {
    x: 15,
    y,
    size: 9,
    font: bold,
  ...
  for (const word of warningWords) {
    if ((warningLine + ' ' + word).length > 95) {
      page.drawText(toWinAnsi(warningLine.trim()), {
        x: 15,
        y,
        size: 8,
        font,
  ...
  page.drawText(toWinAnsi('CLAIMANT:'), { x: 10, y, size: 9, font: bold, color: rgb(0, 0, 0) });
  ...
  page.drawText(toWinAnsi(claimantName), {
    x: 10,
    y: y - 30,
    size: 8,
    font,
  ...
  // Footer
  page.drawText(
    toWinAnsi(`Generated by Saguaro CRM  \u2022  ${new Date().toLocaleDateString()}  \u2022  Preliminary 20-Day Notice`),
