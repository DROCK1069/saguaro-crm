...
function walk(dir: string, base: string, out: TileFile[]) {
  for (const name of readdirSync(dir)) {
    ...
    const ct = name.endsWith('.dzi') || name.endsWith('.xml') ? 'application/xml'
...
export async function generateDziTiles(image: Buffer, opts?: { size?: number }): Promise<DziResult> {
  ...
  const stem = join(work, 'sheet'); // → sheet.dzi + sheet_files/<level>/<x>_<y>.jpg
  try {
    // format (jpeg quality) must be set BEFORE .tile() so it applies to tiles
    await sharp(image).jpeg({ quality: 82 }).tile({ size: opts?.size || 256, overlap: 1, layout: 'dz' }).toFile(`${stem}.dzi`);
