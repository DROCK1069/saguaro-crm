...
export async function sendEmail(
  ...
  try {
    ...
    const result = await resend.emails.send({
      ...
      // Forward attachments (e.g. the scheduled-report CSV) so the email that
      // says "the complete dataset is attached" actually carries the file.
      // Resend accepts { filename, content } where content is a Buffer or base64.
